...
**Investigation — which clue changes the explanation?** Predict whether revealing an earthquake will raise, lower or leave the burglary posterior unchanged, first with no calls and then with both calls. Reveal the calculation and inspect the compatible-world masses. Change John's false-call probability from 0.05 to 0.20; make a new prediction before comparing. Then fix \(A=1\) and alter that false-call entry again. It should have no effect because the selected evidence never uses the \(A=0\) row.
