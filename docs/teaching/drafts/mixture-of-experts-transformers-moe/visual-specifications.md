...
**Route tokens through experts and capacity.** Edit token/router scores, expert values, capacity, grouping and budget dimensions; manipulate supported retained images.
**See the consequence.** Show selected experts, discarded probability mass, overflow/drop routes, recombined outputs, balance terms and active/total resource counts together.
**Decision connection.** Choose routing/capacity policies from missing contributions and resource tradeoffs; auxiliary balance does not establish task quality.
...
Feedback gives the actual baseline-to-current delta and the routing contribution responsible, including unchanged cases. Use 1e−8 for hand arithmetic, 1e−5 for explicitly rounded values and an initial 2e−5 portable float32 comparison tolerance; change the last only with numerical evidence. These compare computations, not a learner's answer.
...
**Fresh problem.** routing.fresh_selected uses masses [1,3,2,4], meaning their logarithms are logits, outputs [[2,-1],[1,2],[-1,4],[3,0]], and k=2. The staged change raises expert 2's mass to 5. Display the selected expert identities and both output coordinates, with signed changes from the labeled baseline. Baseline chooses 3/1 with weights 4/7,3/7 and output [15/7,6/7]. Changed input chooses 2/3 with weights 5/9,4/9 and output [7/9,20/9]. These differ from the solved three-expert example.
...
**View and feedback.** Score bars feed path lanes and signed 2D contribution arrows. Show gained/lost paths and exact products . A mobile table can display one coordinate at a time. Phase two checks both normalizers, local gradients, ties, constant/common-shift nulls, dense reference agreement and snapshot invalidation. No training is needed.
...
**View and checks.** Matrix outlines and selected branches precede a resource table with units. No seconds or hardware-speed graph. Phase two checks default/fine fixtures, shared additions, integer bounds, safe arithmetic, units and the retained baseline state.
