...
Each investigation begins with editable entities visible and its **prediction unset**. A prediction consists of a selected outcome plus optional reasoning text, stored against a canonical key containing all active data, parameters, model and operation choices. A radio group must have no default answer. “Record prediction and run” requires an answer; the recorded answer is then shown alongside the observed result.

Every substantive edit invalidates the old result and prediction and returns to the unsolved state. Reset restores initial entities and also clears prediction, traces, step cursor and feedback. Switching a purely presentational plot/table view or stepping through an already solved trace does not invalidate the experiment. If a prior result is retained for comparison, label it “previous inputs,” do not present it as current feedback.

Prediction feedback is explanatory, not a score assigned to prose. Say what changed, what stayed invariant and which update terms explain it. Use tolerances rather than exact float equality; show values/error magnitude and a reason. An arbitrary edited input can produce a null outcome—compute it rather than forcing a preferred result.
...
Compute only on explicit run or bounded debounced updates after a new recorded prediction; no unbounded autoreplay. State must belong to this open lesson instance. No cross-lesson singleton or localStorage persistence that restores a stale answer into a different fixture.
...
**Editable entities and initial state.** Allow 1–16 signed input samples in [−10,10]; two diagonal continuous rates in [−3,0]; B and C coordinates in [−2,2]; D in [−2,2]; Δ in [.05,2]; and two initial-state coordinates in [−5,5]. Start with A=diag(−1,−2), B=[1,1], C=[1,−.5], D=.25, Δ=ln2, initial state zero, and input [1,−2,3,0]. This input differs from the worked example. An integrator preset uses A=0,B=1,C=1,D=0,Δ=.5,initial=3,input=[2,−1]. Presets restore unsolved entities and never select a prediction.

**Prediction and feedback.** For an input edit, ask whether earlier outputs can change or only that time and later outputs can change; for an initial-state edit, ask whether the initial-memory contribution matters. Also record whether the three evaluations should agree. The selected intervention and all parameters belong to the prediction key. Before recording, show input entities and diagram outlines but no final trace. Afterwards show the prediction, observed changes, maximum discrepancy and the contributing terms. The common reset/invalidation rules apply.
...
**Prediction.** Choose whether the selective schedule, constant gate or neither will have smaller absolute error from the most recent marked value at the selected read time; add optional reasoning. Markers define a target, while gates are manual teaching inputs, not fitted neural weights. The key contains the entire sequence, flags, gates, initial state, read index, comparison gate and mode. If no marker precedes the read, explain that the target is undefined and request either a marker or the separate zero-input question; do not invent a target.
...
**Prediction.** Record whether recurrent, matrix and chunked outputs should agree and which rows the selected intervention should change. The key contains all arrays, initial state, q, chosen row and operation. Initially show shapes and a causal-mask outline, not an already solved influence matrix.
...
**Prediction and input binding.** Offer “same top class,” “different top class,” or “unsure; I expect a probability change,” with optional reasoning. The original path and source label may appear before the prediction; classifier answers and the comparison remain hidden until it is recorded. The key includes source ID, all active and base coordinates, model/seed/digest, operation and evaluation mode. Changing any of these clears the prediction/result. Reset restores the original path and clears the exercise.
