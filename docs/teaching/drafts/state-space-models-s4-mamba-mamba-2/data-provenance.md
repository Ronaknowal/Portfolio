...
`trajectory_state_models.py::prepare` loads the source values as float64. It groups exact coordinate tuples, confirms all duplicate labels agree, and retains first source occurrence. There are330 unique trajectories and30 duplicate copies. Complete one-based duplicate groups and retained source IDs are in `trajectory-results.json`.
...
Raw coordinates are transformed by the fixed rule2x−1 and converted to float32, without ordinal tags. No fitted preprocessing uses validation/test data. The ordered logistic baseline uses the ninety normalized coordinates; temporal models process 45 coordinate pairs. Exact copies are removed before role creation. The byte-original source files retain all360 rows for provenance; experimental roles use330.

## Computations and retained outputs

The complete CPU program trains an ordered-coordinate C=1 logistic baseline and two small temporal classifiers: diagonal complex-mode and selective real-state mixers. Both use width16 and two residual blocks. Seeds17/41, Adam learning rate.003 and100 full-batch epochs are predeclared. Validation cross-entropy selects each neural epoch before assessment. The tested environment was Python3.12.14, NumPy2.3.5, PyTorch2.14.0+cpu and scikit-learn1.9.1, two threads and deterministic algorithms. No model or hyperparameter was tuned after inspecting assessment outcomes.

Diagonal runs contain1,487 parameters; selective runs2,287; logistic baseline1,365 coefficients/intercepts. Neural parameter counts are not matched. Assessment errors are30/60 and28/60 for both model kinds, compared with22/60 for the baseline. These are small educational runs and do not rank full S4/Mamba architectures or claim new-performer transfer.

trajectory-results.json records source roles, duplicates, histories, selected epochs, all role metrics and confusion matrices. trajectory-state-fits.npz retains selected parameter dictionaries and logits for all original source rows, plus logistic coefficients/intercepts. It contains no pickled objects; load with NumPy allow_pickle=False. Full original rows remain stored for traceability, while only the deduplicated role rows determine metrics.

investigation-checks.json and author_calculations.py retain a predetermined first validation source row7, coordinate reflection and reversed-order comparisons, reset/null and future-prefix checks, plus a hand-derived/autograd-checked readout update. Hypothetical edits do not receive certified new movement labels. Neural examples are original learned fits; raw source attribution and modification notice must accompany exported data.

Two separate algorithm evaluations of the diagonal fitted model agree within4.8e−6 in logits. The selective model has only an executed serial reference; its repeated-call agreement is not a check against an independent fused kernel. A metadata clarification removed that misleading parity label without changing trained weights or reported metrics.
