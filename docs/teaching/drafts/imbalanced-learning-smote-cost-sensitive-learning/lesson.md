...
**Investigation 2 — Two action costs cross.** Edit the two costs and a case's posterior to update both risk lines, their crossing, the actual numerical comparison and the lower-cost action. Multiplying both costs by the same positive factor changes the vertical units but leaves the decision threshold unchanged. At the crossing, either action has the same expected cost; use a declared tie rule.
