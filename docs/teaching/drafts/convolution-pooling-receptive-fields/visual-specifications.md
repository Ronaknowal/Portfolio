...
**Move a window and see every dependency.** Edit image/kernel cells, stride/dilation/padding, pooling inputs, shared-weight targets/rate and receptive-field threshold.
**See the consequence.** Synchronize the selected patch, products, output map, transpose contributions, gradient accumulation and ancestry paths. Geometry edits visibly change output size, alignment and holes rather than only a summary label.
**Decision connection.** Choose window geometry and pooling from reach, alignment, information loss and update behavior.
...
Unless a case states otherwise, exact tiny arithmetic uses tolerance 1e−8; saved float32 comparisons use their full stored numbers and a 1e−7 tie threshold. Current entity values, geometry, selected model/run/step and evidence identity must agree in all views. Presets are starting points; valid direct edits recompute the result immediately.
...
Initial live result: selected top-right output, numeric blank; Calculate and explain 5 in the current live view. Window movement recompute current outputs. Edit image values in [−5,5] and weights [−3,3], steps .5; render the current selected sum, not cached source output.
...
**Live update:** with learning rate .1, show the first weight .9, full weights [.9,−.6], outputs [−.9,1.5] and loss 1.53. Editing targets or rate recomputes both gradient contributions and the simultaneous update. Explain why lower total error does not require each output error to fall.
...
Null target equal to current outputs gives zero loss/gradients/update. Free edit targets [−3,3], learning rate [0,.2]; show current target and reduction. Compute the comparison from the complete current inputs. The zero-rate case shows unchanged weights even with nonzero gradient.
...
Initial n8,k4,s1,d1,left1,right1 is unsolved for target output8. The live readout shows its actual output size7; show it immediately. Construction asks learner to achieve output8 with total padding3, each side≤2, while matching requested first center1. Only left1/right2 matches both; left2/right1 has output8 but center0, a useful rejected contrast. Left0/right3 is excluded by the stated bound. Null k1,s1,p0 is identity geometry, but weight values still determine the operator.
...
Home: §5, fixtures.pooling. Side-by-side max winner paths and average contribution paths share an editable small input. For [1,4,3], k2,s1, output [4,4], gradient of output sum [0,2,0]. current comparison asks middle gradient; grade2. Contrast [3,1,4] yields[3,4] and[1,0,1].
...
The live readout shows r after second pooling; grade10. Optional selected output then shows center=a+index*j and traced discrete ancestors. Padding is hatched outside input; bounding width and unique observed coordinate count are separate labels.
...
Branch overlay uses equal output dimensions but different a to illustrate misalignment; Calculate and explain no new merge task unless the implemented controls really edit both paths. The source's deeper practice stride2→dilation2 has nine connected offsets in an11-wide bound with holes±2; retain as changed transfer.
...
**Evidence comparison:** display both selected measured models immediately. Seed 1 dense CE .07949448 versus max-CNN .08332337 is a contrast; seed 3 reverses it. Select recorded updates 0/1/25/100/200/400, plotting only those actual samples and labeling connecting segments as interpolation. Reset restores the seed-1 pair. No browser training is implied.

A separate stress table shows all recorded right/down zero-fill shifts and unchanged label convention, with clipping caveat. No unrecorded predictions per shifted specimen. Explain changed protocol versus tensor equivariance. Null same model/seed/step comparison must Calculate and explain equal. Counts always show denominators; CE is mean over rows. Construction is the written changed-training protocol practice, not a misleading browser “train” button.
...
Live observation: display the width that remains at the selected depth and relative-to-peak threshold, updating it when either control changes. Initial depth20/threshold1% grades21; depth10/1% grades15. General threshold in[.001,.2] recomputes coordinates satisfying value≥threshold*peak. Compute the comparison from the complete current inputs. Discuss differing support/concentration growth from those measured widths separately; do not ask a fixed yes/no question whose answer cannot change across the allowed settings.
...
Home: §9, fixtures.adjoint and transposed_coverage. Matrix view pairs C rows with local windows; scatter view sends each dual input into C-transpose columns. Initial x[1,3,2],g[2,−3], inner products−7, transpose[2,−5,3]. The live readout shows middle transpose coordinate−5. Null g0 yields0 but does not establish invertibility; C-transpose Cx[−2,3,−1] contrasts withx.
