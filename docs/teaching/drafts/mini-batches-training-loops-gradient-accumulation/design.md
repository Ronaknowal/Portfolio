...
| Step microbatches with separate weights/gradients/counters | Figure B and investigation 1 retain this exact teaching question; entity editing, immediate results and fault/null cases make it an investigation |
...
| Backward deposits derivatives; optimizer changes state | Three-row derivative table, three storage lanes, executed scalar program | Fresh row edits, before-backward/after-step state comparison, policy comparison | Core |
...
4. **Investigations:** all three use fresh initial problems and a live computed readout, entity edits, contrasts and null cases. The reread found that a mandatory live comparison would prevent answering the no-target-mass case; the specification now explicitly offers an immediately computed `undefined: no target mass` answer. It also clarifies exact row tuples and the step-and-clear fault policy. Numerical fixtures have author calculations; output readouts, invalidation/reset and result checks are detailed specifications, not implemented evidence.
...
9. **Screenshots:** none in phase one. Specs name required informative changed/null/live-output states, desktop/phone/zoom rendering and keyboard checks for phase two. No screenshot or accessibility pass claimed.
...
**Topic disposition:** Separate examples, gradient buffers and updates. Edit rows, microbatch boundaries, learning rate, clearing/step policy, target weights and normalization groups. Populate row model outputs/errors/gradients immediately; step backward and optimizer events with distinct clocks and buffers. Compare final policies using the same rows and initial state. Decide when accumulation is equivalent, what receives influence and why partition-sensitive operations change the computation.
...
| --- | --- | --- | --- | --- | --- |
| Accumulation, unequal denominators and tail groups | lesson.md §§2–5; author_checks.py::gradient/scalar_paths; train_iris.py::train | trace_update.py; PyTorch backward/zero_grad/SGD | same group objective, per-chunk gradient additions, one parameter/state update | Practice 1–4; new 7-row microbatch solution | Scratch derivative/optimizer owners actually implemented and linked |
| Mode/normalization/randomness and boundary operations | author_checks.py::normalization_case/dropout_gradient; lesson.md §§7–8 | PyTorch modes, gradient clipping, AMP API described | same per-example computation needed; unscale/clip/update ordering | Practice 5; normalization investigation | DDP/AMP described with operation contracts; no claimed multi-device runtime evidence |
