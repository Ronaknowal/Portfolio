...
Every graded investigation starts with prediction = null and the reveal closed. Bind a prediction to its task ID, input revision, model hash, configuration and selected output. An input change invalidates the old prediction, grade and stale worker response. After a worked demonstration, require a genuinely changed input. Compute its result; do not grade a replay of the nearby printed answer. Null changes are legitimate findings.

Use explicit Run, Step, Previous and Reset controls. Reset restores the selected fixture, round 0, initial inputs, null prediction and closed feedback. Keep original and edited states distinct. Async jobs carry the exact revision; discard older responses. Debounce dragging or require Run to avoid queued calculations. Do not train models or fetch external datasets in the browser.
...
**Placement:** sections 1–2. A stepped explanation; it need not be a graded lab.
...
Since the first answer appears in the prose, graded mode starts with the fresh scores [ln3,0,ln2,0] and values [4,−1,7,2], or a learner-created edit. Keep 27/7 hidden until commitment. Grade a numeric output with absolute/relative tolerance 1e−6, against the current computation. A separate categorical task asks whether an edit changes the output. After reveal, explain the signed contributions and actual denominator.
...
Start an assessment with L = 12, P = 3 or an unsolved user swap. Ask which arrangement has lower summed round-critical work, allowing a tie. Grade computed counts, not an assumption that striping always wins. Show both useful-pair total and executed-cell count. At coarse 4×4 granularity in the L16/P4 example, all three arrangements reach 64: retain this null.
...
Check arbitrary swaps, ownership conservation, tile boundaries, global masks, packed and empty rows, target alignment, weighted loss, computed grading, reset and accessible tables.
...
For the scalar worked input and upstream gradient 1, dV = [1,2,4,1]/8 and dS = [−.0625,1.375,−.75,−.5625]. Edit an actual value coordinate or upstream gradient in [−3,3], predict a selected derivative's sign, then compute it. Use a fresh input for grading once the worked answer is visible. Zero upstream gives zero gradients; common score shifts preserve derivatives. Editing a value can change other score derivatives through the row average.
...
Start with source 77 (actual class 4, predicted 5). The fresh source is 20 (actual 1, predicted 2). Preserve both classification errors after reveal. Classification accuracy is not a grade of execution equivalence.
...
Use two initially empty predictions: will the model's output change, and should dense/ring equivalence remain? Grade the first from actual before/after probabilities and the second from the current dense/ring discrepancy. Choose tolerances from actual browser parity evidence; 1e−6 is a starting point for probability change, not an untested blanket guarantee. If asking whether the class changes, grade argmax with explicit tie handling instead of treating every numerical change as a new class.
...
Seven representation families do not require seven full widgets. Keep a figure static or stepped when that teaches the mechanism; add a graded investigation only when a fresh prediction and input change improve understanding. Preserve all written practice with closed hints and solutions.
