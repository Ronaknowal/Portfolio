...
Seven representation families serve distinct mechanisms; five can support meaningful fresh-input investigations. Static or stepped explanations remain valid where a full lab adds little. All computed states require a current comparison, exact input/model revision, actual computation, changed examples, null controls, reset/invalidation and accessible alternatives. No browser components were built.
...
Author reread checks: first-pass intuition before notation; same scalar example through stable arithmetic; dimensions/units and conditions local to their claims; real input/output preserved with actual mistakes; different visual forms for different questions; independent changed practice; useful source annotations with honest review extents; appropriate same-module sequence links. Eight paired written-practice hint/solution disclosures remain closed by default; the live lab displays its initial result immediately. No training plots or hardware timings were invented.
...
| --- | --- | --- | --- | --- | --- |
| Stable online softmax, global masks and local ownership | ring-attention-reference.py::ring_attention/dense_attention | distributed_ring.py::ring_forward/rotate/layout | global contiguous offsets and uneven packet lengths; dense SDPA oracle | Practice 1–5; three-rank eight-position exercise | Complete CPU/Gloo ring, no fused or overlapped performance claim |
| Manual blockwise backward and gradient return | ring-attention-reference.py::blockwise_backward; distributed_ring.py::ring_backward | torch.distributed.batch_isend_irecv/P2POp and autograd comparison | P−1 forward vs P backward transfers; dQ stays local; dK/dV return home | Practice 6–8; omitted-final-rotation solution | New real-process execution deferred; numerical code complete |
| Communication/storage models, Ulysses and serving distinctions | systems-calculations.py::layout/work_counts/cost; lesson.md §§5–8,12 | Named PyTorch context-parallel and DeepSpeed integration references | actual ownership/work assumptions rather than guessed GPU curves | Practice 3–5,7 | Alternative collectives, packed documents, GPU overlap/fused kernels and serving caches remain explicitly separate extensions |
