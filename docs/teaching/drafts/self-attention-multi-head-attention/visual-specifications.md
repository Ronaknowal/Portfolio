...
For every investigation, separate **working inputs** from the last **revealed result**. Prediction starts unanswered. The learner edits a meaningful entity, records a prediction (categorical choice plus optional reasoning, or a numeric estimate where appropriate), then chooses **Reveal result**. Bind that prediction/result to all material inputs: vectors, masks, positions, row/head selections when the question depends on them, scaling, value edits, and model state. On a material edit, clear correctness feedback and mark any prior result as previous-input evidence; never show a stale answer as current. Allow exploration/reveal without forcing correctness, but do not preselect or silently manufacture a prediction. Reset restores the original inputs and clears predictions/results. Opening a worked illustration does not automatically reveal the next changed-input question.

Controls have associated labels, keyboard operation, visible focus, useful bounds and semantic buttons. Numeric input is always an alternative to drag. No autoplay or time-limited challenge; optional transition motion is short and respects reduced-motion preferences. A screen-reader summary states receiver, allowed donor count, computed weights/output and the change that was measured after reveal. It must not expose an unrevealed answer prematurely. Tables/CSV-style text views provide exact accessible alternatives to matrix heatmaps.
...
Computation is small and bounded. Recompute elementary fixtures synchronously on reveal, not on every pointer-move event. For the real model, cap the browser workspace at 60 points, two heads, width 24 and 15 classes; the original has 45 points. Recompute on reveal or an explicitly requested diagnostic, not on scroll. Lazily import its small weight asset/component when the investigation is near or opened. Full training, the 360-row source file, torch, sklearn, and native author programs do not belong in the initial reader bundle. Keep only the selected row/head details in active rendering state. Reuse shared controls/typography without forcing every diagram into the same visual composition.
...
**Workspace:** four explicit input positions A/B/C/D, query vectors and keys `[[1,0],[0,1],[1,1],[.5,-1]]`, values `[[1,0],[0,2],[3,1],[-2,3]]`. Let the learner select receiver index, edit allowed edges in that row or construct a full mask, and edit a donor value. Initial task selects zero-based receiver 1 and asks the learner to mark legal donors for a next-token causal model. Start with blank/unanswered mask choices in the challenge; offer the already-explained causal pattern only as an optional closed hint. Keep every legal-set validation local. Do not normalize an empty set; show “choose at least one legal donor” with no probability result.

**Prediction:** after setting a mask, ask whether changing future donor D.x from -2 to 4 can alter receiver B's output. Record the prediction with the full mask/value snapshot, reveal both legal and deliberately leaky comparisons, and explain which edge makes the difference. The learner can use arbitrary edits rather than a preset-only replay.
...
**Rectangular extension:** an editable query absolute position and editable key positions are the primary entities, not an opaque “causal” toggle. Default changed challenge has query 2, keys0/1/2 and the original three-token numeric fixture. Ask the learner to mark the legal cells before revealing. Correct all-allowed output is `[1,1]`; the mistaken top-left row gives `[2,0]`, checked in `author-results.json.fixtures`. Add a document-ID field as a deeper optional task: legal iff same document and key position<=query position. Use practice 2's exact sets for that check. Do not silently equate array index with absolute position.
...
**Phase-two verification:** reproduce computed null/contrast, verify renormalization and empty-row handling, check document boundary cases and rectangular alignment; confirm changing a mask invalidates previous predictions. Native checks already support arithmetic; browser behavior remains unverified.
...
Prediction starts blank and binds to scores/dimension/temperature: “Will this edit make the distribution more even, more concentrated, or unchanged?” A shared offset to both raw scores is a null; doubling their difference at fixed dimension/temperature is a contrast unless the displayed rounding conceals an already-saturated change. At zero gap, p1=.5 and derivative=.25. At gap 2, p1≈.880797 and derivative≈.104994. At gap 0 plus any shared offset, p1=.5. State these are derived scalar identities, not a sampled distribution of trained Q/K.
...
Phase two verifies the three-donor contrast/equality construction, the two-donor null/contrast, normalized inputs, correct entropy treatment 0 log 0, input-bound prediction reset and keyboard/mobile behavior. No claims about model explanations are scored beyond the specified mathematical question.
