...
**Change pixels, positions and feature relationships.** Edit patch pixels/projection coefficients, patch-position swaps, window/shift geometry, teacher/student logits and feature angles.
**See the consequence.** Update patch contributions, actual frozen-model logits/maps, spatial dependency sets, teacher targets/gradients and relational loss immediately.
**Decision connection.** Separate image edits from position changes, direct neighbors from multi-hop reach and target sharpening from learned quality.
...
The lesson must remain understandable without interacting. Every inline figure has a meaningful initial view, caption and equivalent text/table. Labs supplement the core explanation; they do not conceal a first definition or the only account of a mechanism. Keep worked examples ungraded and distinct from the fresh lab defaults and independent exercises. The retained JSON supplies reference results for the matching inputs; phase two must recompute arbitrary edits rather than display canned outcomes.

Each investigation displays the exact current input, weights/configuration and computed consequence immediately. Valid edits recompute all linked representations from that state; explain actual changes and nulls. An immutable baseline identifies the original source and parameters. No prediction collection, commitment, scoring or answer reveal remains.

Provide numeric controls and keyboard equivalents for every drag/paint operation. Apply on explicit button or bounded debounced inference; do not run inference on every pointermove. Step, Back and Reset restores the stated inputs and immediately displays their computed result. Buttons have accessible names and visible focus. No automatic animation; offer single steps and respect reduced motion. Plot tooltips also open on focus/tap. Invalid finite ranges, shapes, zero directions and temperatures receive local explanatory feedback.
...
Mobile uses a modest wide-enough plot with shorter tick labels and the selectable per-operation table. Phase two verifies formula sums, point ordering, linear transform, small 224 pair value visibility and no fabricated runtime interpretation. No interactive result checks is required; this is an explanatory calculated figure and values table.
...
Text includes dot products and Gram matrices; rotate by an explicit button, never continuously by default. Mobile matrix cells remain at readable size. The fresh computed Gram task is I5 below, with four vectors, distinct from this worked pair and Exercise 9's three vectors.
...
Purpose: inspect which information a projection keeps and loses. This is an image-patch arithmetic workspace, not a dropdown quiz over named rules.

Display which output coordinates change and their exact new values as a patch pixel or coefficient is edited. Keep signed product contributions next to the corresponding pixel and output cell. Values are computed from the actual edited patch.

Entity controls: select/edit any pixel within 0…16, edit each projection coefficient within −4…4 and biases −8…8, choose one proposed pixel operation or directly construct a replacement patch. Operation preview shows what is held fixed. The current view shows signed multiply/add contributions next to their source pixels. computed-result comparison uses a stated 1e−8 tolerance for entered numbers and exact coordinate-change classification beyond that tolerance. User-created variations are calculated from their actual input, not computed against a preset answer.

Checked null: replacing the baseline by `[[1,1],[4,1]]` changes two pixels but preserves both features. Do not present the null recipe in the initial computed screen. After feedback, ask the learner to find their own changed patch with equal output. Optional hint points to a preserved sum while displaying an example. This differs from Exercise 2 and the solved 0–15 image.

State: any valid edit recomputes the current result; backward trace restores the preceding dot-product stage for that input. Full reset restores the declared fixture and its outputs. Reject nonfinite/out-of-range values. Arithmetic remains tiny; main thread is sufficient. Mobile patch and editable coefficient rows stack with same numbered scalar labels. Text mode exposes the entire operation as editable numeric rows. Phase two verifies changed and null fixtures against `author-results.json.fresh_patch`, alternative coefficient edits, keyboard/pointer parity and recompute current outputs.
...
Show the annotated class-2 probability and its signed difference from the original as the learner edits the supported image. Keep the complete probability vector, original wrong prediction and current maps available; explain the changed model input rather than grading an expected direction.

**Entity controls:** edit 8×8 pixels on the original 0–16 grid while also displaying normalized values; select two of 16 actual patch locations to swap; choose whether their position vectors move too. Valid edits recompute patch vectors, attention, final features, logits and probabilities from the same frozen model. Never leave old output bars beside a new heatmap. Label-only changes do not enter the model.
...
Fresh path task: a 6×6 scalar grid, window side 2, first shift 0, second shift 1, source (0,0)=16, all other values 0, destination (2,2). The proposed change toggles the second layer from fixed windows to shifted windows. The live readout shows whether the destination changes and optionally requests its value. Display the two-layer coefficients and results immediately for the current input. Author-only checked values: fixed windows 0, shifted windows 1. The two-layer coefficient for a source at (5,5) is 0, yielding a nontrivial distant null.
...
Fresh bias task: one shifted-window layer on a 6×6 grid, M=2, shift=1, source (1,1)=10, destination (2,2), zero content scores and relative bias. Add log 2 to offset (+1,+1) and show destination 2.5→4 and source weight 1/4→2/5 immediately. An edit at (5,5) leaves this query unchanged. These forward and null cases are retained in `author-results.json.fresh_window_bias`; new edits recompute the same formula.
...
Explain the actual gradient sign using the teacher-target minus student-probability difference. Display the probabilities and signed derivative together as logits, center or temperature change. Independent written practice may retain a closed hint and solution; the lab has no answer field or grade.

### I5 — Preserve feature relationships (§6 deeper)

The fresh exploration task has four 2D unit features `[[1,0],[0,1],[−1,0],[0,−1]]`, associated with four constructed patch locations. The proposed edit moves the fourth feature to `[1,0]`. Inspect which Gram row/column changes and whether the squared Frobenius difference stays zero or increases. Original and changed Gram values, feature arrows and inputs remain visible together and update on valid edits.

Entity controls edit one feature's angle, select features by patch identity and apply a common rotation. If allowing coordinate edits instead, normalize explicitly and show normalized vectors; zero vectors are invalid cosine directions. The author-only changed loss is 6 under the unnormalized sum reduction; a common 90° rotation has loss 0. Both were checked in `author-results.json.fresh_gram`. Worked pair geometry and Exercise 9's three-vector fixture must not become the fresh exploration default.
...
Transfer asks the learner to construct another common orthogonal transform and compare it with changing one feature. Valid input changes recompute every dependent result and explanation; retained baselines keep their original inputs. Provide button/numeric angle alternatives to dragging; motion is optional and respects reduced motion. On mobile, put arrows above the matrix and the selected pair's dot product below it. The tiny calculation runs on Apply. Phase two checks norms, Gram symmetry, diagonal, loss normalization and the fresh contrast/null, together with keyboard operation and a copyable matrix.

## Disclosure, resources and implementation checklist

Make the current exact values available to screen readers and visual users at the same time. Accessible names identify controls; an adjacent live/text table reports the actual model state. Separate written practice may hide its solution, but the lab must not hide outputs to preserve an attempt.

Derive initial inline figures from compact topic data. Load I2's weights and model only when needed. Training programs and original data remain optional downloads. Show reference values with their exact matching input and provenance; do not confuse them with results of arbitrary edited inputs. Preserve source hashes, evidence and offline provenance. Required later work: implement the topic-specific SVG/HTML/canvas representations and models; independent content/model review; execute or correct the optional checkpoint program if it is to be presented as verified code; browser numerical parity; desktop/mobile layout; keyboard/focus/readouts; load failure and retry; exports, lazy entry and payload checks; normal publication/integration. None of those is claimed completed by this packet.

## Precise fixture and scope details retained for implementation

**Real-image edit fixture.** Test source 3 is actual digit 2. The starting edit is pixel (2,3), .8125→.1875 in normalized units. Show baseline and edited probabilities, logits, attention and maps using the retained frozen model; `author-results.json.fresh_image_change` supplies the parity reference. No new fit is needed. The first two specimens remain alternative worked inspections.

## Written implementation route and placement — 22 September 2026

Add a small mapping strip beside Swin: qkv→qkv, output→proj, head-by-offset bias→offset-by-head table. Label divisible-shape equality versus padded-policy comparison explicitly. The torchvision and DINOv2 checkpoint programs remain optional downloads; no precomputed parity/speed is invented before native verification. Preserve detailed window/EMA/patch visual forms.
