...
Change information arrival/cutoff, candidate eligibility, action capacity and costs. Keep event time separate from availability time; directly connect the available inputs to the chosen action set and its consequences.
...
Runtime ownership: `src/learn/components/lesson-labs/FormulationLabs.jsx` and its topic-specific helpers. Current migration evidence: [Classical ML live exploration review](../../LIVE-EXPLORATION-CLASSICAL-LATE.md).
