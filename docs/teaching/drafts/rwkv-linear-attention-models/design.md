...
- All four investigations have fresh visible inputs, input-bound a live computed readout, genuine sequence/key/coordinate edits, contrasting and null fixtures, explanations, reset/invalidation, mobile/accessibility/bounds and explicit phase-two work.
...
A future authorized finish request must use --work finish, consume this packet, implement topic-owned figures/labs with lazy loading and the stated state/live-output contracts, port only necessary frozen data, independently verify the new shipped model, execute any newly added displayed code and complete correctness/learning/browser/accessibility/performance/integration review. Reconcile any content corrections against these exact sources and keep implementation status separate.
...
**Topic disposition:** Read and write a compact memory. Edit keys/queries/values, decay, current-token bonus, memory write/correction inputs and real stream interruptions. Update summary matrices/denominators, present output versus stored history and chronological state together. Compare a continued stream with an explicit reset. Choose the correct current-read and future-memory semantics and see what compact state cannot retain.
...
| --- | --- | --- | --- | --- | --- |
| Positive-kernel causal state and chunks | linear_memory_mechanisms.py::positive_recurrent; lesson §§2–3 | trajectory_memory_models.py::MemoryMixer positive_kernel branch | Matrix/reference versus chunks 1/2/3/4/8; full/carry feature identity | Uneven chunk boundaries, zero overlap policy, weighted readout practice 1/2/7 | Local; dense matrix only an independent small oracle, O(m dv) recurrent state |
| RWKV-4 weighted read, stable state, token/channel shifts | linear_memory_mechanisms.py::rwkv_stable; lesson §§4–5 | MemoryMixer, MemoryBlock, TrajectoryMemoryClassifier and complete Adam fit | Same read/write timing, bonus outside state, complete shift-state continuation | Common +1000 key offset, reset versus carry, bonus edit and constant-value null | Local; vector recurrence differs from matrix-state later versions |
| Delta correction, row-gated memory, RWKV-7 update | linear_memory_mechanisms.py::additive_and_delta, gated_memory, rwkv7_step; lesson §6 | Torch autograd local-delta check in author_calculations.py; nn.Module composition route explained | Value-by-key orientation, diagonal retention versus rank-one removal/write | Nonunit/correlated key practice 5/6; unchanged other address versus interference | Local mathematical update; versions5/6 equations and transient bonus mapped; released full models not claimed identical |
| Released-model state continuation | Existing complete scratch MemoryBlock continuation, lesson §8 | rwkv_checkpoint_state.py using official rwkv forward and supplied local tokenizer/checkpoint | One call versus chunk/tokenwise final logits, independent branch copies, state-byte inventory | Change split point, compare reset, invalidate after checkpoint/tokenizer/prefix changes | Local optional package program written; checkpoint execution deferred, no timing/quality claims |
| Random features, sequence compression and accelerated kernels | Local explanation; actual prepared Sparse/Linear lesson §§4–5 owns Gaussian features and Linformer/Nyström | Kernel/backend construction belongs to planned GPU owner; model inference route above is supplied here | Do not call an ELU kernel exact softmax or fixed state complete model memory | Critical evidence and resource comparisons in practice 8–10 | Named-family bridge; full accelerator kernels are a later engineering outcome, not missing recurrence code |
