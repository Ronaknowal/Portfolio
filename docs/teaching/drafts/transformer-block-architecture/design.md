...
4. **Investigations:** real vector, matrix, point/time and output-probe edits are defined. Current results are visible on opening., bind inputs, invalidate on meaningful changes and have checked contrasts/nulls. There is no preset-only or stepper-only substitute.
...
9. **Screenshots:** none taken under content-only scope. Specifications name informative changed-output, edited-input, failure, null, desktop and narrow-screen states for later independent implementation review.
...
**Topic disposition:** Follow a complete block. Edit features, normalizer offset/scale, FFN matrices, branch multiplier, pre/post placement and supported retained trajectory inputs. Update normalization reference sets, feature writes, residual state, block output and derivative paths together. Show exact zero-branch and common-shift cases. Reason about placement and branch scaling using their actual function and gradient effects rather than an architecture slogan.
...
| --- | --- | --- | --- | --- | --- |
| Pre/post normalization block wiring | lesson §5 complete TransformerBlock; author-calculations.py::TransformerBlock | nn.TransformerEncoderLayer with explicit norms, activation, widths, dropout and batch_first | Attention/norm/FFN state mapping, output, input/parameter gradients and equal update for both placements | Matched epsilon change then single-side mismatch; zero-branch diagnostic | Local wiring; attention reused from prepared Self-Attention §5, normalization from implemented Normalization |
| GELU FFN, RMSNorm and SwiGLU branch | lesson §§4/8 complete tensor/module branches | nn.Linear/GELU and deliberate custom nn.Module for the branch | Row/feature orientation, gate multiplication and residual width | Change one feature detector/write direction; count matched-parameter widths | Local composition; primitives have actual earlier owners, no separate fashionable package required |
| Sequence model, readout and optimization | MovementClassifier plus full offline experiment in author-calculations.py; lesson §6 | PyTorch optimizer/loss/state_dict and standard module workflow | Same time-tag protocol and full block state; fit versus gradient probe distinction | Shuffle storage with tags, reverse coordinates, test informative gradient contrast | Local; retained fits conserved |
| Task architectures and distribution | lesson §8 decoder/encoder/cross-attention layout; §9 count model | Attention/cache operations reused; distributed execution owned by planned Tensor Parallelism | Mask/source roles differ from wiring; MACs, FLOPs and time distinct | Copy-task loss denominator and parallel branch practice | Architecture/resource reasoning locally; full distributed kernel not a claimed outcome |
