...
Change each input/pair factor to update the entire path ledger and trellis. Change prior and later compatibilities to compare private and shared denominators on identical inputs. Invalid drafts show an error and no newly computed diagram.
...
Runtime ownership: `src/learn/components/lesson-labs/CrfLabs.jsx` and its topic-specific helpers. Current migration evidence: [Classical ML live exploration review](../../LIVE-EXPLORATION-CLASSICAL-LATE.md).
