...
**Arrange readers, memory and availability.** Edit rectangular Q/K/V cells, shape assignments, input order/availability, compressor entries and gate parameters.
**See the consequence.** Show current read contributions, dependency legality, compression collisions and gradient routes. Saved image/question selectors reveal the actual retained outputs immediately.
**Decision connection.** Choose a cross-attention arrangement from what can be read, when it is available and what compression removes.
...
Valid entity edits recompute the current result and all linked views immediately. Keep a labeled original baseline when comparing an intervention. Reset restores the default inputs and their result; there is no predicted-answer field, multiple-choice assessment or success score.
...
Draw two rails, X and tanh (α)F, meeting at Y. Below, show loss residual, ∂L/∂α and ∂L/∂F for the same scalar squared-error fixture. Inputs X,F,target,α,rate bounded to−4..4 and rate 0..1. Learner inspects sign/zero status and next Y and take one update. Only α changes in this scalar experiment; label F fixed for this step. Default X1,F2,target 3,α0,rate.1 gives gate derivative−4, α′.4, Y′≈1.759898 and L′≈.768927. Use computed values rather than copying these rounded annotations.
...
Controls select only retained source/question/model/seed/head combinations. Display each selected paired record immediately, including its actual digit/parity output. Missing combinations explain that no record was retained. Do not provide an arbitrary pixel editor without weights; use the exact §1 editor for unrestricted small-entity changes.
...
Extend the cache view with source/preprocessing identity, parameter revision, projected K/V and new query rows. Reordering slots must reorder mask columns. Changing an unavailable donor demonstrates a null; replacing source/model identity marks stale cache before recomputation. The two images' known targets remain separate from learner interaction and are never answer-unlocked.
