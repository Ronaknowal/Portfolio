...
The preceding encoder–decoder packet repaired an initial literal-lemma-only split before freezing its final data. Its [split-integrity record](../sequence-to-sequence-encoder-decoder/split-integrity-repair.json) retains that superseded audit. This attention packet begins with the exact final CSV bytes and runs six new fits (additive/general, seeds1–3) on that final split only. The earlier three final fixed-context results and rule baseline are copied into `mechanics-results.json` with their source JSON hash; the baseline was not fitted again. No superseded model result belongs in this manuscript.

The training set estimates parameters. The development set was already inspected for checkpoints, error slices and greedy/beam comparisons in the preceding lesson; it is inspected here for the six attention runs and their traces. It remains development, not a new independent final test. This packet's measured predictions use greedy decoding; no attention beam-score advantage is claimed.
...
`prepare-inflection-data.py` is an exact copy of the preceding packet's executed extraction program and recreates the final extraction from the pinned public file, including hashes/group audit. It was not downloaded/run again for this packet: the already audited CSV and extraction report were copied unchanged and the final hash and split invariants were rechecked. The learner training program reads the supplied CSV offline. Source/provenance links, license and adaptation notes must accompany a published copy.
