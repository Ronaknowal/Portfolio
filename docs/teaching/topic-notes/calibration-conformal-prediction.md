...
Closed. All four proposals are implemented and independently reviewed. Each was checked line by line against the frozen manuscript rather than accepted from the 12 September disposition: the conditioning, plot-direction and finite-bin treatment with its exact counterexample; the decision-information material with both population groups and both calibrated forecasts; the score-interface distinction with the four-role example; and the current-API migration explaining frozen estimators, fold-specific ensembles, transformations inside folds and selection boundaries.

**One sub-point is closed in production rather than in prepared content, and is recorded as such.** This note asked that class absence alter interpretation; the manuscript does not treat class absence at all. The implementation supplies it — a no-value sigmoid answer, an AUC no-value branch and an all-positive preset — recorded as a declared departure. A reader comparing manuscript to page should expect that difference and find it explained there.

The independent review reproduced every constructed number in exact rational arithmetic, decided **260,040 conformal rank cases in BigInt**, and refit both pipelines from the served data with no disagreement. Two findings are worth recording. The shipped integer rank is correct at every grid point, where `Math.ceil((n+1)(1−α))` is wrong at 26 — of which one is reachable through the lesson's own controls. And the frozen packet is wrong at its last digits: an independent 60-digit re-solve of the Platt fixture gives the minimiser **0.267701773035583449…**, against which the packet's SciPy value carries a 5.2×10⁻¹¹ error. The packet and its `checked-results.json` are deliberately left untouched, because that file faithfully records what the packet's own program produces and the examples verifier reproduces it byte for byte; the disagreement is recorded in the design record, and the page displays nine decimals, where the two agree.

Implementation, independent review ([CALIBRATION-INDEPENDENT-REVIEW.md](../CALIBRATION-INDEPENDENT-REVIEW.md)) and integration are complete, with source hashes and evidence bound in the delivery ledger. This note is closed.
