...
The initial migration records **107 previously completed topic revisions** with both phases complete. This is a migration of existing review records, not a fresh review or retrospective claim of checks never performed. `legacyReview: true` delegates source currency to the existing inventory review logic. At migration, the proposed K-Means revision was explicitly pending and its changed source was reported as stale. The [21 September independent review](K-MEANS-REVISION-REVIEW.md) subsequently closed that proposal as normal revision 2, retaining the historical row. New bytes must never be marked reviewed merely to make counts uniform.
