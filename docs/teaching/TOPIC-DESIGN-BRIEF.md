...
Linux example: “Why can I read a known filename but not list the directory?” needs directory-name listing versus path traversal, then file read permission. A sequence of permission checks makes the first failed check visible. Toggle directory read while keeping search/file read enabled and observe that file access can still succeed; then change another permission and diagnose its effect. The current path and permission results stay visible. An octal table alone does not expose the mechanism.
...
For an interactive investigation, additionally record the visible initial result, meaningful controls, held-fixed quantities, live consequences, step/reset/back behavior, invalid states, causal explanation, decision or tradeoff, transfer task and simulation boundaries. Specify input → mechanism → intermediate state → output → decision. State when edits take effect and how connected views stay synchronized. Use sliders/direct manipulation with numeric and keyboard equivalents where useful; preserve exact values beside intuitive geometry. Specify baseline comparison when it helps. Do not include prediction entry, commitment, grading or a prediction reveal, even optionally. Static figures do not need invented controls or simulator code to satisfy the design brief.

Record three things the standard checks at review for every investigation: how the **current result and its cause are visible immediately** and update on valid edits; which control lets the learner **act on the topic's entities** beyond choosing among described presets; and the **fixture check**, listing alternatives actually run to confirm the promised contrast and the null case in which it correctly shows none. Specify continuous updates, quick repeated edits and reset behavior. Process stepping or an explicit bounded calculation may take time, but never waits for a learner answer; identify the active input state and any pending calculation. If the natural running example cannot show a promised contrast, specify a second fixture with exactly representable critical values. For quantitative figures, record how the feature is visible at desktop and phone size, including the axis transform and baseline.
...
Apply the standard's [implementation-depth contract](../../LESSON-TEACHING-STANDARD.md#build-the-mechanism-then-control-the-library). For each core computational outcome fill this ownership map; avoid treating a lab's hidden JavaScript or a library fit as the learner's scratch implementation.

| Mechanism / learner outcome | Scratch implementation or exact prerequisite owner | Idiomatic library/tool route | Bridge and verification | Research/engineering control and independent practice |
| --- | --- | --- | --- | --- |
| State the promised operation and abstraction boundary | Runnable source + lesson section; identify reused primitives and intentional exclusions | Actual API/configuration and supplied runnable example; reason if no separate API is appropriate | Same inputs/state, shape/convention mapping, forward/gradient/update/state comparisons or justified difference | What the learner changes, diagnoses or builds, with success criteria |

Classify every outcome as local, reused, later-owner overview or noncomputational with a reason. Verify prerequisite coverage rather than relying on titles, and distinguish an already published owner from a complete but unimplemented prerequisite packet. Complete this map while planning and fill the actual manuscript/code while writing. Before declaring content complete, resolve every missing promised mechanism or ordinary-tool explanation; a finish-agent TODO is not a completed content outcome. The map locates complete teaching rather than standing in for it. Record source-located evidence and actual execution status in the design. Full native/independent/browser verification can remain deferred; material teaching and API-contract decisions cannot. Do not retroactively label historical checkpoints compliant.

For the final scratch route, record the algorithm choice, time/space costs, stable numerical/state conventions, important tested edges and reasons for any performance tradeoff. Distinguish a hand-traced naive baseline from the recommended efficient implementation. Back a performance claim with measurements; justify an optimality claim under an explicit computational model. Do not optimize away explanations or silently delegate the very operation the learner is meant to implement.
...
Run the standard's [learning-experience checklist](../../LESSON-TEACHING-STANDARD.md#learning-experience-checklist) as a distinct pass from correctness verification, and record its findings separately. Two items are commonly skipped and must not be: a hedging pass over the whole manuscript, and screenshots of informative states (the contrasting edited inputs, boundary/null results, each full-width figure at desktop width) rather than defaults only. Check that no old prediction controls or masking remain in the live exploration. When a reviewer's finding concerns a pathological input, resolve it in prose or in the verifier before touching displayed teaching code.
