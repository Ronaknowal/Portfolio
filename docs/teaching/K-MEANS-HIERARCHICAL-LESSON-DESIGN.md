...
## Proposed revision, 12 September 2026 — historical proposal, reviewed 21 September

The following records the proposal's state on 12 September. Its then-pending independent review is now complete; see the closure below. Present-tense statements in this historical section describe that earlier checkpoint.
...
On 21 September the user explicitly authorized reviewing this pending revision. The [current review](K-MEANS-REVISION-REVIEW.md) owns the independent assessment, confirmed repairs and refreshed source-bound checkpoint. Earlier suggestions to commit or discard workspace changes are superseded: neither is authorized by this review request, and unrelated work must be preserved.

## Revision review closed, 21 September 2026

The [completed review](K-MEANS-REVISION-REVIEW.md) supersedes the proposal's pending state. Independent content and numerical reviewers assessed the actual revision; confirmed fixes address forecasting leakage, mathematical qualifications, prediction/reveal contracts, probability versus simulation, palette limits and narrow-screen annotations. The final build and 15 production browser groups pass, alongside 159 grouped model cases and 27 native/data checks. The review links the exact evidence and its limits.

The full-mode manuscript is the implemented lesson; the [retained visual/teaching contract](K-MEANS-REVISION-VISUAL-CONTRACT.md) describes the current five investigations and seven figures. The central phase ledger records revision 2 with both phases complete and the previous row retained in history. This original design and the earlier eight-source ledger remain interpretable historical records. User acceptance is separate; this completion does not authorize a commit, deployment or next topic.
