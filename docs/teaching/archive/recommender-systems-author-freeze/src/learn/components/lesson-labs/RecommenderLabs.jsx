...
export function ImplicitConfidenceLab() {
  ...
  return <Investigation title="See the all-pair confidence bowl" prompt="Remove the missing-item term. Does the target vector change, or does the penalty for one score disappear?" reset={() => {
    ...
    <div className="rec-two"><div><Matrix caption="Fixed item factors; counts [2, 0, 1]" labels={['q₁', 'q₂', 'z', 'weight']} rows={result.items.map((item, index) => [...item, result.targets[index], result.confidence[index]])} /><Matrix caption="Normal system A p = b" labels={['A column 1', 'A column 2', 'b']} rows={result.normal.map((row, index) => [...row, result.right[index]])} /></div><svg className="rec-plot" viewBox="0 0 280 280" role="img" aria-label="Exact quadratic contours at minimum plus .1, .5 and 1; axes are user factors"><text x="47" y="17">Objective above its minimum</text><line x1="47" y1="230" x2="257" y2="230" className="rec-axis" /><line x1="47" y1="40" x2="47" y2="230" className="rec-axis" />{[0, .5, 1].map(fraction => <g key={fraction}><text x={47 + 210 * fraction} y="247" textAnchor="middle">{number(limits[0][0] + fraction * (limits[0][1] - limits[0][0]), 2)}</text><text x="40" y={234 - 190 * fraction} textAnchor="end">{number(limits[1][0] + fraction * (limits[1][1] - limits[1][0]), 2)}</text></g>)}{result.contours.map((contour, index) => <polyline key={contour.excess} points={contour.points.map(point => `${x(point[0])},${y(point[1])}`).join(' ')} stroke={[gold, blue, mint][index]} strokeWidth="2" fill="none" />)}<circle cx={x(result.user[0])} cy={y(result.user[1])} r="4" fill="white" /><text x="150" y="269" textAnchor="middle">user factor p₁</text><text x="47" y="33">p₂</text></svg></div>
