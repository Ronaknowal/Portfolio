export default {
  ...
  groups: [{
    ...
    title: 'Deeper state families · identify what crosses the boundary',
    introduction: 'Use the complete interval, tree and digit branches before attempting these tasks. These three public official statements were separately inspected on 11 September 2026; the retained earlier selections keep their original inspection date.',
    problems: [{
      number: 312,
      title: 'Burst Balloons',
      slug: 'burst-balloons',
      difficulty: 'Hard',
      prerequisite: 'The interval last-operation and last-removal derivations, surviving sentinel boundaries and postorder reconstruction in this lesson.',
      focus: 'Choose the final interior removal so the two subproblems have fixed surviving neighbors. Distinguish a split-tree root from the first chronological removal.',
      hint: 'While the last balloon remains, what protects one child interval from removals in the other? Return the table optimum for the judge; retain an original-ID witness while testing it locally.',
      transfer: 'Add zero-valued balloons and replay the recovered order against current neighbors. Then allow a negative singleton but still require its removal: explain why a zero-initialized candidate maximum would invent an impossible result.'
    }, {
      number: 337,
      title: 'House Robber III',
      slug: 'house-robber-iii',
      difficulty: 'Medium',
      prerequisite: 'Tree parent/child structure and postorder, followed by the explicit parent-selected state in this lesson. The platform supplies binary-tree node objects rather than an indexed edge list.',
      focus: 'Return the weighted independent-set optimum while preserving the parent boundary condition. Translate node identity and missing children rather than assuming equal values identify equal nodes.',
      hint: 'When taking a node, ask each child for its forced-off answer. When skipping it, each child is free. An iterative child-before-parent order handles a long chain without depending on Python recursion depth.',
      transfer: 'Recover an actual selected set, allow negative weights with an empty choice, then add one cross-child conflict. Explain why the last change destroys the one-parent-boundary proof.'
    }, {
      number: 2376,
      title: 'Count Special Integers',
      slug: 'count-special-integers',
      difficulty: 'Hard',
      prerequisite: 'Decimal prefix comparison, tight/started/used-digit state and exact count semantics from the digit branch; the earlier membership-mask primer supplies the encoding.',
      focus: 'Count positive integers with distinct ordinary decimal digits under an inclusive bound, separating padding zeros from actual zeros.',
      hint: 'A smaller prefix makes every remaining position loose. A zero before starting consumes no digit bit, while a real zero does. The all-padding leaf contributes zero to this positive-only task.',
      transfer: 'Count an inclusive range by subtracting two prefix counts. Then forbid only equal adjacent digits and decide whether zero is included: derive the smaller previous-digit state and the changed endpoint base.'
    }]
  }],
