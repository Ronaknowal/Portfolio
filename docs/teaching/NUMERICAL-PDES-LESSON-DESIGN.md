...
Prepared 11 September 2026 and assessed before implementation. The complete lesson now has [actual author verification](NUMERICAL-PDES-VERIFICATION.md) and an [exact-source author packet](evidence/numerical-pdes-author-review.json). The original design rationale below remains historical; its [exact pre-implementation copy](evidence/numerical-pdes-assessed-design.md), [original planned entry and complete inventory](evidence/numerical-pdes-original-plan.json) are preserved. No old published body or complete program existed. Independent source review, integrated production checks and user acceptance remain separate from the author freeze.
...
Incoming notes were planned as follows; their implemented dispositions are recorded at the end and in the destination note:
...
The authored scope follows all 15 stages with 16 complete executed programs, 15 changed tasks, 11 investigations and one inline figure. Counts report the chosen coverage, not a quota. The first-pass route separates the steady rod, time stepping, and later conservation/finite-element branches. The fixed-size Godunov face and two-grid operations are complete optional extensions; the core does not depend on opening them.

The final solver is a positive-pivot tridiagonal LDLᵀ implementation for the stated SPD family. Its exact residual certificate uses fractions of the stored binary64 vector and conservative conversions, separating actual arithmetic, nodal discretization and reconstructed-field error. The sparse two-dimensional native solve uses the verified SciPy API; no browser Python dependency was introduced. The Robin hand fixture was corrected to u=20/3−x² before body registration. The optional second forced-rod capstone was not duplicated: the changed quartic report instead adds nonzero boundary values, changed length/source, exact residual allowances and explicit finite-budget failure/success.

Incoming PDE56, Real54 and Conditioning52 notes are now adapted in their [destination record](topic-notes/numerical-pdes-grids-finite-elements-stability.md). The author inspected the actual PDE56 balance and weak-form source, retaining outward flux signs, compatibility before a mean constraint, point-load test-function weights and the Green-function kink. The separate reconstruction and normalization notes are combined through the locally proved maximum principle/barrier bound, the interpolation term, and later integrated L2/gradient quantities.

The final representation choices include actual stencil contributions, a three-part certificate, an exactness/refinement counterexample, computed same-time mode evolution, a material resistance/flux profile, a boundary balance, transported cell averages, a scalar entropy face, hat assembly and source alignment, a coordinate/index map, equal-scale triangle geometry and energy projection. There is no fabricated benchmark. Triangle coordinates actually change its aspect ratio; graphs retain overshoot scales and distinguish drawn samples from exact analytic statements.

The receiving author completed the previously pending source reads:

| Source | Actual additional inspection and resulting boundary |
| --- | --- |
| [FNC upwind discussion](https://fncbook.com/upwind/) | Domain of dependence, CFL necessity versus sufficiency, velocity direction and inflow. The body supplies its own periodic convex-weight and conservation proofs. |
| [Official Clawpack Burgers notebook](https://github.com/clawpack/riemann_book/blob/master/Burgers.ipynb) | Accessible official raw notebook sections on shock speed, rarefaction and entropy selection were read; the learner-facing GitHub notebook was also opened. The inaccessible older HTML was not treated as a read source. |
| [Schöberl's finite-element error analysis](https://jschoeberl.github.io/iFEM/FEM/erroranalysis.html) | Affine derivative transformation, shape regularity, H2 interpolation, coercive approximation and the regularity-dependent L2 argument. The lesson proves only its stated 1D estimate locally; the page is annotated as an advanced extension. |
| [FNC finite-difference convergence](https://fncbook.com/python/fd-converge/) and [diffusion stability](https://fncbook.com/python/absstab-diffusion/) | Definitions/Taylor/order and scalar factors, periodic zero mode and spectrum inspected. The lesson's Dirichlet eigenvalues and finite-grid-versus-positivity distinction are derived for its actual matrices. |
| [FNC Laplace/Poisson](https://fncbook.com/laplace/) | Equation sign and Sylvester/Kronecker construction revisited; the negative-Laplacian convention and row mapping remain explicit here. |
| [SciPy spsolve](https://docs.scipy.org/doc/scipy/reference/generated/scipy.sparse.linalg.spsolve.html) | Square CSR/CSC systems, vector shapes, ordering and output qualifications checked against the official 1.18 page; programs executed under SciPy 1.18.1. |
| [MIT Lecture 17 official video/transcript page](https://ocw.mit.edu/courses/18-085-computational-science-and-engineering-i-fall-2008/resources/lecture-17-finite-elements-in-1d-part-1/) | Official page, instructor and available video/transcript verified. The parent's specific Lecture 17/18 transcript passages above remain the actual inspected scope. No full-video playback claim. |

Final source-matched native checks completed at 09:26:20 UTC; the actual-font 1440/390/320 browser run completed at 09:23:11 UTC. Thirty final ordinary viewport images were opened, including every investigation's distinct mechanism, lower practice/output and narrow equations. The [verification record](NUMERICAL-PDES-VERIFICATION.md) and exact packet own the reproducible evidence and limits.
