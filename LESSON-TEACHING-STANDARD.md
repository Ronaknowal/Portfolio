...
Updated 21 September 2026: the user's live-exploration requirement replaces the former prediction-and-reveal lab contract. Labs show current results immediately and let learners manipulate meaningful inputs; do not include prediction entry, prediction grading or prediction gates, even as optional features. The 12 September learning-experience, accuracy, delivery-phase and review requirements remain. Coverage, titles and useful applications must be reconsidered during authoring as well as before it. This is the authoritative teaching policy. [The handoff](LESSON-AUTHORING-HANDOFF.md) owns current scope and status; historical reports do not override this standard.
...
The manuscript contains the real explanations and practice solutions at the required depth, not instructions telling the next agent to write them. Specifications identify the exact placement, entities, data/formulas and provenance, labels/units, states/transitions, live controls and causal feedback, text equivalent, mobile composition, edge cases and what must be verified. Supply enough detail to build each representation without inventing its teaching purpose; allow the implementer to improve it with a recorded reason. A specified graph is not a verified rendered graph.
...
Follow [the engineering layout contract](docs/engineering/LEARNING-CODE-STANDARD.md#diagram-layout-and-svg-legibility). A repeatable triage tool supports this pass but cannot certify all interactive states, typography or teaching quality. Fix confirmed findings and repeat affected checks; reuse unrelated passing evidence.
...
### Build the mechanism, then control the library
...
The teaching route, distributed naturally through the page, includes:

1. **Construct:** start with the question, contract, small inputs, shapes/state and invariant or equations. Derive the essential operations, trace one case and supply a complete runnable implementation with setup and outputs. Include training/update/state transitions when these are part of the outcome, not inference alone. Keep explanation next to the decisions; do not append an unexplained code dump.
2. **Use the usual tool:** show the idiomatic library/tool route on the same task, then its practical use where useful. Cover the settings that change semantics: parameter orientation, reductions, defaults, train/eval state, randomness, precision, stopping rules, persistence and error handling as relevant. No requirement to use a fashionable package when the standard library or a small custom function is the normal practice.
3. **Bridge:** map the learner's variables and steps to the API's inputs, parameters, state and outputs. Match inputs and initial state before comparing. Check forward values and, when in scope, gradients, one update or state transition; explain tolerances, ties and intentional differences. Unmatched random fits or similar final accuracy do not demonstrate implementation equivalence. Identical outputs alone do not prove algorithmic identity.
4. **Take control:** teach a meaningful modification, diagnostic or extension: a custom objective, changed invariant, alternate reduction, new stopping rule, corrected axis, state restoration or measured optimization. Explain which part should be customized and which is best delegated to a maintained implementation. Include an independent changed-input implementation/debugging exercise with hints, reasoned solution and success criteria. Lab outputs remain immediately visible; never reintroduce learner-prediction gates.
5. **State the limit:** distinguish an inspectable reference implementation from library robustness, distributed behavior, hardware performance or scientific validity. Teach the important limitations concretely without surrounding the algorithm with production boilerplate. Do not present an API wrapper as a from-scratch implementation of its hidden mechanism.

Reuse rather than recopy: link the exact earlier section and runnable program, refresh its contract briefly, then implement only the new mechanism or compose it into the current workflow. Repeat/adapt when notation, representation or new assumptions make it genuinely clearer, and explain why. Keep one semantic source owner for reusable teaching code; never make the browser eagerly import prerequisite lessons just to support a link.

**High-quality implementations, not disposable toy code.** The owned implementation must use an appropriate efficient algorithm, stable numerical formulas, correct state/lifetime handling and idiomatic readable structure. State input contracts, time and space costs, important edge cases and extension points. Verify meaningful boundary cases, invariants and an independent reference. A naive loop may introduce the idea, but when it is materially inefficient, continue to the optimized algorithm/representation or explicitly reuse its taught owner; do not leave the naive version as the promised final implementation. Vectorize or batch where it helps without hiding the mechanism or allocating an unjustifiably large intermediate. Discuss sparse/streaming/chunked alternatives when scale makes them relevant. Separate algorithmic optimality under a stated cost model from measured hardware performance: “optimal” is not a universal claim about every shape, dtype, device or memory budget. If claiming a speed or memory advantage, measure it with disclosed conditions. Do not promise parity with all fused/distributed library kernels merely because the reference is correct. Instructional clarity is compatible with carefully engineered code; a simplified scope is acceptable only when explicitly stated and does not omit the core outcome.

**This is a content-planning and writing requirement.** Before completing the content phase, account for every advertised core computational outcome in the design and supply its actual explained scratch source, ordinary tool route and meaningful modification exercise, or inspect and link the exact reusable owner. The manuscript must teach the correspondence between variables, shapes, state and API settings. A saved instruction to "add a scratch implementation/library bridge during finishing" is an unresolved content gap, even if an earlier version was marked content-complete. Do not replace missing teaching with an ownership table, a documentation link, a browser-model source file or a promise to research it later. A named later owner is appropriate for a clearly identified overview, not a silent deferral of this lesson's promised outcome.

For content-first work, keep complete instructional source with its topic's manuscript/packet and specify the comparison, output interpretation and extension plan; be honest about executed versus analytically derived or expected results. Research API contracts and settle material algorithm/version choices while writing. Small reasoning checks and code probes can establish the claims being written without starting the full implementation campaign. Defer rendering, packaging, full native/model verification, independent implementation review and browser integration to phase two; do not defer the design or writing of an already-promised mechanism. During finishing, consume and improve that prepared source as needed, execute the appropriate native examples, compare by an independent route, check that displayed/downloaded sources agree, and confirm the learner can find both paths. New discoveries may still require corrections, but discovering/writing a known missing core is not the planned implementation workflow. Record confirmed gaps separately from unchecked execution. Keyword counts and program counts are navigation aids, not a mastery audit. The [implementation-depth review](docs/teaching/IMPLEMENTATION-DEPTH-REVIEW.md) records the bounded assessment and current handoffs.
...
For an included application, establish the problem and why it matters, map the topic's entities/operations to that setting, walk through a small example or causal chain, interpret the result, and explain the relevant assumptions or limits. Add a changed condition, diagram, live exploration or separate exercise when it helps the learner test that connection. A sentence listing industries or a link to an impressive project is not adequate treatment. For a surprising fact, explain why it is true and the misconception or consequence it clarifies; a full extra lab is not required.
