...
(async () => {
  ...
  try {
    for (const width of [1440, 390]) {
      ...
      assert.equal(await examples.count(), Object.keys(dynamicProgrammingExamples).length);
      for (const example of Object.values(dynamicProgrammingExamples)) {
      ...
      assert.equal(await practice.locator('a[href^="https://leetcode.com/problems/"]').count(), 12);
      ...
      results.push({ width, rewardStates, gridStates, sequenceStates, capacityStates, subsetStates, examples: Object.keys(dynamicProgrammingExamples).length, practice: 12, anchors: anchors.length, overflow: false });
      await page.close();
    }
    assert.deepEqual(errors, []);
    const result = { checkedAt: new Date().toISOString(), base, results, errors };
