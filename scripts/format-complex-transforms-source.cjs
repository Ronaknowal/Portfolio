...
const records = [];
for (const file of ['src/learn/data/topics/complex-numbers-fourier-laplace-transforms.jsx', 'src/learn/components/lesson-labs/ComplexTransformsLabs.jsx', 'src/learn/data/complex-transforms-models.js', 'src/learn/data/curriculum/blueprints/complex-numbers-fourier-laplace-transforms.js']) {
...
const cssFile = 'src/learn/components/lesson-labs/complex-transforms-labs.css';
...
fs.writeFileSync('scratch/complex-transforms-verification/format-conservation.json', JSON.stringify({ checkedAt: new Date().toISOString(), records }, null, 2) + '\n');
