...
const directory = 'scratch/analysis-decisions-integration';
...
for (const [name, script, ...args] of [
  ...
  try {
    const result = spawnSync(process.execPath, [script, ...args], { encoding: 'utf8', maxBuffer: 24 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] });
