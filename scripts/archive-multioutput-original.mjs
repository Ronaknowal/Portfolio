...
const source = 'src/learn/data/topics/multi-label-multi-output-learning.jsx';
const directory = 'docs/teaching/archive/multioutput-original';
const destination = 'docs/teaching/evidence/multioutput-original.json';
...
fs.writeFileSync(destination, JSON.stringify({ capturedAt: new Date().toISOString(), source, archive, sourceSha256: crypto.createHash('sha256').update(bytes).digest('hex'), programs, sharedBaseline: 'docs/teaching/evidence/classical-ml-supervised-baseline.json', priorTopicOwnedSupportFiles: [], note: 'Preserve every legacy code block and comment exactly. Several are dependent fragments; the last downloads a single-label text dataset and adds synthetic random labels. Execution evidence will distinguish actual stdout from unsupported old comments.' }, null, 2) + '\n');
