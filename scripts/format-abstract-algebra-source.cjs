...
const records = [];
for (const file of ['src/learn/data/topics/abstract-algebra-groups-symmetry-actions.jsx', 'src/learn/components/lesson-labs/AbstractAlgebraLabs.jsx', 'src/learn/data/abstract-algebra-models.js', 'src/learn/data/curriculum/blueprints/abstract-algebra-groups-symmetry-actions.js']) {
...
const cssFile = 'src/learn/components/lesson-labs/abstract-algebra-labs.css';
...
fs.writeFileSync('scratch/abstract-algebra-verification/format-conservation.json', JSON.stringify({ checkedAt: new Date().toISOString(), records }, null, 2) + '\n');
