...
const records = [];
for (const file of ['src/learn/components/lesson-labs/LinkedTraversalLabs.jsx', 'src/learn/components/lesson-labs/MonotonicStackLabs.jsx', 'src/learn/data/linked-traversal-models.js', 'src/learn/data/monotonic-stack-models.js']) {
...
for (const cssFile of ['src/learn/components/lesson-labs/linked-traversal-labs.css', 'src/learn/components/lesson-labs/monotonic-stack-labs.css']) {
...
fs.writeFileSync('scratch/linked-traversal-extension-verification/format-conservation.json', JSON.stringify({ checkedAt: new Date().toISOString(), records }, null, 2) + '\n');
console.log('Six owned source files formatted; semantic AST and JSX transforms conserved.');
