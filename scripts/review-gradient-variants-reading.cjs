...
const directory = path.resolve(__dirname, '../scratch/gradient-variants-browser');
...
(async () => {
  ...
  for (const width of [1440, 390, 320]) {
    ...
    await page.goto('http://127.0.0.1:5173/learn/path/full-curriculum/gradient-descent-variants-sgd-adam-adagrad-rmsprop-lamb-lars');
    await page.locator('.optimizer-lesson').waitFor();
    const lesson = page.locator('.optimizer-lesson');
    await lesson.locator('.optimizer-lab').first().waitFor();
    ...
    for (let index = 0; index < 2; index++) {
      await lesson.locator('.optimizer-inline').nth(index).screenshot({ path: path.join(directory, `inline-${index + 1}-${width}.png`) });
    }
    ...
    const scrollableTables = lesson.locator('.optimizer-table');
    ...
    const plotTextOutside = await lesson.locator('svg.optimizer-plot').evaluateAll(nodes => nodes.flatMap((svg, index) => {
