...
const ids = read('docs/teaching/evidence/deep-learning-convolution-baseline.json').ids;
const next = [...ids.slice(1), 'depthwise-separable-dilated-convolutions'];
const chunks = Object.entries(lessons).map(([id, path]) => ({id, file:manifest[`src/learn/data/${path.slice(2)}`].file}));
const output = 'docs/teaching/evidence/deep-learning-cnn-production-integration.json';
...
(async()=>{
 ...
 try {
  for (const [i,id] of ids.entries()) {
   ...
   assert.ok(anchors.length>=10 && anchors.every(row=>row.count===1),JSON.stringify(anchors));
  ...
  for(const kind of ['import','render']) {
   ...
   await page.route(`**/${file}*`,async route=>{
    ...
    if(requests===1) {
     ...
     else await route.fulfill({status:200,contentType:'text/javascript',body:'export default { content() { throw new Error("Scoped CNN render failure"); } };'});
   ...
   if(kind==='import') {
    ...
    await page.waitForFunction(()=>document.querySelector('.initialization-lesson')||document.querySelector('.lesson-load-error'));
   }
   if(await error.count()) await error.getByRole('button',{name:'Reload page',exact:true}).click();
   await page.locator('.initialization-lesson').waitFor();
