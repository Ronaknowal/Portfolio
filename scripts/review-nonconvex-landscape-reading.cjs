  ...
  try {
    for (const width of [1440, 390, 320]) {
      ...
      for (let index = 0; index < 3; index++) {
        await lesson.locator('.landscape-inline').nth(index).screenshot({ path: path.join(folder, `inline-${index + 1}-${width}.png`) });
      }
      ...
      for (let labIndex = 0; labIndex < 4; labIndex++) {
        const controls = lesson.locator('.landscape-lab').nth(labIndex).locator('button:enabled,input,select');
      ...
      const slider = lesson.getByRole('slider', { name: 'Direction angle in degrees', exact: true });
      await slider.focus();
      await page.keyboard.press('Home');
      await page.keyboard.press('ArrowRight');
      assert.equal(await slider.inputValue(), '15');
      const select = lesson.locator('[data-lab="stationary"] select');
      await select.focus();
      await page.keyboard.press('End');
      await page.keyboard.press('Enter');
      assert.equal(await select.inputValue(), 'valley');
      await lesson.locator('.landscape-practice summary').last().focus();
      ...
      const clippedText = await lesson.locator('svg.landscape-plot').evaluateAll(nodes => nodes.flatMap((svg, index) => {
      ...
      results.push({ width, ordinarySections: 9, inlineFigures: 3, keyboardControls: visits.length, visits, keyboardScrolledTables, mathCount: math.length, overflowingMath, clippedText, pageOverflow });
