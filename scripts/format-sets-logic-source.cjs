...
const records = [];
for (const file of ['src/learn/data/topics/sets-logic-relations-proof-techniques.jsx', 'src/learn/components/lesson-labs/SetsLogicLabs.jsx', 'src/learn/data/sets-logic-models.js', 'src/learn/data/curriculum/blueprints/sets-logic-relations-proof-techniques.js']) {
...
const file = 'src/learn/components/lesson-labs/sets-logic-labs.css';
...
fs.writeFileSync('scratch/sets-logic-verification/format-conservation.json', JSON.stringify({ checkedAt: new Date().toISOString(), records }, null, 2));
