...
function boundedVector(vector, size) {
  ...
  for (let index = 0; index < size; index += 1) {
    if (!Object.hasOwn(vector, index) || !Number.isFinite(vector[index]) || Math.abs(vector[index]) > 10000) {
      throw new RangeError('Provide the expected number of finite coordinates in [-10000,10000].');
    }
  }
}

function validateCandidates(candidates, allowEmpty = false) {
  if (!Array.isArray(candidates) || candidates.length > neighborRows.length || (!allowEmpty && !candidates.length)) {
    throw new RangeError('Use a dense subset of the declared neighbor fixture.');
  }
  const seen = new Set();
  for (let index = 0; index < candidates.length; index += 1) {
    const row = candidates[index];
    if (!Object.hasOwn(candidates, index) || !neighborRows.includes(row) || seen.has(row)) {
      throw new RangeError('Use a dense subset of distinct rows from the declared neighbor fixture.');
    }
    seen.add(row);
  }
}
...
export function metricDistance(first, second, metric = 'euclidean') {
  ...
  if (metric === 'cosine') {
    ...
    if (!firstNorm || !secondNorm) throw new RangeError('Cosine comparison requires two nonzero vectors.');
    const similarity = (first[0] / firstNorm) * (second[0] / secondNorm)
      + (first[1] / firstNorm) * (second[1] / secondNorm);
...
export function neighborReport({ query = [3.1, 2.9], k = 5, metric = 'euclidean', weights = 'uniform', candidates = neighborRows } = {}) {
  ...
  if (!Number.isInteger(k) || k < 1 || k > candidates.length || !['uniform', 'distance'].includes(weights)) {
