...
export function parseProcessNumber(text, minimum, maximum, name) {
  const literal = String(text).trim();
  if (!/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:e[+-]?\d+)?$/i.test(literal)) {
    throw new RangeError(name + ' needs a decimal number.');
  }
  const value = Number(literal);
  if (value === 0 && /[1-9]/.test(literal.split(/e/i)[0])) {
    throw new RangeError(name + ' is below the supported numeric range.');
  }
  bounded(value, minimum, maximum, name);
  return value;
}
export const MARKOV_PRESETS = freeze({
  ...
  sticky: {
    name: 'Same equilibrium, slower',
    a: 0.02,
    b: 0.03
  },
  independent: {
    name: 'Fresh draw each step',
    a: 0.4,
    b: 0.6
  },
  alternating: {
    name: 'Always alternate',
    a: 1,
    b: 1
  },
  identity: {
    name: 'Never change state',
    a: 0,
    b: 0
  },
  absorbing: {
    name: 'Sunny is absorbing',
    a: 0,
    b: 0.3
  }
});
export function finiteProcessLaw(kind, length = 4) {
  ...
  for (let mask = 0; mask < 2 ** length; mask += 1) {
    const values = Array.from({
      length
    }, (_, index) => mask >> length - index - 1 & 1);
    const allowed = kind === 'fresh' || values.every((value, index) => kind === 'frozen' ? value === values[0] : value === (values[0] + index) % 2);
    if (allowed) paths.push({
      values,
      probability: kind === 'fresh' ? 2 ** -length : 0.5
    });
  }
  const marginalOne = Array.from({
    length
  }, (_, time) => paths.reduce((sum, path) => sum + path.probability * path.values[time], 0));
  const adjacentEqual = paths.reduce((sum, path) => sum + path.probability * Number(path.values[0] === path.values[1]), 0);
  return freeze({
    kind,
    length,
    paths,
...
export function markovState({
  ...
  const history = [{
    step: 0,
    distribution: [initialSunny, 1 - initialSunny],
    flows: null
  }];
  ...
  return freeze({
    ...
    limitingDistribution: a + b === 0 ? history[0].distribution : a === 1 && b === 1 ? initialSunny === 0.5 ? [0.5, 0.5] : null : stationary
  });
}
export function transitionEstimate(trajectories, stateCount = 3) {
  ...
  const estimate = counts.map((row, index) => departures[index] === 0 ? null : row.map(value => value / departures[index]));
  return freeze({
    counts,
    departures,
    estimate
...
export function absorptionState({
  boundary = 4,
  start = 2,
  upward = 0.5,
  steps = 16
  ...
  const columns = system.map((_, index) => solveLinear(system, Array.from({
    length: boundary - 1
  }, (__, row) => Number(row === index))));
  const visits = system.map((_, row) => columns.map(column => column[row]));
  const initial = Array.from({
    length: boundary + 1
  }, (_, state) => Number(state === start));
  const history = [{
    step: 0,
    distribution: initial,
    firstLower: initial[0],
    firstUpper: initial[boundary],
    surviving: 1 - initial[0] - initial[boundary]
  }];
  for (let step = 1; step <= steps; step += 1) {
    const previous = history.at(-1).distribution;
    const distribution = matrix.map((_, destination) => previous.reduce((sum, mass, source) => sum + mass * matrix[source][destination], 0));
...
export function integratedRate(time, rates, switchTime) {
  ...
  if (!Array.isArray(rates) || rates.length !== 2) throw new RangeError('Use two rates.');
  rates.forEach(rate => bounded(rate, 0, 6, 'Rate'));
  return rateMass(rates[0], Math.min(time, switchTime)) + rateMass(rates[1], Math.max(0, time - switchTime));
}
function rateMass(rate, duration) {
  const mass = rate * duration;
  if (rate > 0 && duration > 0 && mass === 0) {
    throw new RangeError('Positive integrated rate is below the supported numeric range.');
  }
  return mass;
}
...
export function arrivalState({
  ...
  // Integrate overlaps directly; subtracting two large accumulated intensities
  // can erase a small positive interval mean.
  const intervalMean = rateMass(rates[0], Math.max(0, Math.min(interval[1], switchTime) - interval[0])) + rateMass(rates[1], Math.max(0, interval[1] - Math.max(interval[0], switchTime)));
  const selected = events.filter(event => event.time > interval[0] && event.time <= interval[1]);
  const pmf = Array.from({
    length: 41
  }, (_, count) => poissonMass(intervalMean, count));
  // Sum the small tail directly: 1 - CDF can erase a representable rare event.
  const pmfTail = Array.from({
    length: 120
  }, (_, index) => poissonMass(intervalMean, 41 + index)).reduce((sum, value) => sum + value, 0);
  return freeze({
    rates: [...rates],
    switchTime,
    horizon,
    ...
    markedCounts: interval[1] <= observedUntil ? [0, 1].map(mark => selected.filter(event => event.mark === mark).length) : null,
    zeroProbability: Math.exp(-intervalMean),
    pmf,
    pmfTail,
...
export function splitCountLaw(mean, first, second, probabilityFirst) {
  ...
  const markKernel = combinations * probabilityFirst ** first * (1 - probabilityFirst) ** second;
  // With zero intensity, a positive total is impossible. The formal binomial
  // marking kernel still exists, but it is not an identified conditional law.
  const conditioningEventPossible = mean > 0 || first + second === 0;
  return freeze({
    conditional: conditioningEventPossible ? markKernel : null,
    markKernel,
    conditioningEventPossible,
    joint: poissonMass(mean, first + second) * markKernel,
    product: poissonMass(mean * probabilityFirst, first) * poissonMass(mean * (1 - probabilityFirst), second)
  });
}
export function jumpClockState({
  ...
  const probabilityOn = time => stationaryOn + (Number(initial === 0) - stationaryOn) * Math.exp(-(alpha + beta) * time);
  const history = Array.from({
    ...
    return {
      time,
      probabilityOn: probabilityOn(time)
    };
  });
  const expectedOnExposure = stationaryOn * horizon + (Number(initial === 0) - stationaryOn) * -Math.expm1(-(alpha + beta) * horizon) / (alpha + beta);
  const segments = [];
  let time = 0;
  let state = initial;
  for (let index = 0; index < maxJumps && time < horizon; index += 1) {
    ...
    segments.push({
      ...
      jumped: time + duration <= horizon
    });
    time = end;
    state = 1 - state;
  }
  const exposure = [0, 1].map(value => segments.filter(segment => segment.state === value).reduce((sum, segment) => sum + segment.end - segment.start, 0));
  const departures = [0, 1].map(value => segments.filter(segment => segment.state === value && segment.jumped).length);
  return freeze({
    alpha,
    beta,
    horizon,
    initial,
    seed,
    generator: [[-alpha, alpha], [beta, -beta]],
    stationaryOn,
    history,
    expectedOnExposure,
    segments,
...
export function brownianFromNormals(normals, {
  horizon = 1,
  drift = 0,
  scale = 1
} = {}) {
...
export function brownianState({
  ...
  const fine = Array.from({
    ...
    return brownianFromNormals(normals, {
      horizon,
      drift,
      scale
    });
  ...
  const paths = fine.map(values => Array.from({
    length: count + 1
  }, (_, index) => values[index * stride]));
  ...
  return freeze({
    ...
    rawVariationVariance: 2 * scale ** 4 * horizon ** 2 / count + 4 * drift ** 2 * scale ** 2 * horizon ** 3 / count ** 2,
    centeredVariationExpectation: scale ** 2 * horizon,
    centeredVariationVariance: 2 * scale ** 4 * horizon ** 2 / count,
    terminalMean: drift * horizon,
    terminalVariance: scale ** 2 * horizon,
    sampledMaximum: Math.max(...selected),
    sampledMinimum: Math.min(...selected)
  });
}
export function bridgeState({
  ...
  probability(fraction, 'Time fraction');
  bounded(barrier, -10, 10, 'Barrier');
  const mean = left + (right - left) * fraction;
  const variance = scale ** 2 * duration * fraction * (1 - fraction);
  const logCrossing = barrier <= Math.max(left, right) ? 0 : -2 * (barrier - left) * (barrier - right) / (scale ** 2 * duration);
