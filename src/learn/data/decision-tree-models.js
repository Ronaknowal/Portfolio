...
function validDenseArray(values, minimum, maximum, accepts) {
...
const validFeatures = values => validDenseArray(values, 2, 2, value => Number.isFinite(value) && Math.abs(value) <= 1000);

function validateRows(rows) {
  if (!validDenseArray(rows, 1, 64, row => row && validFeatures(row.features) && [0, 1].includes(row.label))) throw new RangeError('Use 1–64 binary-labelled rows with two finite features in [−1000,1000].');
}
export function impurity(labels, criterion = 'gini') {
  if (!validDenseArray(labels, 0, 64, label => [0, 1].includes(label)) || !['gini', 'entropy'].includes(criterion)) throw new RangeError('Unsupported labels or impurity.');
...
export function splitCandidates(rows, { criterion = 'gini', minimumLeaf = 1, features = [0, 1] } = {}) {
  validateRows(rows);
  if (!Number.isInteger(minimumLeaf) || minimumLeaf < 1 || minimumLeaf > 32 || !validDenseArray(features, 1, 64, feature => [0, 1].includes(feature))) throw new RangeError('Unsupported split settings.');
...
export function treePrediction(tree, features) {
  if (!validFeatures(features)) throw new RangeError('Supply two bounded finite query values.');
  const path = [];
  let node = tree;
  while (node.left) {
