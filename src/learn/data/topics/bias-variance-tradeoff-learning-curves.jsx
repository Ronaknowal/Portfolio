...
const biasVarianceContent = {
  ...
  content: () => <div className="lesson-pilot bv-lesson">
    ...
    <Prose>Try editing the three prediction dots in the investigation below. Watch the total expected error change directly. Moving one dot toward the true mean can change both the average offset and the spread; the two terms must be recalculated together.</Prose>
    <SpreadLab />
    ...
    <Prose>In the investigation below, edit curvature, noise level and the probe location. Switch between fits to inspect the actual eight fitted curves, average curve and separate error contributions, including changes that preserve expected squared error. A second mode adds training inputs at <Math>{'-.5,.5'}</Math>, enumerating 32 possible datasets. Setting noise to zero is a useful null: a correctly specified, identified polynomial reproduces the truth in every training draw.</Prose>
    <WorldsLab />
