...
function Drawing({
  ...
  return <div className={`conditioning-drawing${compact ? " compact" : ""}`} role="region" tabIndex={0} aria-label={`${label}; scroll horizontally if needed`}><svg viewBox="0 0 440 250" role="img" aria-label={label}>{children}</svg></div>;
}
...
export function RoundingCellsLab() {
  ...
  return <section className="conditioning-lab" aria-label="Rounding cells investigation"><h3>Which stored neighbor wins?</h3><p>Predict the rounded value before moving through the halfway points. This is an exact four-significant-bit toy system with nearest-even rounding.</p><div className="conditioning-controls"><label>Exponent bin<select aria-label="Exponent bin" value={bin} onChange={event => setBin(Number(event.target.value))}><option value="0">1 to 2</option><option value="1">2 to 4</option></select></label><Slider label="Half-step position" value={half} minimum={0} maximum={16} onChange={setHalf} /></div><Drawing label="Exact rounding grid and halfway input"><line x1="42" x2="394" y1="105" y2="105" className="axis" />{result.grid.map((value, index) => <g key={value}><line x1={point(value)} x2={point(value)} y1="96" y2="114" className="tick" /><text x={point(value)} y={index % 2 ? 158 : 138} textAnchor="middle">{value}</text></g>)}<line x1={point(result.input)} x2={point(result.input)} y1="55" y2="94" className="input" /><circle cx={point(result.input)} cy="55" r="6" className="input-fill" /><text x="42" y="25">Input {result.input}</text><circle cx={point(result.rounded)} cy="105" r="8" className="chosen" /><text x="42" y="203">Stored {result.rounded}</text><text x="42" y="228">Gap between neighbors = {result.spacing}</text></Drawing><p className="conditioning-readout" aria-live="polite">Input = {result.input}; stored = {result.rounded}. {result.halfway ? `Exactly halfway: the retained significand is even.` : 'Already on the grid; no rounding change.'}</p><button onClick={() => {
      setBin(0);
      setHalf(1);
...
export function SummationLab() {
  ...
  return <section className="conditioning-lab" aria-label="Summation investigation"><h3>Follow the lost contribution</h3><p>Predict which method will win before changing the order. The table inspects an explicit left fold, not Python's version-dependent built-in sum.</p><div className="conditioning-controls"><label>Input order<select aria-label="Input order" value={preset} onChange={event => {
