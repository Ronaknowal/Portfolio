...
export function TemperatureThresholdLab() {
  ...
  const reset = () => {
    ...
    setOnlyAlerts(true);     setRowPage(0);
  };
  ...
  // The threshold drives both row filtering and the workload summary.
  ...
  return <Investigation
    ...
    <p className="ad-readout" aria-live="polite">Windows with an alert: {hitCount} of 4. Alerts outside every window: {integer(outcome[OUTCOME.outside])}. Change the calibration quantile to follow the threshold, timeline and workload together.</p>
    ...
    <figure className="ad-plot">
      ...
      <svg viewBox="0 0 340 222" role="img" aria-label={`Temperature from ${time(0)} to ${time(rowCounts.total - 1)}, drawn as ${overviewBins.length} bins that keep each bin's lowest and highest reading. The first ${integer(rowCounts.reference)} rows are the reference period, the next ${integer(rowCounts.calibration)} the calibration period, and the remaining ${integer(rowCounts.test)} the inspected period. Four annotated windows are shaded. ${`At the chosen threshold ${integer(outcome[OUTCOME.testAlerts])} later rows alert, ${integer(outcome[OUTCOME.inside])} of them inside a window.`}`}>
        ...
        <g transform="translate(0 118)">
          ...
          <text x="36" y="18">highest score in each bin{', with the threshold'}</text>
          ...
          {<>
            ...
            {overviewBins.filter(bin => bin[scoreColumn] !== null && bin[scoreColumn] > threshold).map(bin => <rect key={bin[0]} x={scaleX(bin[0])} y="84" width="1.4" height="7" fill="#e7b94a" />)}
    ...
    {<>
      <p className="ad-readout" aria-live="polite">
        {mode === 'quantile' ? `Quantile ${round(quantile, 3)} of the ${integer(rowCounts.calibration)} calibration scores is ${round(threshold, 6)}.` : `Your ${round(direct, 3)} snapped to the nearest available level, ${round(threshold, 6)}.`}
      ...
      <Table caption={mode === 'quantile' ? 'The same calibration quantile, method by method, so the workload can be compared on equal footing' : 'A typed score belongs to one method’s scale, so the others cannot be read at the same number'} headings={['method', 'threshold', 'calibration alerts', 'test alerts', 'inside windows', 'outside windows', 'windows hit']} rows={methodOrder.map(key => {
    const row = mode === 'quantile' ? quantileOutcomes[key].find(entry => entry[0] === quantileIndex(quantile, rowCounts.calibration)).slice(1) : null;
    return row === null ? [methodLabels[key], key === method ? round(threshold, 6) : 'other scale', '—', '—', '—', '—', '—'] : [methodLabels[key], round(row[OUTCOME.threshold], 6), integer(row[OUTCOME.calibrationAlerts]), integer(row[OUTCOME.testAlerts]), integer(row[OUTCOME.inside]), integer(row[OUTCOME.outside]), `${windowHits(row[OUTCOME.hits]).filter(Boolean).length}/4`];
    ...
    <details>
      ...
      <div className="ad-buttons">
        ...
        <button type="button" disabled={!true} onClick={() => { setOnlyAlerts(!onlyAlerts); setRowPage(0); }}>{onlyAlerts ? 'Show every row in this block' : 'Show only alerting rows'}</button>
        {false}
      </div>
      <figure className="ad-plot">
        <figcaption>Window {window + 1} with six hours of context on each side{`, and the ${integer(alertRowCount)} rows that alert at this threshold`}</figcaption>
        <svg viewBox="0 0 340 110" role="img" aria-label={`Temperature across ${detail.level.length} rows from ${time(detail.firstRow)} to ${time(detail.firstRow + detail.level.length - 1)}, with the annotated window shaded.${` ${integer(alertRowCount)} of these rows alert.`}`}>
          {(() => {
            ...
            return <>
              ...
              {detailRows.filter(row => row.alert).map(row => <rect key={row.index} x={x(row.index)} y="86" width="1.3" height="7" fill="#e7b94a" />)}
              ...
              {<text x="36" y="12">each mark below the axis is one alerting row</text>}
            </>;
          })()}
        </svg>
      </figure>
      ...
      <Table scroll caption={`${filtered ? `Alerting rows in this block: ${integer(alertRowCount)}` : `Rows in this block: ${integer(detailRows.length)}`}, ${visibleRows.length} shown on this page. Levels, changes and scores are rounded for display; the alert mark uses the exact comparison.`}
        ...
        rows={visibleRows.map(row => [
          ...
          row.inside ? `window ${window + 1}` : 'outside', row.alert ? 'yes' : 'no',
        ])}
