...
export function BashArgumentsLab() {
  ...
  return <MechanismLab id="bash-arguments" title="Where does one argument end?">
    <p>Predict how many arguments a program receives from <code>show_args $value</code>. Then change the quotes. A filename with a space is one value; the shell can accidentally turn it into two.</p>
...
export function BashStatusLab() {
  ...
  return <MechanismLab id="bash-status" title="Data can arrive even when a producer fails">
    <p>A producer prints two rows, then fails. The consumer reads those rows and exits successfully. Predict the pipeline status before enabling pipefail.</p>
...
export function BashPublicationLab() {
  ...
  return <MechanismLab id="bash-publication" title="Which version can a reader see?">
    <p>A previously complete report exists. Predict whether a failed replacement should change it, then follow staging and publication.</p>
