...
export function ApiBoundaryLab() {
  const [kind, setKind] = useState('ordinary'),
    [minimum, setMinimum] = useState(0.85),
    [step, setStep] = useState(0),
    states = apiBoundary(kind, minimum),
    boundaryState = states[step];
  return <Investigation id="api-boundary" kicker="Hints describe; checks decide" title="Can filtering conceal an invalid score?">
 <p className="lesson-note">Change the decoded values and threshold, then follow validation to see why one invalid entry stops the operation.</p><div className="nt-controls"><LabChoices label="Decoded data" value={kind} onChange={v => {
        setKind(v);
        setStep(0);
      }} items={[["ordinary", "0.81 and 0.86"], ["zero", "0 and 1"], ["equal", "Exactly 0.85"], ["invalid", "0.9 and −0.1"], ["boolean", "Boolean true"], ["nan", "Not a number (NaN)"]]} /><LabChoices label="Minimum (inclusive)" value={minimum} onChange={v => {
        setMinimum(Number(v));
        setStep(0);
      }} items={[[0, '0'], [0.85, '0.85'], [1, '1']]} /></div>
...
export function ApiOwnershipLab() {
  const [mode, setMode] = useState('shared'),
    [step, setStep] = useState(0),
    states = ownershipTrace(mode),
    ownershipState = states[step];
  return <Investigation id="api-ownership" kicker="Follow the object, not the variable name" title="Who observes an append?">
 <p className="lesson-note">Run the two calls and inspect the returned references. Compare a shared default, aliasing the supplied list and copying it.</p><div className="nt-controls"><LabChoices label="Ownership policy" value={mode} onChange={v => {
        setMode(v);
        setStep(0);
      }} items={[["shared", "Shared mutable default"], ["mutate", "Alias caller's list"], ["copy", "Copy caller's outer list"]]} /></div>
...
export function ApiCompatibilityLab() {
  const [change, setChange] = useState('same'),
    compatibility = compatibilityModel(change);
  return <Investigation id="api-compatibility" kicker="An unchanged caller meets a changed contract" title="Can a change type-check and still break users?">
 <p className="lesson-note">Switch the library revision and trace whether it rejects the call or changes the returned value. Use the inclusive threshold case to inspect compatibility.</p><div className="nt-controls"><LabChoices label="Library revision" value={change} onChange={setChange} items={[["same", "Same contract"], ["rename", "Rename minimum to min_score"], ["exclusive", "Change ≥ to >"], ["units", "Return percentages"]]} /></div>
