...
export function ComplexBoundaryLab() {
  ...
  return <section className="tda-lab" aria-label="Chains and fillings investigation"><h3>What makes this cycle count as a hole?</h3><p>Start with all three edges selected. Inspect whether adding the face changes their boundary, their status as a cycle, or their status as a <em>boundary of something else</em>.</p>
...
export function RipsFiltrationLab() {
  ...
  return <section className="tda-lab" aria-label="Rips filtration and persistence investigation"><h3>Connect, create a loop, then fill it</h3><p>At ε=2 the square has its perimeter but no diagonal. Inspect the next event: an extra edge by itself or a whole tied group of edges and faces?</p><div className="tda-controls"><Choice label="Point sample" value={kind} options={Object.entries(TOPOLOGY_POINT_FIXTURES).map(([key, item]) => [key, item.label])} onChange={next => changeFixture(next, scale)} /><Choice label="Uniform coordinate scale" value={scale} numeric options={[[0.5, 'Half size'], [1, 'Original size'], [2, 'Double size']]} onChange={next => changeFixture(kind, next)} /></div>
    {/* An integer control endpoint preserves the exact computed maximum after native range serialization. */}
    <label className="tda-control">Rips edge threshold ε: <strong>{format(threshold)}</strong><input
      aria-label="Rips edge threshold ε"
      aria-valuetext={`ε = ${format(threshold)}`}
      type="range" min={0} max={1000} step="any"
      value={1000 * threshold / computation.maximum}
      onChange={event => setThreshold(Number(event.target.value) / 1000 * computation.maximum)}
    /></label>
