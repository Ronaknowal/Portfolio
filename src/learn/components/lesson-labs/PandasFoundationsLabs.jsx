...
export function PandasAlignmentLab() {
  ...
  return <Investigation id="pandas-alignment" kicker="FOLLOW THE LABEL" title="Which fee reaches this order?">
    <p>The table's visible order is b, a. Fees arrive in a different order. Inspect whether an order's total should change when you reorder the rows.</p>
    <p className="lesson-note">Change fee labels and remove the fee for a. Inspect how alignment follows labels even when row positions differ.</p>
    <div className="nt-controls"><label>Assignment rule<select value={mode} onChange={e => setMode(e.target.value)}><option value="labels">Match index labels</option><option value="positions">Use values by position</option></select></label><label>Order row arrangement<select value={String(reversed)} onChange={e => {
          setReversed(e.target.value === 'true');
          setSelected(0);
        }}><option value="false">b, then a</option><option value="true">a, then b</option></select></label><label>Incoming fee labels<select value={String(incomplete)} onChange={e => setIncomplete(e.target.value === 'true')}><option value="false">a: 1 and b: 2</option><option value="true">b: 2 and c: 7 (a missing)</option></select></label><label>Inspect order row<select value={selected} onChange={e => setSelected(Number(e.target.value))}>{model.orders.map((r, i) => <option key={r.label} value={i}>{r.label} — amount {r.value}</option>)}</select></label></div>
...
export function PandasCleaningLab() {
  const [step, setStep] = useState(0),
    [policy, setPolicy] = useState('known');
  ...
  return <Investigation id="pandas-cleaning" kicker="KEEP THE REASON FOR MISSINGNESS" title="Can a successful conversion hide a bad input?">
    ...
    <p className="lesson-note">Switch between known-value and positive-value masks. Track zero and missing values separately through the selected rows.</p>
    <div className="nt-controls"><label>Selection policy<select value={policy} onChange={e => {
          setPolicy(e.target.value);
          setStep(0);
        }}><option value="known">Keep known numbers, including zero</option><option value="positive">Keep strictly positive numbers</option></select></label></div>
    ...
    <p className="nt-feedback" aria-live="polite">{['These are strings. Preserve raw text alongside converted values so that an invalid token does not become indistinguishable from an empty field.', 'to_numeric(errors="coerce") turns unparseable text into missing. Casting to Float64 gives a nullable numeric Series. Coercion has not validated or repaired the source.', policy === 'known' ? 'notna() returns True for 10 and 0, False for both absent numeric values.' : 'The comparison value > 0 yields True, NA, NA, False. With this nullable mask, .loc treats NA as not selected.', `Selected source rows: ${rows.filter(r => r.keep).map(r => r.index).join(', ')}. Keep excluded rows and their reasons for inspection; do not silently describe them as all zero.`][step]}</p><Stepper step={step} count={4} setStep={setStep} />
...
export function PandasGroupingLab() {
  const [keepMissing, setKeepMissing] = useState(true),
    [fillZero, setFillZero] = useState(false),
    [key, setKey] = useState('North');
  const result = groupingModel(keepMissing, fillZero),
    group = result.groups.find(g => g.key === key) || result.groups[0];
  return <Investigation id="pandas-grouping" kicker="SPLIT → REDUCE → RETURN" title="Two rows: why is the mean 10 rather than 5?">
    ...
    <p className="lesson-note">Compare row counts with measured-value counts, then change the missing-value policy and inspect how it changes the aggregate.</p>
    <div className="nt-controls"><label>Missing group key<select value={String(keepMissing)} onChange={e => {
          setKeepMissing(e.target.value === 'true');
          setKey('North');
        }}><option value="true">Keep it: dropna=False</option><option value="false">Omit it: dropna=True</option></select></label><label>Unknown amount policy<select value={String(fillZero)} onChange={e => setFillZero(e.target.value === 'true')}><option value="false">Preserve unknown values</option><option value="true">Assume unknown means zero</option></select></label><label>Trace group<select value={group.key ?? 'missing'} onChange={e => setKey(e.target.value === 'missing' ? null : e.target.value)}>{result.groups.map(g => <option key={g.key ?? 'missing'} value={g.key ?? 'missing'}>{g.key ?? 'Unknown region'}</option>)}</select></label></div>
