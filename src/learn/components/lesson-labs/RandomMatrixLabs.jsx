...
function Axes({
  ...
  return <><path d="M48,25 V190 H338" fill="none" stroke="var(--rm-line)" /><text x="48" y="17">{yLabel}</text><text x="190" y="225" textAnchor="middle">{xLabel}</text><text x="43" y="195" textAnchor="end">0</text><text x="43" y="45" textAnchor="end">{format(maximum, 2)}</text><text x="48" y="210" textAnchor="middle">{format(left, 2)}</text><text x="338" y="210" textAnchor="middle">{format(right, 2)}</text></>;
}
export function NoiseSampleFigure() {
  return <figure className="rm-inline"><div className="rm-counted"><div><strong>Eight observed rows</strong><table><thead><tr><th>First</th><th>Second</th><th>Count</th></tr></thead><tbody>{[[1, 1, 3], [-1, -1, 3], [1, -1, 1], [-1, 1, 1]].map((row, index) => <tr key={index}>{row.map((value, column) => <td key={column}>{value}</td>)}</tr>)}</tbody></table></div><SmallMatrix values={[[1, .5], [.5, 1]]} caption="XᵀX / 8" /></div><Plot label="Observed principal directions: same-sign direction has scale 1.5; opposite-sign direction has scale 0.5"><path d="M60,180 H300 M180,210 V30" stroke="var(--rm-line)" /><path d="M90,190 L270,40" stroke="var(--rm-gold)" strokeWidth="4" /><path d="M150,90 L210,140" stroke="var(--rm-blue)" strokeWidth="4" /><text x="240" y="30" fill="var(--rm-gold)">1.5</text><text x="218" y="145" fill="var(--rm-blue)">0.5</text><text x="288" y="174">x₁</text><text x="186" y="44">x₂</text></Plot><figcaption>This is a possible sample under independent population coordinates. Its observed counts favor matching signs. The gold and blue lines show eigenvector directions, with lengths proportional to eigenvalues; they are not confidence intervals.</figcaption></figure>;
}
export function CenteringRankFigure() {
  return <figure className="rm-inline"><div className="rm-rank">{[[5, 'Five raw row directions', 'At least 3 zero eigenvalues'], [4, 'At most four after centering', 'At least 4 zero eigenvalues']].map(([rank, title, note]) => <div key={title}><strong>{title}</strong><div className="rm-dots" aria-label={rank + ' possible nonzero directions out of eight'}>{Array.from({
            length: 8
...
export function FiniteSpectrumLab() {
  ...
  return <Investigation id="random-finite-calibration" kicker="FINITE NULL · MODEL CHECK" title="Compare a largest eigenvalue with a finite null">
    ...
    <Plot label="Fifty-nine finite null maxima and the observed maximum"><Axes xLabel="Largest eigenvalue" yLabel="59 null draws" maximum={59} right={maximum} /><line x1={x(result.bulkEdge)} x2={x(result.bulkEdge)} y1="30" y2="185" stroke="var(--rm-gold)" strokeDasharray="5 4" />{result.maxima.map((value, index) => <circle key={index} cx={x(value)} cy={55 + index % 7 * 13} r="3.5" fill="var(--rm-blue)" />)}<path d={'M' + x(result.observed) + ',172 l-7,-11 h14z'} fill="var(--rm-green)" /></Plot>
...
export function InverseGainFigure() {
  return <figure className="rm-inline"><div className="rm-gains">{[[.01, 100, 1 / .11], [2, .5, 1 / 2.1]].map(([eigenvalue, before, after]) => <div key={eigenvalue}><strong>Direction with λ = {eigenvalue}</strong><p>Input perturbation: 0.02</p><p>Inverse gain {format(before)} → change {format(.02 * before)}</p><p className="rm-green">Add α = 0.1: gain {format(after)} → change {format(.02 * after)}</p></div>)}</div><figcaption>Adding αI changes every inverse gain from 1/λ to 1/(λ + α). This controls amplification in the weak direction, while also changing the estimation target.</figcaption></figure>;
}
