...
export function TreeSearchLab() {
  ...
  return <TreePanel name="tree-search" eyebrow="COMPARE → CHOOSE ONE SUBTREE" title="Where can this key still be?">
    <p>Predict the path to 7 before stepping. The interval keeps every ancestor restriction; going right from 6 does not erase the earlier requirement to stay below 8.</p>
...
export function TreeTraversalLab() {
  ...
  return <TreePanel name="tree-traversal" eyebrow="PENDING WORK IS DIFFERENT FROM OUTPUT" title="When does a visited node enter the answer?">
    <p>The tree stays fixed. Predict the first three output keys, then compare the frontier with the answer. A call can begin at 8 while inorder output begins at 1.</p>
...
export function TreeDeletionLab() {
  ...
  return <TreePanel name="tree-deletion" eyebrow="RECONNECT THE STRUCTURE, KEEP THE ORDER" title="Which node disappears when you delete a key?">
    <p>Predict the final root and surviving identities before stepping. With two children, this implementation copies the successor's key into the target, then removes the successor node. Copying a key is different from moving the whole node.</p>
