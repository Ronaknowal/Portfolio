...
function Prediction({ label, value, onChange, options }) {
  return <label>{label}<select aria-label={label} value={value} onChange={event => onChange(event.target.value)}><option value="">Choose your prediction</option>{options.map(([key, text]) => <option key={key} value={key}>{text}</option>)}</select></label>;
}
function ExplanationPrompt() {
  return <details><summary>Explain it, then construct a different case</summary><p>Name the input that supplied or removed evidence. Change an edge, coordinate or view value of your own; record a fresh prediction before calculating. The presets are comparisons, not a replacement for your own experiment.</p><label>Your explanation (kept in this lab while you read)<textarea placeholder="Which input caused the change? What observation would test whether it helps?" /></label></details>;
}
...
export function LabelPropagationLab() {
  ...
  const current = trial?.signature === signature ? trial : null;
  const update = callback => { callback(); setPrediction(''); setTrial(null); setStep(0); setShowEquilibrium(false); };
  ...
  function run() {
    const computed = attempt(() => {
      ...
      if (index < 0) throw new Error('Choose an unknown node as the prediction target.');
      const result = propagateGraph(parsed.value, mode, alpha);
      return { result, targetIndex: index, answer: scoreSide(result.scores[index]) };
    });
    setTrial({ signature, prediction, ...computed });
    setStep(0); setShowEquilibrium(false);
  }
  ...
  const canRun = prediction && parsed.value && parsed.value.nodes.some(node => node.name === target && node.label === null);
  return <section className="ssl-lab" aria-labelledby={`${identifier}-title`} data-ssl-lab="graph">
    <h3 id={`${identifier}-title`}>Investigation · repair the neighborhood</h3>
    <p>Edit the actual edges and observed labels. Predict the selected unknown node’s <strong>equilibrium class-1 score</strong>. A half-score is tied; “unavailable” means no evidence. Compare your result with the chain calculation above.</p>
    ...
    <div className="ssl-controls">
      ...
      <Prediction label="My final score prediction" value={prediction} onChange={value => { setPrediction(value); setTrial(null); }} options={[[ 'below', 'Below 0.5' ], [ 'above', 'Above 0.5' ], [ 'tie', 'Exactly tied at 0.5' ], [ 'unavailable', 'Unavailable / no evidence' ]]} />
    </div>
    {parsed.error && <p className="ssl-input-error" role="alert">{parsed.error}</p>}
    <div className="ssl-controls"><button disabled={!canRun} onClick={run}>Commit prediction and calculate</button><button onClick={() => update(() => { setNodes(initialNodes); setEdges(initialEdges); setMode('hard'); setAlpha('0.8'); setTarget('B'); })}>Reset graph</button></div>
    {current?.error && <p role="alert" className="ssl-input-error">{current.error}</p>}
    {model && <div className="ssl-result">
      <div className="ssl-controls"><button disabled={showEquilibrium || step >= model.trace.length - 1} onClick={() => setStep(value => value + 1)}>One synchronous update</button><button disabled={showEquilibrium} onClick={() => setShowEquilibrium(true)}>Reveal equilibrium and feedback</button><button onClick={() => { setStep(0); setShowEquilibrium(false); }}>Replay same prediction</button></div>
...
export function SelfTrainingLab() {
  ...
  const current = trial?.signature === signature ? trial : null;
  const update = callback => { callback(); setMovement(''); setChange(''); setTrial(null); setStep(0); };
  ...
  return <section className="ssl-lab" aria-labelledby={`${identifier}-title`} data-ssl-lab="self-training">
    <h3 id={`${identifier}-title`}>Investigation · a guess changes the next guess</h3>
    <p>Compare the final model with the observed-only baseline for <em>your current input</em>. Predict the boundary’s movement and whether the query’s class changes. A tie in either query comparison has its own answer.</p>
    ...
    <div className="ssl-controls"><Prediction label="My boundary movement prediction" value={movement} onChange={value => { setMovement(value); setTrial(null); }} options={[[ 'left', 'Moves left' ], [ 'right', 'Moves right' ], [ 'unchanged', 'Unchanged' ], [ 'unavailable', 'One model has no unique boundary' ]]} /><Prediction label="Does the query class change?" value={change} onChange={value => { setChange(value); setTrial(null); }} options={[[ 'yes', 'Yes' ], [ 'no', 'No' ], [ 'tie', 'Either comparison is tied' ]]} /></div>
    {parsed.error && <p className="ssl-input-error" role="alert">{parsed.error}</p>}
    <div className="ssl-controls"><button disabled={!parsed.value || !movement || !change} onClick={() => { setTrial({ signature, movement, change, result: prototypeTraining(parsed.value) }); setStep(0); }}>Commit prediction and calculate</button><button onClick={() => update(() => { setObserved('-2,0\n2,1'); setPool('-1, 0, 1, 3'); setThreshold('0.8'); setQuery('1.25'); })}>Reset self-training</button></div>
    {model && <div className="ssl-result">
      <div className="ssl-controls"><button disabled={finished} onClick={() => setStep(value => value + 1)}>Promote one batch and refit</button><button disabled={finished} onClick={() => setStep(model.history.length)}>Reveal final model and feedback</button><button onClick={() => setStep(0)}>Replay same prediction</button></div>
...
export function CoTrainingLab() {
  ...
  const current = trial?.signature === signature ? trial : null;
  const update = callback => { callback(); setPrediction(''); setTrial(null); setStep(0); setStage(0); };
  const model = current?.result;
  ...
  return <section className="ssl-lab" aria-labelledby={`${identifier}-title`} data-ssl-lab="co-training">
    <h3 id={`${identifier}-title`}>Investigation · pass a label through the other view</h3>
    <p>Predict the final category rule for one row in one view. Choose “conflict” when the final views give different answers; unresolved evidence is “unknown.” Each recipient keeps its own label array.</p>
    ...
    <div className="ssl-controls"><label>Target row<select aria-label="Target row" value={target} onChange={event => update(() => setTarget(event.target.value))}>{Number(target) >= (parsed.value?.length ?? 0) && <option value={target}>Choose a row</option>}{parsed.value?.map((row, i) => <option key={i} value={i}>{i}: {row.views.join(' / ')}</option>)}</select></label><label>Recipient view<select aria-label="Recipient view" value={view} onChange={event => update(() => setView(event.target.value))}><option value="0">View 1</option><option value="1">View 2</option></select></label><Prediction label="My final rule prediction" value={prediction} onChange={value => { setPrediction(value); setTrial(null); }} options={[[ '0', 'Class 0' ], [ '1', 'Class 1' ], [ 'conflict', 'Views conflict' ], [ 'unknown', 'Remains unknown' ]]} /></div>
    {parsed.error && <p role="alert" className="ssl-input-error">{parsed.error}</p>}
    <div className="ssl-controls"><button disabled={!prediction || !parsed.value || Number(target) >= parsed.value.length} onClick={() => { setTrial({ signature, prediction, result: categoricalCoTraining(parsed.value) }); setStep(0); setStage(0); }}>Commit prediction and calculate</button><button onClick={() => update(() => { setText(serializeViews(coTrainingRows)); setTarget('3'); setView('0'); })}>Reset co-training</button></div>
    {model && <div className="ssl-result">
      <div className="ssl-controls"><button disabled={final} onClick={() => { if (stage < 4) setStage(value => value + 1); else { setStep(value => value + 1); setStage(0); } }}>Next transfer stage</button><button disabled={final} onClick={() => { setStep(model.history.length - 1); setStage(4); }}>Reveal final rules and feedback</button><button onClick={() => { setStep(0); setStage(0); }}>Replay same prediction</button></div>
