...
export function LinkedReversalLab(){
  const uid=useId(),[size,setSize]=useState(3),[broken,setBroken]=useState(false),[step,setStep]=useState(0),trace=reverseTrace(size,broken),s=trace[step];
  ...
  return <Investigation id="linked-reversal" kicker="SAVE THE ROUTE BEFORE REWIRING IT" title="How do you reverse links without losing the rest of the list?">
    <p>Node identities A, B and C remain fixed on the page; their arrows determine traversal order. A reference stores which node to reach, not the node's screen position.</p><Predict>After changing A.next to empty, how will the loop still find B? Try the faulty order to see what information disappears.</Predict>
    <div className="nt-controls"><label>List length<select value={size} onChange={e=>{setSize(Number(e.target.value));setStep(0);}}><option value="3">Three nodes</option><option value="1">One node</option><option value="0">Empty list</option></select></label><label>Rewiring order<select value={String(broken)} onChange={e=>{setBroken(e.target.value==='true');setStep(0);}}><option value="false">Save successor before overwriting next</option><option value="true">Bug: overwrite next before saving it</option></select></label></div>
    ...
    <svg className="nt-diagram foundation-diagram" viewBox="0 0 360 210" role="img" aria-label={s.nodes.length?s.nodes.map(n=>`${n.id} value ${n.value} points to ${n.next??'empty'}`).join('; '):'Empty list: no nodes'}>
      ...
      {s.nodes.filter(n=>n.next!==null).map(n=>{const direction=x(n.next)>x(n.id)?1:-1;return <path key={n.id} d={`M${x(n.id)+direction*29} 87 Q${(x(n.id)+x(n.next))/2} ${direction===1?42:137},${x(n.next)-direction*31} 87`} fill="none" stroke="#dabb79" strokeWidth="2" markerEnd={`url(#${uid})`}/>;})}
...
export function BracketStackLab(){
  const [text,setText]=useState('([])'),[step,setStep]=useState(0),trace=bracketTrace(text),s=trace[step];
  return <Investigation id="bracket-stack" kicker="MOST RECENT UNMATCHED OPENER CLOSES FIRST" title="Why are equal bracket counts not enough?">
    <p>A stack keeps unmatched openers in the order they must close. The next closer must match the top; it cannot jump past a newer opener to find a convenient match below it.</p><Predict>Both ([]) and ([)] contain one pair of each kind. Which one breaks proper nesting, and at which character?</Predict>
    <div className="nt-controls"><label>Bracket input<select value={text} onChange={e=>{setText(e.target.value);setStep(0);}}>{['([])','([)]',')','(()',''].map(v=><option key={v} value={v}>{v||'(empty input)'}</option>)}</select></label></div>
    <div className="foundation-units" aria-label="Input and current position">{Array.from(text).map((char,i)=><span className="nt-chip" style={{background:i===s.index?'#423319':'transparent'}} key={i}>{char}<small> {i}</small>{i===s.index?' ←':''}</span>)}{!text&&<span>empty input</span>}</div>
...
export function CircularQueueLab(){
  const [capacity,setCapacity]=useState(4),[step,setStep]=useState(0),trace=ringTrace(capacity),s=trace[step];
  return <Investigation id="circular-queue" kicker="PHYSICAL SLOTS WRAP · LOGICAL ORDER DOES NOT" title="How can a queue reuse the front without shifting every item?">
    <p>Jobs arrive as A, B, C, D, E and F. Head identifies the oldest stored job; size counts occupied slots. The next insertion position is (head + size) remainder capacity.</p><Predict>When head moves to 1 and the next insertion wraps to slot 0, is slot 0 now the oldest item? What should happen when the queue is full?</Predict>
    <div className="nt-controls"><label>Buffer capacity<select value={capacity} onChange={e=>{setCapacity(Number(e.target.value));setStep(0);}}><option value="3">3 jobs</option><option value="4">4 jobs</option></select></label></div>
    ...
    <details><summary>Inspect exact slot contents</summary><div className="foundation-buffer" style={{gridTemplateColumns:`repeat(${capacity},minmax(0,1fr))`}}>{s.cells.map((value,i)=><div key={i} className={(i===s.head?'is-head ':'')+(i===s.changed?'is-changed':'')}><small>slot {i}</small><strong>{value??'·'}</strong><small>{[i===s.head?'HEAD':'',i===s.tail?'NEXT':''].filter(Boolean).join(' + ')||'—'}</small></div>)}</div></details>
