...
export function ArrayMovementLab(){
  const uid=useId(),[operation,setOperation]=useState('insert'),[full,setFull]=useState(false),[step,setStep]=useState(0);
  const trace=arrayMovementTrace(operation,full),s=trace[step],newY=s.oldCells?147:92;
  ...
  return <Investigation id="array-movement" kicker="LOGICAL LENGTH ≠ ALLOCATED CAPACITY" title="Why can inserting one item move several others?">
    <p>Keep A, B and C in order. Insert X or remove B while following each slot write. Empty dots are unused capacity, not stored items.</p><Predict>Which direction must a right shift proceed to avoid overwriting unread values? Is appending still one write when the backing storage is full?</Predict>
    <div className="nt-controls"><label>Sequence operation<select value={operation} onChange={e=>{setOperation(e.target.value);setStep(0);}}><option value="front">Insert X at index 0</option><option value="insert">Insert X at index 1</option><option value="append">Append X at index 3</option><option value="delete">Delete B at index 1</option></select></label><label>Starting capacity<select value={String(full)} onChange={e=>{setFull(e.target.value==='true');setStep(0);}}><option value="false">4 slots · one spare</option><option value="true">3 slots · full</option></select></label></div>
...
export function TextUnitsLab(){
  const [kind,setKind]=useState('decomposed'),[normalize,setNormalize]=useState(false),[selected,setSelected]=useState(3);
  const m=textModel(kind,normalize),index=Math.min(selected,m.units.length-1),u=m.units[index];
  return <Investigation id="text-units" kicker="VISIBLE TEXT · CODE POINTS · ENCODED BYTES" title="Can two identical-looking names have different lengths?">
    <p>Python strings are sequences of Unicode code points. UTF-8 encodes those points into bytes for storage or transmission. A mark you perceive as one character can contain several code points.</p><Predict>Compare café with the version built from e and a separate accent. Will exact equality agree with appearance? Does normalization remove the accent?</Predict>
    <div className="nt-controls"><label>Text sample<select value={kind} onChange={e=>{setKind(e.target.value);setSelected(0);}}><option value="ascii">cat · ASCII</option><option value="composed">café · precomposed é</option><option value="decomposed">cafe + combining acute accent</option><option value="emoji">🙂 · one emoji code point</option></select></label><label>Text operation<select value={String(normalize)} onChange={e=>{setNormalize(e.target.value==='true');setSelected(0);}}><option value="false">Keep original code points</option><option value="true">Create NFC-normalized string</option></select></label></div>
    ...
    <p>Encoded bytes (hexadecimal):</p><div className="foundation-units">{m.bytes.map((byte,i)=><span key={i} className="nt-chip" style={{borderColor:i>=u.byteOffset&&i<u.byteOffset+u.bytes.length?'#edc268':'#61533b'}}>{byte.toString(16).toUpperCase().padStart(2,'0')}</span>)}</div>
    ...
    <button type="button" onClick={()=>{setKind('decomposed');setNormalize(false);setSelected(3);}}>Reset text</button>
...
export function HashBucketLab(){
  const [key,setKey]=useState(18),[capacity,setCapacity]=useState(4),[operation,setOperation]=useState('get'),[step,setStep]=useState(0);
  const trace=hashTrace(key,capacity,operation),s=trace[step];
  return <Investigation id="hash-buckets" kicker="HASH NARROWS · EQUALITY DECIDES" title="What stops a collision from returning the wrong value?">
    <p>Event IDs 10, 14 and 18 map to counts 2, 5 and 1. This teaching table chooses a bucket using ID remainder bucket-count, then compares the complete keys inside it.</p><Predict>With four buckets, all three IDs collide. Should looking up 22 return the count for 10 just because both reach bucket 2?</Predict>
    <div className="nt-controls"><label>Event key<select value={key} onChange={e=>{setKey(Number(e.target.value));setStep(0);}}>{[10,14,18,22].map(k=><option key={k}>{k}</option>)}</select></label><label>Bucket count<select value={capacity} onChange={e=>{setCapacity(Number(e.target.value));setStep(0);}}><option>4</option><option>5</option></select></label><label>Map operation<select value={operation} onChange={e=>{setOperation(e.target.value);setStep(0);}}><option value="get">Look up value</option><option value="set">Set value to 99</option></select></label></div>
