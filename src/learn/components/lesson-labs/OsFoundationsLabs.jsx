...
export function SchedulingLab() {
  const [quantum, setQuantum] = useState(1),
    [io, setIo] = useState(true),
    [step, setStep] = useState(0);
  const trace = scheduleTrace(quantum, io),
    state = trace[step];
  return <Investigation id="os-scheduler" kicker="ONE CPU · TWO SAVED CONTEXTS" title="Where does a paused calculation keep its place?">
    ...
    <p className="lesson-note">Change the time slice and block A for device work. Follow which process runs and which counters belong to it.</p>
    <div className="nt-controls"><label>Time slice<select value={quantum} onChange={e => {
          setQuantum(Number(e.target.value));
          setStep(0);
        }}><option value="1">One instruction interval</option><option value="2">Two instruction intervals</option></select></label><label>A's middle instruction<select value={String(io)} onChange={e => {
          setIo(e.target.value === 'true');
          setStep(0);
        }}><option value="true">Request I/O, then wait two intervals</option><option value="false">Add 1 without waiting</option></select></label></div>
...
export function AddressTranslationLab() {
  const uid = useId(),
    [process, setProcess] = useState('A'),
    [address, setAddress] = useState(22),
  ...
  return <Investigation id="os-translation" kicker="VIRTUAL BYTE → PAGE ENTRY → PHYSICAL BYTE" title="Can the same address identify different data?">
    <p>Select A or B while holding virtual byte 22 fixed. These toy pages contain 16 bytes; real base pages are usually much larger. Page 0 is read-only, page 1 is private data, page 2 is valid but not resident, and page 3 is unmapped.</p><p className="lesson-note">Change access type and page state. Follow address translation and distinguish permission failures from valid nonresident pages.</p>
    <div className="nt-controls"><label>Selected process<select value={process} onChange={e => {
          setProcess(e.target.value);
          setStep(0);
        }}><option>A</option><option>B</option></select></label><label>Virtual byte address<select value={address} onChange={e => {
          setAddress(Number(e.target.value));
          setStep(0);
        }}>{[0, 1, 2, 3].map(page => page * 16 + m.offset).map(v => <option key={v} value={v}>{v} · page {Math.floor(v / 16)}</option>)}</select></label><label>Requested access<select value={access} onChange={e => {
          setAccess(e.target.value);
          setStep(0);
        }}><option value="read">Read</option><option value="write">Write 99</option></select></label></div>
    <p><strong>Choose a byte within page {m.page}.</strong> Each position below is one byte offset. Inspect which part of the physical address changes when you choose a neighbor.</p>
    <div className="offset-strip" role="group" aria-label="Byte offset within the selected virtual page">{Array.from({
        length: 16
      }, (_, offset) => <button type="button" key={offset} aria-label={`Offset ${offset}`} aria-pressed={m.offset === offset} onClick={() => {
        setAddress(m.page * 16 + offset);
        setStep(0);
      }}>{offset}</button>)}</div>
...
export function CopyOnWriteLab() {
  ...
  return <Investigation id="os-sharing" kicker="PRIVATE BEHAVIOR CAN START WITH SHARED STORAGE" title="Why can copying wait until the first write?">
    <p>A and B initially read 7 from the same physical frame. Choose whether their mapping promises private writes or deliberately shared updates; then let B write 9.</p><p className="lesson-note">Compare private and shared mappings while writing 7 or 9. Track virtual addresses and backing frames to see which process observes the write.</p>
    <div className="nt-controls"><label>Mapping contract<select value={String(shared)} onChange={e => {
          setShared(e.target.value === 'true');
          setStep(0);
        }}><option value="false">Private · copy on write</option><option value="true">Explicit shared mapping</option></select></label></div>
