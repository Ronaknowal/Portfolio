...
export function SqlJoinLab() {
  ...
  const change = fn => e => { fn(e.target.value); setSelected(0); };
  return <section className="lesson-lab data-lab" data-lab="sql-join" aria-labelledby={`${uid}-title`}>
    ...
    <p>A has two readings, B has one reading whose value is NULL, and C has no reading. Predict the output count before changing a control, then select an output row to find its sources.</p>
    ...
    <button className="data-reset" type="button" onClick={() => { setJoin('left'); setDuplicate(false); setCutoff(10); setPlacement('on'); setSelected(0); }}>Reset join</button>
...
function CreditState({ title, values }) {
  return <div className="data-state"><h4>{title}</h4>{values.map((value,index) => <div className="data-credit" key={index}><span>Account {index === 0 ? 'A' : 'B'}</span><div className="data-credit-track"><i style={{ width: `${value * 10}%` }} /></div><strong>{value}</strong></div>)}<p>Total: <strong>{values[0] + values[1]} credits</strong></p></div>;
}
...
export function SqlTransactionLab() {
  ...
  return <section className="lesson-lab data-lab" data-lab="sql-transaction" aria-labelledby={`${uid}-title`}>
    ...
    <div className="data-controls">
      <label>Transaction boundary<select value={atomic ? 'atomic' : 'separate'} onChange={e => { setAtomic(e.target.value === 'atomic'); setStep(0); }}><option value="atomic">One explicit transaction</option><option value="separate">Commit each statement separately</option></select></label>
      <label>Second statement<select value={fail ? 'fail' : 'success'} onChange={e => { setFail(e.target.value === 'fail'); setStep(0); }}><option value="fail">Bug: invalid negative balance</option><option value="success">Correct credit of 2</option></select></label>
    </div>
    <DataPrediction key={`${atomic}-${fail}`} id={`${uid}-prediction`}>After failure and rollback, what will each account contain?</DataPrediction>
