...
export function Investigation({id,kicker,title,children}) {
  const uid=useId();
  return <section className="lesson-lab nt-lab" data-live-exploration data-investigation={id} aria-labelledby={uid}><p className="lesson-eyebrow">{kicker}</p><h3 id={uid}>{title}</h3>{children}</section>;
}
