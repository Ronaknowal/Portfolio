...
export function OopBindingLab() {
  ...
  const reset = () => {
    setState(initialObjectState());
    setReceiver("morning");
    setValue(18);
    setStep(0);
  };
  return <section data-live-exploration className="oop-lab" aria-labelledby="oop-binding-title" data-oop-lab="binding">
    <p className="lesson-eyebrow">INVESTIGATE · WHICH OBJECT CHANGES?</p>
    <h3 id="oop-binding-title">Follow the receiver into self</h3>
    <p>There are two log objects and two lists. The names <code>morning</code> and <code>alias</code> both point to A. Change the receiver and input, then follow the four stages of one method call.</p>
    ...
    <StepControls step={step} setStep={setStep} reset={reset} extra={<button type="button" disabled={step !== 4} onClick={() => {
      setState(frames[4].state);
      setStep(0);
    }}>Keep result; prepare another call</button>} />
...
export function OopLookupLab() {
  ...
  const [message, setMessage] = useState("Append 18 through A and inspect what B reads from the shared or separate list.");
  const chosen = lookupValues(state, receiver);
  const act = action => {
    setHistory([...history, state]);
    setState(lookupAction(state, receiver, action));
    setMessage(action === "assign" ? `${receiver.toLowerCase()}.values = [99] creates an instance attribute on object ${receiver}. The class list and the other object's attribute do not change.` : `${receiver.toLowerCase()}.values was found on the ${chosen.source}. append(18) mutates list ${chosen.listId}; it does not assign a new attribute.`);
  };
  const mode = nextMode => {
    setState(initialLookupState(nextMode));
    setHistory([]);
    setMessage(nextMode === "class" ? "Reset: both objects must look up values on the class." : "Reset: __init__ has created one values list per instance.");
  };
  return <section data-live-exploration className="oop-lab" aria-labelledby="oop-lookup-title" data-oop-lab="lookup">
    ...
    <div className="oop-controls">
      ...
      <button type="button" disabled={!history.length} onClick={() => {
        ...
        setMessage("Previous model state restored. Re-read where each object finds values.");
      }}>Undo action</button>
...
export function OopValidationLab() {
  ...
  return <section data-live-exploration className="oop-lab" aria-labelledby="oop-validation-title" data-oop-lab="validation">
    <p className="lesson-eyebrow">INVESTIGATE · A VALID STATE</p><h3 id="oop-validation-title">Where must the operation stop?</h3>
    <p>The log starts with [18.0]. Change the candidate input and inspect the first failing check and resulting list immediately. Every attempt uses that same starting state, so the comparison isolates validation.</p>
    <div className="oop-controls oop-controls--inputs"><label>Candidate reading<select aria-label="Candidate reading" value={candidate} onChange={e => {
          setCandidate(e.target.value);
        }}>{readingCandidates.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><button type="button" onClick={() => {
        setCandidate("bool");
      }}>Reset investigation</button></div>
...
export function OopCompositionLab() {
  ...
  return <section data-live-exploration className="oop-lab" aria-labelledby="oop-composition-title" data-oop-lab="composition">
    ...
    <p>The caller has a temperature in degrees Celsius. The report adds a label; its formatter turns the number into text. Change the input and formatter, then step into and back out of the two calls to trace the output.</p>
    ...
    <StepControls step={step} setStep={setStep} reset={() => {
      setFormatter("celsius");
      setValue(20);
      setStep(0);
    }} />
    <div className="oop-feedback" aria-live="polite" aria-atomic="true"><strong>{frame.title}</strong><p>{frame.note}</p></div>
