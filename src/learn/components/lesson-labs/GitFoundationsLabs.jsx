...
export function GitStagingLab() {
  const [step,setStep]=useState(0),[restage,setRestage]=useState(false);
  const trace=stagingTrace(restage),state=trace[step];
  return <Investigation id="git-staging" kicker="EDIT → CAPTURE → COMMIT" title="Does committing save the file you are looking at?">
    <p>One tracked file begins at version 1. Stage version 2, then edit it again. Keep the three stored versions visible as you follow the arrows.</p><Predict>At version 3 on disk, will a normal commit record 2 or 3? Which command changes that answer?</Predict>
    <div className="nt-controls"><label>Before committing<select value={String(restage)} onChange={e=>{setRestage(e.target.value==='true');setStep(0);}}><option value="false">Inspect without staging again</option><option value="true">Stage version 3 again</option></select></label></div>
...
export function GitBranchLab() {
  const uid=useId(),[step,setStep]=useState(0),[diverged,setDiverged]=useState(false);
  const trace=branchTrace(diverged),state=trace[step],positions={A:[52,143],B:[167,65],C:[167,225],M:[293,143]};
  const checked=state[state.head],files=state.nodes.find(n=>n.id===checked).files;
  return <Investigation id="git-branches" kicker="SNAPSHOTS STAY; REFERENCES MOVE" title="Why can one merge just move a branch name?">
    <p>Feature adds a note. Main may also add units. Watch commits, parent arrows and the branch that HEAD names. Files shown below belong to the checked-out snapshot.</p><Predict>After creating feature at A, how many commits exist? If main adds C, can it still fast-forward to B?</Predict>
    <div className="nt-controls"><label>Main branch activity<select value={String(diverged)} onChange={e=>{setDiverged(e.target.value==='true');setStep(0);}}><option value="false">Main stays at A</option><option value="true">Main makes its own commit C</option></select></label></div><pre className="nt-code">{state.command}</pre>
    <svg className="nt-diagram" viewBox="0 0 360 295" role="img" aria-label={`${state.nodes.length} commits. main at ${state.main}; feature at ${state.feature??'not created'}; HEAD names ${state.head}. Arrows point to parents.`}>
      ...
      {state.nodes.flatMap(n=>n.parents.map(parent=>{const [x,y]=positions[n.id],[px,py]=positions[parent],length=Math.hypot(px-x,py-y),ux=(px-x)/length,uy=(py-y)/length;return <line key={n.id+parent} x1={x+ux*23} y1={y+uy*23} x2={px-ux*25} y2={py-uy*25} stroke="#a99260" strokeWidth="2" markerEnd={`url(#${uid})`}/>;}))}
      {state.nodes.map(n=>{const [x,y]=positions[n.id];return <g key={n.id}><circle cx={x} cy={y} r="22" fill={checked===n.id?'#3c3019':'#1a211b'} stroke={checked===n.id?'#e2bb68':'#8dab91'} strokeWidth="2"/><text x={x} y={y+6} textAnchor="middle" className="nt-emphasis">{n.id}</text><text x={x} y={y+43} textAnchor="middle" className="nt-small">{[state.main===n.id?'main':'',state.feature===n.id?'feature':''].filter(Boolean).join(', ')}</text></g>;})}
...
export function GitRemoteLab() {
  const [step,setStep]=useState(0),[localChange,setLocalChange]=useState(false);
  const trace=remoteTrace(localChange),state=trace[step];
  return <Investigation id="git-remotes" kicker="SHARED STATE ≠ YOUR LAST FETCH" title="Did fetch update your files or your knowledge?">
    <p>A colleague publishes B after your clone of A. Origin/main is a reference stored on your machine; it records what you last learned about the shared branch.</p><Predict>Which locations change when the colleague pushes? Which change when you fetch? What if you already made local commit C?</Predict>
    <div className="nt-controls"><label>Your local work<select value={String(localChange)} onChange={e=>{setLocalChange(e.target.value==='true');setStep(0);}}><option value="false">No new local commit</option><option value="true">Commit a local note as C</option></select></label></div><pre className="nt-code">{state.command}</pre>
...
export function GitConflictLab() {
  const [step,setStep]=useState(0),[resolution,setResolution]=useState('combined');
  const trace=conflictTrace(resolution),state=trace[step];
  return <Investigation id="git-conflict" kicker="BASE + TWO CHANGES → A DECISION" title="Can a successful resolution still lose the intended meaning?">
    <p>One branch adds “by model”; the other adds “by region” to the same title. The intended combined report groups by both. Compare the three source versions before editing.</p><Predict>Does deleting conflict markers tell Git which meaning is correct? After editing, is the path already marked resolved?</Predict>
    <div className="nt-controls"><label>Chosen resolution<select value={resolution} onChange={e=>{setResolution(e.target.value);setStep(0);}}><option value="combined">Describe model and region</option><option value="ours">Keep only the model wording</option></select></label></div>
