...
function PriceTrace({
  ...
  return <Plot title="Selected prefix of projected price updates and an optimal reference price"><line x1="44" x2="308" y1="213" y2="213" className="duality-axis" /><line x1="44" x2="308" y1={y(state.optimumPrice)} y2={y(state.optimumPrice)} className="duality-line green dashed" /><path d={polyline(state.frames.slice(0, step + 1).map(frame => [frame.step, frame.price]), x, y)} className="duality-line amber" />{state.frames.slice(0, step + 1).map(frame => <circle key={frame.step} cx={x(frame.step)} cy={y(frame.price)} r={frame.step === step ? 5 : 2.5} className="duality-dot amber" />)}<text x="44" y="23" className="duality-label">price λ</text><text x="308" y="23" textAnchor="end" className="duality-tick">top {number(top)}</text><text x="44" y="235" className="duality-tick">0</text><text x="308" y="235" textAnchor="end" className="duality-tick">{state.updates}</text><text x="308" y="255" textAnchor="end" className="duality-label">price update index</text></Plot>;
}
