...
function NumericField({ label, value, onChange, min, max, step = 'any' }) {
  ...
  return <div className="gp-numeric-field">
    <label htmlFor={numberId}>{label}</label>
    <input type="range" aria-label={`${label} slider`} value={value === '' ? min : value} min={min} max={max} step={step} onChange={event => onChange(event.target.value)} />
    <input id={numberId} type="number" value={value} min={min} max={max} step={step} onChange={event => onChange(event.target.value)} />
  </div>;
}
...
export function GaussianConditioningLab({ direct = false }) {
  ...
  const { result, error } = useMemo(() => {
    try {
      const before = calculate(baseline), after = calculate(settings);
      return { result: { before, after, expected: compareTarget(before, after) }, error: '' };
    } catch (failure) { return { result: null, error: failure.message }; }
  }, [settings, baseline, direct]);
  const reset = () => {
  ...
  return <section className="gp-lab" data-live-exploration="gaussian-process" data-gp-lab={direct ? 'direct' : 'conditioning'} aria-label={direct ? 'One-reading conditioning investigation' : 'Editable Gaussian conditioning investigation'}>
    <h3>{direct ? 'One reading: change the connection' : 'Move the measurements; inspect the information'}</h3>
    <p>Compare your edited state with this saved baseline: <strong>{baselineText}</strong>. Initially the two states match. Change an input and inspect the live comparison; an unchanged experiment is a valid null case.</p>
    ...
    <div className="gp-actions"><button type="button" disabled={!result} onClick={() => setBaseline(structuredClone(settings))}>Save this state as baseline</button><button type="button" onClick={reset}>Reset investigation</button></div>
    {error && <p role="alert" className="gp-error">{error}</p>}
    {result && <div data-gp-result="conditioning"><div className="gp-feedback" role="status"> The calculated change is: {changes.find(row => row[0] === result.expected)[1].toLowerCase()}. Comparisons treat absolute differences ≤ 10⁻⁹ as numerical equality.</div>
...
export function GaussianForecastLab() {
  ...
  const [developmentVisible, setDevelopmentVisible] = useState(true);
  ...
  const edit = next => { setSettings(next); clear(); };
  const { result, error } = useMemo(() => {
    if (mode !== 'explore') return { result: null, error: '' };
    try {
      ...
      return { result: { forecast, scores: scoreForecast(forecast.rows, forecast.mean, forecast.observationSD) }, error: '' };
    } catch (failure) { return { result: null, error: failure.message }; }
  }, [settings, mode]);
  return <section className="gp-lab" data-live-exploration="gaussian-process" data-gp-lab="forecast" aria-label="Historical CO2 forecast investigation"><h3>A forecast is a decision made before future readings</h3>
    ...
    {mode === 'reported' ? <>
      <p>Observe 1990–95 first. The annual period in the composite family is fixed at one year. Compare how each family continues both trend and seasonality into 1996–97.</p>
      <GPChart label="Only the available 1990–95 monthly CO2 observations" series={[{ values: data.observations.slice(0, 72).map(row => [1990 + row.x, row.co2]) }]} xLabel="calendar year" yLabel="CO₂ (ppm)" xFormat={v => Number(v.toFixed(1))} />
        {developmentVisible && <div data-gp-result="development"><p role="status" className="gp-feedback"> Composite development MAE: {gpNumber(data.real.development.trend_periodic.mae)} ppm; RBF-only: {gpNumber(data.real.development.rbf.mae)} ppm. Freeze the composite family for the final refit.</p>
        <p>The two forecast panels share calendar and vertical scales.</p><div className="gp-panels">{Object.entries(data.real.development).map(([family, forecast]) => <ForecastResult key={family} forecast={forecast} cutoff={72} title={`${family} · development`} yDomain={developmentDomain} />)}</div>
            {testVisible && <div data-gp-result="test"><p role="status" className="gp-feedback">Point error and interval behavior answer different questions. Test MAE is {gpNumber(data.real.final_test.mae)} ppm versus {gpNumber(data.real.seasonal_naive.mae)} ppm for seasonal-naive, yet only {data.real.final_test.covered}/24 observations fall inside the intervals.</p><ForecastResult forecast={data.real.final_test} cutoff={96} title="Final test after the frozen-family refit" /></div>}
      </div>}
    </> : <>
      ...
      {result && <div data-gp-result="exploration"><p role="status" className="gp-feedback">The first target has not changed. The same observed prefix and kernel give the same marginal prediction at that target. Additional requested targets do not become new observations.</p>
        ...
        <p>Now shorten the horizon to six with the same prefix and family. Inspect and compare the first six table rows. Then change the cutoff: that introduces genuinely new observations.</p>
      </div>}
    </>}
...
export function GaussianProbeLab() {
  ...
  const { result, error } = useMemo(() => {
    try {
      ...
      return { result: { rows, winners }, error: '' };
    } catch (failure) { return { result: null, error: failure.message }; }
  }, [settings]);
  return <section className="gp-lab" data-live-exploration="gaussian-process" data-gp-lab="probe" aria-label="Next probe investigation"><h3>Buy information about a particular target</h3><p>Choose the reading that most reduces the latent variance at your target. Candidate measurement values are still unknown; the gain does not depend on their future observed values.</p>
    <ObservationEditor rows={settings.observations} onChange={observations => edit({ ...settings, observations })} />
    ...
    {error && <p role="alert" className="gp-error">{error}</p>}
    {result && <div data-gp-result="probe"><p role="status" className="gp-feedback">Largest variance reduction: {result.winners.join(", ")}. {result.winners.length > 1 && 'The maximum is tied.'}</p><ProbeResults rows={result.rows} /><p>The subtraction starts from the current posterior, before the new reading. Zero covariance gives exactly zero gain. Move the target toward 4, or increase one candidate’s noise, and compare the recalculated gains.</p></div>}
  </section>;
}
