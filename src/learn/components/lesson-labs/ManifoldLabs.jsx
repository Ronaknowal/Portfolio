...
import {
  DataTable, DigitLegend, DigitScatter, DigitTile, EqualPlot, Investigation, NumberField,
  Prediction, Reflection, SelectField, comparisonValues, format, useManifoldInvestigation,
} from './ManifoldShared.jsx';
...
const changeName = (before, after) => Math.abs(after - before) <= 1e-9 ? 'same' : after > before ? 'higher' : 'lower';
const verdict = correct => correct ? 'Your prediction matches. ' : 'Compare this with your prediction. ';
...
export function ManifoldGraphLab() {
  ...
  const baseline = draft.start !== draft.end
    ? radiusGraph(active.points, active.radius, draft.start, draft.end) : graph;
  ...
  const evaluate = (inputs, old, prediction) => {
    const before = radiusGraph(old.points, old.radius, inputs.start, inputs.end);
    const after = radiusGraph(inputs.points, inputs.radius, inputs.start, inputs.end);
    const observed = before.distance === null ? (after.distance === null ? 'no' : 'yes')
      : after.distance === null ? 'none' : changeName(before.distance, after.distance);
    setEdgeIndex(0);
    return { observed, before: before.distance, after: after.distance,
      message: `${verdict(prediction === observed)}You predicted ${prediction}. ${inputs.start}–${inputs.end}: ${format(before.distance)} → ${format(after.distance)} coordinate units. ${after.distance === null ? 'The chosen edges do not join these endpoints.' : `The shortest route is ${after.path.join(' → ')}; its ${after.routeEdges.length} edge lengths sum to ${format(after.distance)}.`}` };
  };
  return <Investigation kind="graph" title="A · Build a route before flattening it" state={state}
    ...
    <p>Move D sideways, or choose another point and edit its coordinates. Predict what your proposed graph does to the route. Hollow points preview unapplied locations.</p>
    <div className="mf-controls">
      ...
      <NumberField label="Proposed radius ε" value={draft.radius} onChange={radius => state.edit({ radius })} min={0.25} max={3} step={0.25} />
    ...
    <div className="mf-buttons" aria-label="Graph draft fixtures">
      <button type="button" onClick={() => state.edit({ points: U_POINTS, radius: 0.75 })}>Draft U · ε 0.75</button>
      <button type="button" onClick={() => state.edit({ points: U_POINTS, radius: 1.5 })}>Draft U · ε 1.5</button>
      <button type="button" onClick={() => state.edit({ points: U_POINTS, radius: 2 })}>Draft U · ε 2</button>
      <button type="button" onClick={() => state.edit({ radius: 2.5 })}>Draft ε 2.5</button>
      <button type="button" onClick={() => state.edit({ points: U_POINTS.map(item => item.id === 'D' ? { ...item, y: 3 } : item), radius: 1 })}>Draft D at (1, 3)</button>
    </div>
    <p className="mf-caption">Compare the same proposed endpoints, {draft.start} and {draft.end}, on the old graph (ε = {format(active.radius)}) and the proposed graph. Their old-graph distance is {format(baseline.distance)}.</p>
    <Prediction state={state} prompt={baseline.distance === null ? 'Will the proposed graph have a path?' : 'How will the shortest graph path change?'}
      choices={baseline.distance === null ? [['yes', 'A path will exist'], ['no', 'No path']]
        : [['lower', 'Shorter'], ['same', 'Same length'], ['higher', 'Longer'], ['none', 'No path']]}
      action="Apply graph" evaluate={evaluate} />
    <div className="mf-geometry-pair">
      <div>
        ...
        {previews.length > 0 && <p className="mf-caption">Draft locations: {previews.map(item => `${item.id} (${item.x}, ${item.y})`).join('; ')}.</p>}
      </div>
      <div>
        ...
        </> : <p>No route edges to trace. Adjust the graph, then predict whether it reconnects.</p>}
      </div>
    </div>
...
export function ManifoldProbabilityLab() {
  ...
  const evaluate = (inputs, old, prediction) => {
    const before = gaussianRow(inputs.distances, old.sigma);
    const after = gaussianRow(inputs.distances, inputs.sigma);
    const index = candidateIds.indexOf(inputs.candidate);
    const beforeValue = inputs.question === 'perplexity' ? before.perplexity : before.probabilities[index];
    const afterValue = inputs.question === 'perplexity' ? after.perplexity : after.probabilities[index];
    const observed = changeName(beforeValue, afterValue);
    const [beforeText, afterText] = comparisonValues(beforeValue, afterValue);
    const equal = inputs.distances.every(distance => distance === inputs.distances[0]);
    return { observed, before: beforeValue, after: afterValue,
      message: `${verdict(prediction === observed)}You predicted ${prediction}. ${inputs.question === 'perplexity' ? 'Perplexity' : `Candidate ${inputs.candidate} probability`}: ${beforeText} → ${afterText} at σ ${format(old.sigma)} → ${format(inputs.sigma)}, using the proposed distance row for both. ${equal ? 'Equal distances give equal weights; normalization cancels their shared scale.' : inputs.sigma === old.sigma ? 'The same row at the same bandwidth gives the same distribution.' : inputs.sigma > old.sigma ? 'Broader bandwidth makes the row more even by giving distant alternatives relatively more weight.' : 'Narrower bandwidth concentrates the row toward the nearest alternatives.'}` };
  };
  return <Investigation kind="probability" title="B · Change who receives the probability" state={state}>
    ...
    <div className="mf-controls">
      ...
      <SelectField label="Predict a quantity" value={draft.question} onChange={question => state.edit({ question })}
    ...
    <div className="mf-buttons">
      <button type="button" onClick={() => state.edit({ distances: [1, 2, 3] })}>Draft distances 1, 2, 3</button>
      <button type="button" onClick={() => state.edit({ distances: [2, 2, 2] })}>Draft distances 2, 2, 2</button>
      <button type="button" onClick={() => state.edit({ sigma: 0.5 })}>Draft σ 0.5</button>
      <button type="button" onClick={() => state.edit({ sigma: 2 })}>Draft σ 2</button>
    </div>
    ...
    <Prediction state={state} prompt={`Will ${draft.question === 'perplexity' ? 'perplexity' : `the probability of ${draft.candidate}`} increase, decrease or stay the same?`}
      choices={[["higher", 'Increase'], ['lower', 'Decrease'], ['same', 'Stay the same']]}
      action="Apply bandwidth" evaluate={evaluate} />
...
function IdealPairLab({ weight, graphDraftKey }) {
  ...
  const previousGraphDraft = useRef(graphDraftKey);
  useEffect(() => {
    if (previousGraphDraft.current !== graphDraftKey) {
      previousGraphDraft.current = graphDraftKey;
      state.invalidate('Graph draft changed. Pair prediction cleared; the last applied separation and graph weight are unchanged.');
    }
  }, [graphDraftKey]);
  ...
  return <div className="mf-ideal-pair" data-manifold-pair>
    ...
    <div className="mf-controls"><NumberField label="Proposed separation r" value={state.draft.separation} onChange={separation => state.edit({ separation })} min={0.1} max={3} step={0.1} />
    ...
    <Prediction state={state} prompt={`Compared with applied r = ${format(state.active.separation)}, will the ideal cost be lower, the same or higher?`}
      choices={[["lower", 'Lower cost'], ['same', 'Same cost'], ['higher', 'Higher cost']]} action="Apply separation"
      evaluate={(inputs, old, prediction) => {
        const before = idealPair(weight, old.separation);
        const after = idealPair(weight, inputs.separation);
        const observed = changeName(before.cost, after.cost);
        const [beforeText, afterText] = comparisonValues(before.cost, after.cost);
        return { observed, message: `${verdict(prediction === observed)}You predicted ${prediction}. Cost ${beforeText} → ${afterText}; similarity ν = ${format(after.similarity)}. ${old.separation === inputs.separation ? 'Reflection leaves separation and cost unchanged.' : `The fixed target is w = ${format(weight)}; moving ν toward it lowers this isolated cost.`}` };
      }} />
...
export function ManifoldFuzzyLab() {
  ...
  return <Investigation kind="fuzzy" title="C · Build a fuzzy connection, then inspect a pair" state={state}>
    ...
    <div className="mf-buttons">
      <button type="button" onClick={() => state.edit({ ...fuzzyInitial })}>Draft d 2 · original scales</button>
      <button type="button" onClick={() => state.edit({ rhoI: draft.distance, rhoJ: draft.distance })}>Draft both offsets at d</button>
      <button type="button" onClick={() => state.edit({ retainedI: false, retainedJ: false })}>Draft neither direction retained</button>
    </div>
    <Prediction state={state} prompt="Estimate the proposed fuzzy union w (0–1, checked within 0.005)." maximum={1} numericStep={0.001}
      action="Apply connection" evaluate={(inputs, old, prediction) => {
        const computed = umapConnection(inputs);
        // Include the stated decimal boundary despite binary floating-point dust.
        const predictionMatches = Math.abs(computed.weight - prediction) <= 0.005 + 1e-12;
        return { observed: computed.weight, message: `${verdict(predictionMatches)}You predicted ${format(prediction)}. Directed strengths ${format(computed.forward)} and ${format(computed.reverse)} give ${format(computed.forward)} + ${format(computed.reverse)} − ${format(computed.forward * computed.reverse)} = ${format(computed.weight)}. ${!inputs.retainedI && !inputs.retainedJ ? 'With neither direction retained, local scale changes cannot create this edge.' : computed.weight === 1 ? 'A strength of 1 in either direction saturates the union.' : 'Subtracting the product avoids counting shared membership twice.'}` };
      }} />
...
export function ManifoldDigitLab() {
  ...
  const audit = state.currentResult?.evaluation.audit;
  ...
  return <Investigation kind="digits" title="D · Audit an image's neighbors" state={state}
    ...
    <p>Choose an observed image by its original source-row ID, then choose a saved map and k. Predict how many input neighbors survive before revealing their identities.</p>
    ...
    <Prediction state={state} prompt={`For source ${draft.query}, how many of its ${draft.k} input neighbors will ${candidate.title} retain?`}
      maximum={draft.k} action="Reveal neighbors" evaluate={(inputs, old, prediction) => {
        const layout = layouts.find(item => item.key === inputs.map);
        const evaluated = queryNeighbors(rows, layout.coordinates, inputs.query, inputs.k);
        return { audit: evaluated, message: `${verdict(prediction === evaluated.count)}You predicted ${prediction} of ${inputs.k}. Source ${inputs.query} in ${layout.title} retains ${evaluated.count} of ${inputs.k} (${format(evaluated.retention * 100, 1)}%). ${inputs.k - evaluated.count} input neighbors are missing and ${inputs.k - evaluated.count} map neighbors are replacements. Inspect their actual pixels below.` };
      }} />
    ...
    {audit ? <>
      ...
      <Reflection key={state.currentResult.key} prompt="Pick one changed neighbor. Which pixel strokes make the replacement plausible or surprising? Compare a second map, then explain whether its global ranking describes this query." />
    </> : <p className="mf-unrevealed">Neighbor links, lists and the current query count stay hidden until you commit a prediction and reveal this draft.</p>}
    <details><summary>Inspect the selected image's 64 pixel values and distance rules</summary>
      ...
      <p>Input collection SHA-256: <code className="mf-hash">{hash}</code>. Every selectable setting is an actual stored fit. Changing the display tab preserves your committed prediction; changing query, k or map clears it.</p>
    </details>
  </Investigation>;
}
