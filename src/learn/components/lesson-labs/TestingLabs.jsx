...
export function DebugExecutionLab() {
  const [input, setInput] = useState('pair'),
  ...
  const values = {
    ...
    states = meanTrace(values, early),
    executionState = states[step];
  const reset = () => setStep(0);
  return <Investigation id="testing-execution" kicker="Follow control, not just output" title="Which readings reach the return?">
 <p className="lesson-note">Trace [18,24] with an early return and compare [18]. Follow the running total to see why a one-element test misses the bug.</p>
 <div className="nt-controls"><LabChoices label="Readings" value={input} onChange={v => {
        setInput(v);
        reset();
      }} items={[["pair", "[18, 24]"], ["single", "[18]"], ["zero", "[0, 24]"]]} /><LabChoices label="Return placement" value={String(early)} onChange={v => {
        setEarly(v === 'true');
        reset();
      }} items={[["true", "Inside the loop (bug)"], ["false", "After the loop (fixed)"]]} /></div>
...
export function TestDiscriminationLab() {
  const [selected, setSelected] = useState(['difference']);
  const rows = testMatrix(selected);
  return <Investigation id="testing-discrimination" kicker="Test the tests" title="Which bugs can your evidence distinguish?">
 <p className="lesson-note">Compare invariant checks with exact examples for Celsius-to-Fahrenheit conversion. Find which check detects a missing offset.</p><fieldset className="rel-checks"><legend>Enable independently justified checks</legend>{temperatureCases.map(c => <label key={c.id}><input type="checkbox" checked={selected.includes(c.id)} onChange={() => setSelected(selected.includes(c.id) ? selected.filter(v => v !== c.id) : [...selected, c.id])} />{c.label}</label>)}</fieldset>
...
export function DependencyConstraintsLab() {
  const [modern, setModern] = useState(false),
    compatibility = dependencyModel(modern);
  return <Investigation id="testing-dependencies" kicker="Find a compatible intersection" title="Why can installing two packages fail?">
 <p className="lesson-note">Change the package requirements and inspect their version intersection. Separate a compatible environment from individually valid packages.</p><div className="nt-controls"><LabChoices label="Reader requirement" value={String(modern)} onChange={v => setModern(v === 'true')} items={[["false", "Reader old: Core ≥1, <2"], ["true", "Reader newer: Core ≥3, <5"]]} /></div>
