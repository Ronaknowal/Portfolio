...
import { irisRows, irisSpecies, irisSpeciesNames, irisFeatures } from '../../data/clustering-evaluation-data';
...
export function Investigation({ title, question, children, onReset }) {
  const id = useId();
  return <section className="ce-investigation" aria-labelledby={id} data-live-exploration>
...
export function CeSilhouetteLab() {
  ...
  return <Investigation title="Change a membership, then explain every moving bar" question="Six locations on a line, two groups. Edit one coordinate or move one observation to another group, and watch the selected point and overall mean respond immediately. Every bar is recomputed, not only the edited one." onReset={() => { setDraft(initial);  setSelected(2);   }}>
    <div className="ce-controls is-compact">
      {sixNames.map((name, index) => <Field key={name} label={`${name} location`} invalid={!Number.isFinite(draft.coordinates[index]) || Math.abs(draft.coordinates[index]) > 20}><input type="number" step="0.5" min="-20" max="20" value={draft.coordinates[index]} onChange={event => { const raw = event.target.value.trim(); if (raw === '' || raw === '-') return; const value = Number(raw); if (!Number.isFinite(value) || Math.abs(value) > 20) return; setDraft({ ...draft, coordinates: draft.coordinates.map((old, i) => i === index ? value : old) }); }} /></Field>)}
    </div>
    <div className="ce-controls is-compact">
...
export function CeChanceLab() {
  ...
  return <Investigation title="A finite chance experiment, not a generic random slider" question="Two binary labelings of eight observations. Edit either labeling and compare observed AMI with the full fixed-margin chance distribution. The experiment enumerates every assignment of V's labels with U held fixed; nothing is sampled." onReset={() => { setDraft(initial);    }}>
    ...
    <div className="ce-buttons"><button type="button" onClick={() => setDraft({ ...draft, v: [...draft.u] })}>Match U exactly</button><button type="button" onClick={() => setDraft({ ...draft, v: [0, 0, 0, 1, 1, 1, 1, 1] })}>Unbalanced 3/5 candidate</button><button type="button" onClick={() => setDraft({ ...draft, v: [0, 0, 0, 0, 0, 0, 0, 0] })}>Constant candidate</button></div>
    ...
    <div className="ce-matrix" style={{ gridTemplateColumns: `repeat(${info.table.columns.length + 2}, auto)` }} role="table" aria-label="Contingency table for the applied binary labelings">
      ...
      {info.table.rows.map((row, r) => <span key={r} style={{ display: 'contents' }}><span className="is-head">U{row}</span>{info.table.cells[r].map((ids, c) => <span key={c} className={r === 0 && info.table.columns[c] === Math.min(...info.table.columns) ? 'is-highlight' : undefined} aria-label={`U${row} and V${info.table.columns[c]}: ${ids.length} observations`}>{ids.map(i => ids8[i]).join('') || '·'} · {ids.length}</span>)}<span>{info.table.rowSizes[r]}</span></span>)}
...
export function CeIrisLab() {
  ...
  const appliedWeights = draftWeights;
  ...
  return <Investigation title="Real specimens: geometry and reference agreement" question="Compare the eight declared fits of the 150 irises, or freeze the standardized three-group partition and change the weight of one measurement: the silhouette bars move while ARI and AMI cannot, because the memberships have not changed." onReset={() => { setMode('compare'); setRepresentation('raw4'); setK(2); setSpecimen(0); setDraftWeights([1, 1, 1, 1]);    }}>
    ...
    {mode === 'rescore' && <>
      <div className="ce-controls is-compact">{irisFeatures.map((feature, index) => <Field key={feature} label={`${feature.replace('_cm', '').replace(/_/g, ' ')} weight`} invalid={!Number.isFinite(draftWeights[index]) || draftWeights[index] < 0.25 || draftWeights[index] > 4}><input type="number" min="0.25" max="4" step="0.25" value={draftWeights[index]} onChange={event => { const raw = event.target.value.trim(); if (raw === '' || !Number.isFinite(Number(raw)) || Number(raw) < 0.25 || Number(raw) > 4) return; setDraftWeights(draftWeights.map((old, i) => i === index ? Number(raw) : old)); }} /></Field>)}</div>
...
export function CeResampleLab() {
 ...
 const setDraft = next => { setDraftState(next); if(next.locations.every((value,index)=>Number.isFinite(value)&&Math.abs(value)<=40&&(index===0||value>next.locations[index-1]))) setApplied(next); };
  ...
  return <Investigation title="Resampling moves the representatives; the probes stay fixed" question="Six fixed probe locations. Two fits use the same locations with different multiplicities, solved exactly by the best contiguous split. Change multiplicities and watch the representatives, probe memberships and agreement move together." onReset={() => { setDraft(initial);  setPair([2, 3]);   }}>
    ...
    <p className="ce-caption">Emphasizing A and B three times pulls the left center to 0.5 and moves the boundary so that C and D join the right group; emphasizing E and F does the opposite, and the two fits then agree on only some pairs, with ARI −1/14. With k = 1 any multiplicities give one group each and ARI 1: a trivially stable answer. Tripling every location triples the centers and multiplies the cost by nine, without changing membership. In a multiplicity experiment the probes stay fixed while the representatives move; editing the shared locations changes both fits’ probe coordinates together.</p>
  </Investigation>;
}
