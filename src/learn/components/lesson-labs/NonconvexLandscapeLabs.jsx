...
export function WellBasinLab() {
  ...
  return <section className="landscape-lab" aria-label="Competing wells investigation" data-lab="wells" data-state={JSON.stringify({
    ...
    <p>Predict where a start at x=0.8 will go. The two minima belong to the same objective; the green rings mark both. Positive tilt lowers the left well. Inputs reset the trace immediately.</p>
...
export function StationaryGeometryLab() {
  ...
  return <section className="landscape-lab" aria-label="Stationary geometry investigation" data-lab="stationary" data-state={JSON.stringify(state)}>
    ...
    <p>Every selected function has gradient zero at the origin. Predict whether the chosen direction rises or falls before reading its exact change.</p>
...
export function SaddleNoiseLab() {
  ...
  return <section className="landscape-lab" aria-label="Saddle noise investigation" data-lab="noise" data-state={JSON.stringify({
    ...
    <p>The surface is F=x²−y². Start at (0.6,0); mini-batch gradient noise has a selectable direction. Predict whether the trajectory can leave the x-axis.</p>
    ...
    <p className="landscape-transfer">Turn noise off and set initial y=0.001. Predict its sign and growth. Then return y to zero and add only x noise. Is “the gradients are noisy” sufficient evidence for escape?</p>
  </section>;
}
export function ReparameterizationLab() {
  ...
  return <section className="landscape-lab" aria-label="Equivalent predictor curvature investigation" data-lab="symmetry" data-state={JSON.stringify(state)}>
    ...
    <p>For ŷ=abx and F=½(ab−1)², every starting pair (a,b)=(s,1/s) predicts ŷ=x. Predict whether increasing s changes the loss curvature even though it changes no prediction.</p>
