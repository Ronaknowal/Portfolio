...
function ArrayGrid({
  ...
  return <figure className="numpy-array">
    ...
    <div className="numpy-array__grid" style={{
      '--numpy-columns': cols
    }}>
      {values.map((value, index) => {
        ...
        return onSelect ? <button type="button" key={index} aria-label={labelText} aria-pressed={selected.includes(index)} className={selected.includes(index) ? 'is-selected' : ''} onClick={() => onSelect(index)}>{contents}</button> : <div key={index} aria-label={labelText} className={selected.includes(index) ? 'is-selected' : ''}>{contents}</div>;
      })}
    </div>
  </figure>;
}
...
export function NumpySelectionLab() {
  ...
  return <Lab name="selection" eyebrow="COORDINATES · FOLLOW A VALUE" title="Which cells survive this selection?">
    <p>Change the selection and watch the output values and shape update. Select an output cell to trace its original coordinate. Compare the two column selections: the numbers match, but one keeps an extra axis.</p>
    ...
    <div className="numpy-linked-arrays"><ArrayGrid values={sensorValues} shape={[3, 2]} label="Original X" sensorLabels selected={[result.source[cell]]} />
      <span className="numpy-flow-arrow" aria-hidden="true">→</span>
      <ArrayGrid values={result.values} shape={result.shape} label="Selection result" selected={[cell]} onSelect={setCell} />
    </div>
    <div className="numpy-feedback" role="status"><><p>{result.why}</p><p><strong>Selected link:</strong> result {shapeText(coordinates(result.shape)[cell])} ← X[{Math.floor(result.source[cell] / 2)}, {result.source[cell] % 2}] = {result.values[cell]} °C.</p></></div>
    <div className="numpy-actions"><button type="button" onClick={() => choose('column')}>Reset selection lab</button></div>
...
export function NumpyMemoryLab() {
  ...
  return <Lab name="memory" eyebrow="VIEWS & COPIES · FOLLOW THE STORAGE" title="Does writing through picked change X?">
    <p>Start with a fresh X each time. Write through the selected sensor reading and compare which buffer changes. The labels A and B identify separate data buffers, not Python variable names.</p>
    <div className="lesson-controls"><label>Selection storage<select value={kind} onChange={e => choose(e.target.value)}>{memoryCases.map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><label>Write to selected position<select value={index} onChange={e => {
          setIndex(Number(e.target.value));
          setWritten(false);
        }}><option value={0}>picked[0]</option><option value={1}>picked[1]</option><option value={2}>picked[2]</option></select></label></div>
    <div className="numpy-code-line"><code>{state.code}<br />{`picked[${index}] = 99`}</code></div>
    <div className="numpy-linked-arrays"><ArrayGrid values={state.original} shape={[3, 2]} label="X → buffer A" selected={written && state.shares ? [index * 2] : []} sourceLabels={[0, 8, 16, 24, 32, 40].map(offset => `A + ${offset} bytes`)} /><span className="numpy-flow-arrow" aria-hidden="true" style={state.shares ? undefined : {
        transform: 'none'
      }}>{state.shares ? '↔' : '≠'}</span><ArrayGrid values={state.picked} shape={[3]} label={`picked → buffer ${state.buffer}`} selected={written ? [index] : []} sourceLabels={state.offsets.map(offset => `${state.buffer} + ${offset} bytes`)} /></div>
    ...
    <div className="numpy-actions"><button type="button" disabled={written} onClick={() => setWritten(true)}>Write 99 through picked</button><button type="button" onClick={() => setWritten(false)}>Undo write</button><button type="button" onClick={() => {
        setKind('view');
        setIndex(1);
        setWritten(false);
      }}>Reset memory lab</button></div>
...
export function NumpyBroadcastLab() {
  ...
  return <Lab name="broadcast" eyebrow="BROADCASTING · ONE RESULT, TWO INPUTS" title="Which offset reaches this exact reading?">
    <p>Change the operand shapes and inspect the resulting array or shape error immediately. Select result cells to follow both inputs. The highlighted operands and equation show exactly which values are reused.</p>
    ...
    <div className="numpy-operand-arrays"><ArrayGrid values={result.a} shape={result.aShape} label="Left operand" selected={active ? [active.aIndex] : []} /><span className="numpy-flow-arrow" aria-hidden="true">−</span><ArrayGrid values={result.b} shape={result.bShape} label="Right operand" selected={active ? [active.bIndex] : []} /></div>
    {result.valid && <div className="numpy-output"><ArrayGrid values={result.values} shape={result.shape} label="Broadcast result" selected={[cell]} onSelect={setCell} /><p className="numpy-equation">{result.a[active.aIndex]} − {result.b[active.bIndex]} = <strong>{active.value}</strong></p></div>}
    <div className="numpy-feedback" role="status">{result.valid ? <><p><strong>Output shape {shapeText(result.shape)}.</strong> {result.meaning}</p><p>Output {shapeText(active.coordinate)} uses left {shapeText(active.aCoordinate)} and right {shapeText(active.bCoordinate)}. A length-one input axis always uses coordinate 0; values are reused along the expanded output axis.</p></> : <p><strong>ValueError: no compatible broadcast.</strong> At aligned axis {result.mismatchAxis}, {result.aAligned[result.mismatchAxis]} and {result.bAligned[result.mismatchAxis]} differ and neither is 1. {result.meaning} Add the intended singleton sensor axis with <code>[:, None]</code>; do not tile or delete data to force a fit.</p>}</div>
    <div className="numpy-actions"><button type="button" onClick={() => choose('sensor')}>Reset broadcast lab</button></div>
    <details><summary>Transfer: the operation works, but is it the one you intended?</summary><p>Choose the square-result case. Select a diagonal cell and then one below it to compare their input operands. Explain why there are nine results when v has only three values.</p><details><summary>Explain the result</summary><p>Shapes (3, 1) and (3,) align as (3, 1) and (1, 3). Both singleton axes expand, producing (3, 3). The diagonal subtracts each reading from itself (zero); below it, a later warmer reading minus an earlier one is positive. For three self-differences, use <code>v - v</code>.</p></details></details>
...
export function NumpyReductionLab() {
  ...
  return <Lab name="reduction" eyebrow="REDUCTIONS · FOLLOW THE CONTRIBUTING CELLS" title="Which values contribute to this mean?">
    <p>Change the reduction axis or keepdims setting and watch the means and shape update. Select a mean to see its contributing readings. Here all readings are temperatures in °C, so both a per-sensor mean and a per-time mean have an interpretable unit.</p>
    <div className="lesson-controls"><label>Axis to average over<select value={axis} onChange={e => change(Number(e.target.value))}><option value={0}>axis=0 · combine times</option><option value={1}>axis=1 · combine sensors</option></select></label><label>Keep the reduced axis<select value={String(keepdims)} onChange={e => {
          setKeepdims(e.target.value === 'true');
        }}><option value="false">keepdims=False</option><option value="true">keepdims=True</option></select></label></div>
    <div className="numpy-code-line"><code>{`X.mean(axis=${axis}, keepdims=${keepdims ? 'True' : 'False'})`}</code></div>
    <div className="numpy-linked-arrays"><ArrayGrid values={sensorValues} shape={[3, 2]} label="Input readings" sensorLabels selected={result.groups[cell]} /><span className="numpy-flow-arrow" aria-hidden="true">→</span><ArrayGrid values={result.values} shape={result.shape} label="Mean result" selected={[cell]} onSelect={setCell} /></div>
    <div className="numpy-feedback" role="status"><><p className="numpy-equation">({result.groups[cell].map(i => sensorValues[i]).join(' + ')}) ÷ {result.groups[cell].length} = <strong>{result.values[cell]} °C</strong></p><p>{axis === 0 ? 'Time varies within each selected group; the sensor coordinate stays fixed. One result survives per sensor.' : 'Sensor varies within each selected group; the time coordinate stays fixed. One result survives per time.'} {keepdims ? `The reduced axis remains with length 1: ${shapeText(result.shape)}.` : `The reduced axis is removed: ${shapeText(result.shape)}.`}</p></></div>
    <div className="numpy-actions"><button type="button" onClick={() => {
        setAxis(0);
        setKeepdims(false);
        setCell(0);
      }}>Reset reduction lab</button></div>
