...
export function OscillatorLab() {
  ...
  return <section className="ode-lab" aria-label="Position velocity and energy investigation"><h3>One state, three views of the same motion</h3><p>Keep m = 1 kg, k = 4 N/m and q(0) = 1 m fixed. Predict what an initial velocity changes that an initial position cannot specify alone.</p><div className="ode-controls"><Choice label="Damping c" value={damping} options={[[0, '0: undamped'], [2, '2: underdamped'], [4, '4: critical'], [6, '6: overdamped']]} onChange={value => setDamping(Number(value))} /><Choice label="Initial velocity" value={velocity} options={[[-2, '−2 m/s'], [0, '0 m/s'], [2, '+2 m/s']]} onChange={value => setVelocity(Number(value))} /><Slider label="Motion inspection time" value={time} max={6} onChange={setTime} /><button onClick={() => {
        setDamping(2);
        setVelocity(0);
        setTime(1);
...
export function FundamentalMatrixLab() {
  ...
  return <section className="ode-lab" aria-label="Fundamental matrix columns investigation"><h3>Build a solution from evolved initial columns</h3><p>Each column of eᵗᴬ is the result of starting from one coordinate vector. Predict the result for e₁ + e₂ before selecting it.</p><div className="ode-controls"><Choice label="Linear system" value={preset} options={Object.entries(odeModePresets).map(([key, value]) => [key, value.label])} onChange={setPreset} /><Choice label="Initial vector" value={initial} options={[["first", "e₁ = [1, 0]"], ["second", "e₂ = [0, 1]"], ["sum", "e₁ + e₂ = [1, 1]"]]} onChange={setInitial} /><Slider label="Matrix inspection time" value={time} max={3} onChange={setTime} /><button onClick={() => {
        setPreset('jordan');
        setInitial('second');
        setTime(1);
