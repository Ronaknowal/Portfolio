...
export function DecoratorOrderLab() {
  const [outer,setOuter]=useState('cap'),[value,setValue]=useState(8),[step,setStep]=useState(0),trace=decoratorOrderTrace(outer,value),wrapperState=trace[step];
  return <Investigation id="decorator-order" kicker="CALL INWARD · RETURN OUTWARD" title="Does swapping two wrappers change the answer?">
    <p>reading returns its input. double multiplies the returned result by 2. cap limits the returned result to at most 10. Neither changes the arguments on the way in.</p><Predict>For input 8, compare cap(double(reading))(8) with double(cap(reading))(8). Which returns 10, and which returns 16?</Predict>
    <div className="nt-controls"><label>Outer decorator<select value={outer} onChange={e=>{setOuter(e.target.value);setStep(0)}}><option value="cap">cap outside double</option><option value="double">double outside cap</option></select></label><label>Reading value<select value={value} onChange={e=>{setValue(Number(e.target.value));setStep(0)}}>{[3,8,12].map(v=><option key={v}>{v}</option>)}</select></label></div>
...
export function ContextLifetimeLab() {
  const [path,setPath]=useState('body-fails'),[suppress,setSuppress]=useState(false),[step,setStep]=useState(0),trace=contextTrace(path,suppress),contextState=trace[step];
  return <Investigation id="context-lifetime" kicker="ACQUIRE → USE → RELEASE" title="Which path still closes the resource?">
    <p>The manager opens an in-memory text file only after successful entry. Its exit method closes that file; choose whether a body error is suppressed. The failure preset raises before acquiring anything during enter.</p><Predict>Does exit run after a failed body? After a failed enter? If the body error is suppressed, where does execution continue?</Predict>
    <div className="nt-controls"><label>Execution path<select value={path} onChange={e=>{setPath(e.target.value);setStep(0)}}><option value="success">Body succeeds</option><option value="body-fails">Body raises ValueError</option><option value="enter-fails">Enter raises ValueError</option></select></label><label>Exit returns<select value={String(suppress)} onChange={e=>{setSuppress(e.target.value==='true');setStep(0)}}><option value="false">False · preserve body error</option><option value="true">True · suppress body error</option></select></label></div>
...
export function ExitStackLab() {
  const [fail,setFail]=useState('C'),[step,setStep]=useState(0),trace=exitStackTrace(fail),cleanupState=trace[step];
  return <Investigation id="context-exit-stack" kicker="REGISTER SUCCESS · UNWIND IN REVERSE" title="What closes if the third acquisition fails?">
    <p>Acquire resources A, B and C in that order. Each successful entry adds one exit action. The top displayed action will run first when leaving the block.</p><Predict>If B fails before acquisition, should B be released? What already-open resource must still be released?</Predict>
    <div className="nt-controls"><label>Acquisition failure<select value={fail} onChange={e=>{setFail(e.target.value);setStep(0)}}>{['A','B','C'].map(v=><option key={v}>{v}</option>)}<option value="none">No failure</option></select></label></div>
