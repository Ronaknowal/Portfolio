...
import { MARKOV_PRESETS, finiteProcessLaw, markovState, absorptionState, arrivalState,
  integratedRate, jumpClockState, brownianState, bridgeState, parseProcessNumber } from '../../data/stochastic-processes-models.js';
...
function Lab({ name, title, children }) {
...
function Figure({ title, height = 220, width = 320, children, wide = false }) {
  const id = useId();
  return <div className={'process-figure-scroll' + (wide ? ' process-wide' : '')}
    tabIndex={wide ? 0 : undefined} role={wide ? 'region' : undefined} aria-label={wide ? title : undefined}>
    <svg viewBox={'0 0 ' + width + ' ' + height} className="process-figure"
      style={wide ? { minWidth: width } : undefined} role="img" aria-labelledby={id}>
...
function Table({ caption, headers, rows }) {
  return <div className="process-table-scroll" role="region" aria-label={caption} tabIndex={0}>
    <table><caption>{caption}</caption><thead><tr>{headers.map(h => <th scope="col" key={h}>{h}</th>)}</tr></thead>
      <tbody>{rows.map((row, i) => <tr key={i}>{row.map((cell, j) => j === 0 ?
        <th key={j} scope="row">{cell}</th> : <td key={j}>{cell}</td>)}</tr>)}</tbody></table>
  </div>;
}
...
function Plot({ title, points, xmax, ymin = 0, ymax = 1, axis = 'Step', children }) {
  ...
  return <Figure title={title} height={235}>
    ...
    {[0, xmax / 2, xmax].map(value =>
      <text key={value} x={x(value)} y="196" textAnchor="middle">{number(value, 2)}</text>)}
    <text x="170" y="227" textAnchor="middle">{axis}</text>
    {points && <polyline points={points.map(point => x(point[0]) + ',' + y(point[1])).join(' ')}
...
export function ProcessSliceFigure() {
  const families = [['fresh', 'Redraw each time', [[0, 1, 1, 0], [1, 0, 0, 1]]],
    ['frozen', 'Copy one initial draw', [[0, 0, 0, 0], [1, 1, 1, 1]]],
    ['alternating', 'Alternate after a fair start', [[0, 1, 0, 1], [1, 0, 1, 0]]]];
    ...
    <div className="process-path-families">{families.map(([kind, title, paths]) => {
      ...
      return <div key={kind}><h4>{title}</h4>
        ...
        {paths.map((path, index) => <div className="process-symbol-row" key={index}>
          {path.map((value, time) => <span className={time === 2 ? 'process-selected' : ''}
            key={time} aria-label={'Time ' + time + ', value ' + value}>{value}</span>)}
        </div>)}
...
export function StateAggregationFigure() {
  return <figure className="process-inline">
    <Figure title="A deterministic hidden cycle A to B to C to A; reported A and B both become zero." height={245}>
      ...
      <text x="160" y="49" textAnchor="middle">← return to A</text>
      {[['0', 50], ['0', 160], ['1', 270]].map(([value, x]) =>
        <text x={x} y="155" textAnchor="middle" key={x}>report {value}</text>)}
    ...
    <figcaption>Reporting only 0 or 1 loses which zero-state you occupy. The previous report restores information that the current report alone lacks.</figcaption>
  </figure>;
}
...
export function MarkovPropagationLab() {
  ...
  const [config, setConfig] = useState({ a: 0.2, b: 0.3, initialSunny: 1 });
  const [step, setStep] = useState(1);
  const [selectedFlow, setSelectedFlow] = useState(0);
  const [error, setError] = useState('');
  const result = markovState({ ...config, steps: 60 });
  ...
  function preset(key) {
    ...
    setConfig({ a: entry.a, b: entry.b, initialSunny: 1 });
    setDraftA(String(entry.a)); setDraftB(String(entry.b)); setStep(1); setError('');
  }
  function apply(event) {
    event.preventDefault();
    try {
      const next = { ...config, a: parseProcessNumber(draftA, 0, 1, 'Sunny to Rainy'),
        b: parseProcessNumber(draftB, 0, 1, 'Rainy to Sunny') };
      markovState(next); setConfig(next); setStep(1); setError('');
  ...
  return <Lab name="Markov probability propagation" title="Move probability, then ask whether it settles">
    ...
    <div className="process-controls">
      ...
      <Select label="Initial sunny probability" value={config.initialSunny}
        onChange={value => { setConfig({ ...config, initialSunny: Number(value) }); setStep(1); }}>
    ...
    <div className="process-pair">
      ...
      <div>
        <Table caption="Applied transition matrix: row now, column next"
          headers={['From / to', 'Sunny', 'Rainy']} rows={result.matrix.map((row, i) => [names[i], ...row.map(value => number(value))])} />
        <p>Choose a contribution to inspect:</p>
        <div className="process-flow-options">{[0, 1, 2, 3].map(index =>
          <button key={index} aria-pressed={index === selectedFlow} onClick={() => setSelectedFlow(index)}>
            {names[Math.floor(index / 2)]} → {names[index % 2]}
          </button>)}</div>
      </div>
    </div>
    {step === 0 ? <p>At time zero this is the chosen initial law. No transition has happened.</p> :
      <div className="process-mass-flow">
        ...
        {current.flows.flatMap((row, i) => row.map((mass, j) =>
          <div className={i * 2 + j === selectedFlow ? 'process-flow-row process-active' : 'process-flow-row'} key={i + '-' + j}>
            <span>{names[i][0]} → {names[j][0]}</span><div><i style={{ width: mass * 100 + '%' }} /></div><strong>{number(mass)}</strong>
          </div>))}
    ...
    <Plot title="Calculated sunny probability for steps zero to sixty" xmax={60}
      points={result.history.map(state => [state.step, state.distribution[0]])}>
      {({ x, y }) => <>
    ...
    <p>{result.stationary ? 'The dashed stationary sunny probability is ' + number(result.stationary[0]) + '.'
      : 'Every distribution is stationary for the identity matrix. There is no unique dashed target.'}
      {' '}{result.periodTwo ? 'This chain is irreducible but periodic: a nonstationary start alternates forever.'
        : result.a + result.b === 0 ? 'The initial law stays exactly where it began.'
          : 'The plotted law converges to its stationary distribution.'}</p>
...
export function AbsorptionLab() {
  ...
  const result = absorptionState({ boundary, start, upward, steps: 60 });
  const current = result.history[step];
  const width = (boundary + 1) * 52 + 20;
  function reset() { setBoundary(4); setStart(2); setUpward(0.5); setStep(4); setSelected(2); }
  return <Lab name="First passage and absorption" title="Watch moving probability reach its first boundary">
    ...
    <div className="process-controls">
      <Select label="Upper reserve boundary" value={boundary} onChange={value => {
        const next = Number(value); setBoundary(next); setStart(Math.min(start, next)); setSelected(Math.min(selected, next));
      ...
      <Select label="Starting reserve" value={start} onChange={value => setStart(Number(value))}>
        {Array.from({ length: boundary + 1 }, (_, i) => <option key={i}>{i}</option>)}
      </Select>
    ...
    <Figure title={'Reserve probability at step ' + step + '. Boundaries zero and ' + boundary + ' absorb.'}
      width={width} height={170} wide={width > 310}>
      {Array.from({ length: boundary + 1 }, (_, state) => {
        const x = 36 + state * 52;
        return <g key={state}>
          ...
          <rect x={x - 15} y={120 - 40 * current.distribution[state]} width="30"
            height={40 * current.distribution[state]} className="process-bar" />
    ...
    <div className="process-mass-strip" aria-label="Absorbed lower, still moving, absorbed upper">
      <span style={{ flex: current.distribution[0] }} className="process-lower" />
      <span style={{ flex: current.surviving }} className="process-surviving" />
      <span style={{ flex: current.distribution[boundary] }} className="process-upper" />
    </div>
    ...
    <Plot title="Surviving transient probability versus step, calculated from the finite chain"
      xmax={60} points={result.history.map(state => [state.step, state.surviving])} />
    <div className="process-controls">
      <Select label="Inspect first-step equations at reserve" value={selected} onChange={value => setSelected(Number(value))}>
        {Array.from({ length: boundary + 1 }, (_, i) => <option key={i}>{i}</option>)}
      </Select>
    </div>
    {selected === 0 || selected === boundary ? <p>This is already a boundary: time to either boundary is 0; upper-boundary success is {selected === boundary ? 1 : 0}.</p> :
      <div className="process-equation-lines">
    ...
    <details><summary>Inspect exact-model probabilities and expected transient visits</summary>
      <Table caption="Solved first-step system" headers={['Reserve', 'Upper success h', 'Mean steps t']}
        rows={result.success.map((h, i) => [i, number(h, 6), number(result.meanSteps[i], 6)])} />
      <Table caption="Fundamental matrix: expected visits before absorption, including time zero"
        headers={['Start / visit', ...Array.from({ length: boundary - 1 }, (_, i) => String(i + 1))]}
        rows={result.visits.map((row, i) => [i + 1, ...row.map(value => number(value))])} />
      <Table caption="First-hit probability and survival at every displayed step" headers={['Step', 'First lower', 'First upper', 'Surviving']}
        rows={result.history.slice(0, step + 1).map(state => [state.step, number(state.firstLower), number(state.firstUpper), number(state.surviving)])} />
    </details>
  </Lab>;
}
...
export function PoissonArrivalLab() {
  ...
  const result = arrivalState({ rates, interval, seed, routing, splitProbability: split });
  ...
  function apply(event) {
    ...
    try {
      ...
      arrivalState({ rates: next });
      setRates(next); setSelected(0); setError('');
    } catch (issue) { setError(issue.message); }
  }
  function reset() {
    setRates([2.5, 2.5]); setDraft(['2.5', '2.5']); setWindow('0,1');
    setRouting('independent'); setSplit(0.5); setSeed(11); setSelected(0); setError('');
  }
  const staircase = [[0, 0]];
  ...
  return <Lab name="Arrival clocks and event counts" title="One event stream, several connected views">
    ...
    <div className="process-controls">
      <Select label="Arrival rate preset" value="" onChange={value => {
        ...
        setRates(next); setDraft(next.map(String)); setSelected(0); setError('');
      }}>
    ...
    <form onSubmit={apply} className="process-controls">
      {draft.map((value, index) => <label key={index}>{index === 0 ? 'Rate before minute 2' : 'Rate after minute 2'}
        <input type="text" inputMode="decimal" value={value} onChange={event =>
          setDraft(draft.map((old, i) => i === index ? event.target.value : old))} />
      </label>)}
    ...
    <div className="process-controls">
      ...
      <Select label="Independent probability of A" value={split} disabled={routing !== 'independent'}
        onChange={value => setSplit(Number(value))}>
        {[0, 0.25, 0.5, 0.75, 1].map(value => <option key={value}>{value}</option>)}
      </Select>
      <button onClick={() => { setSeed(seed + 1); setSelected(0); }}>Draw another event stream</button>
    ...
    <Figure title="Actual event times on A and B routing lanes; highlighted interval is left open, right closed" height={240}>
      ...
      {result.events.map((arrival, index) => <g key={index}>
        <line x1={x(arrival.time)} x2={x(arrival.time)} y1={55 + arrival.mark * 70}
          y2={85 + arrival.mark * 70} className={arrival === event ? 'process-event-selected' : 'process-event'} />
      </g>)}
    ...
    <div className="process-controls">
      <Select label="Inspect arrival" value={event ? result.events.indexOf(event) : ''} disabled={!event}
    ...
    <div className="process-plot-pair">
      <div><h4>Count the jumps</h4>
        <Plot title="Right-continuous cumulative event count on this realized path" points={staircase}
          xmax={3} ymax={Math.max(1, result.events.length)} axis="Time (minutes)" />
      ...
      <div><h4>Accumulate the rate</h4>
        <Plot title="Exact integrated piecewise constant rate; slope changes at minute 2"
          points={[0, 2, 3].map(time => [time, integratedRate(time, rates, 2)])}
          xmax={3} ymax={Math.max(1, result.totalIntensity)} axis="Time (minutes)" />
    ...
    <p>{routing === 'independent' ? 'Independent marking gives independent Poisson output processes in this model; conditioning on their total makes their counts dependent.' :
      'Alternating marks are a counterexample: the labels depend on event order. Successive A waits add two original waits, so the thinned streams are not Poisson.'}</p>
    ...
    <details><summary>Inspect the events and count distribution</summary>
      ...
      <Table caption="Realized arrivals" headers={['Event', 'Minute', 'Gap', 'Λ(time)', 'Route']}
        rows={result.events.map(arrival => [arrival.index, number(arrival.time), number(arrival.gap),
          number(arrival.unitTime), arrival.mark ? 'B' : 'A'])} />
      <Table caption="Theoretical count probabilities for the selected interval" headers={['Count', 'Probability']}
        rows={result.pmf.map((value, count) => [count, number(value, 6)])} />
...
export function JumpClockLab() {
  ...
  const [seed, setSeed] = useState(11);
  const [error, setError] = useState('');
  const state = jumpClockState({ alpha: rates[0], beta: rates[1], horizon, initial, seed });
  ...
  return <Lab name="Continuous time holding clocks" title="Count visits, then measure how long they last">
    ...
    <form className="process-controls" onSubmit={event => {
      ...
      try {
        ...
        setRates(next); setError('');
      } catch (issue) { setError(issue.message); }
    }}>
      {draft.map((value, i) => <label key={i}>{i ? 'Off → On rate β per hour' : 'On → Off rate α per hour'}
        <input inputMode="decimal" value={value} onChange={event =>
          setDraft(draft.map((old, j) => i === j ? event.target.value : old))} />
      </label>)}
    ...
    <div className="process-controls">
      ...
      <button onClick={() => {
        setRates([0.5, 2]); setDraft(['0.5', '2']); setHorizon(4);
        setInitial(0); setSeed(11); setError('');
    ...
    <Figure title="Realized On and Off holding intervals; horizontal length is time spent in that state" height={230}>
      ...
      {state.segments.map((segment, index) => <g key={index}>
        <line x1={x(segment.start)} x2={x(segment.end)} y1={70 + segment.state * 70}
          y2={70 + segment.state * 70} className={segment.state ? 'process-holding-off' : 'process-holding-on'} />
    ...
    <Table caption="One realized path versus model expectations" headers={['Quantity', 'On', 'Off']}
      rows={[
        ['Mean full holding time (hours)', number(1 / rates[0]), number(1 / rates[1])],
        ['Observed time (hours)', number(state.exposure[0]), number(state.exposure[1])],
        ['Observed departures', state.departures[0], state.departures[1]],
        ['Long-run time fraction', number(state.stationaryOn), number(1 - state.stationaryOn)],
        ['Long-run jump-visit fraction', '½', '½'],
      ]} />
    ...
    <Plot title="Exact probability that the device is On as a function of time; not the single realized path"
      points={state.history.map(row => [row.time, row.probabilityOn])} xmax={horizon} axis="Time (hours)">
      {({ x, y }) => <line x1={x(0)} x2={x(horizon)} y1={y(state.stationaryOn)}
        y2={y(state.stationaryOn)} className="process-equilibrium" />}
    </Plot>
    ...
    <details><summary>Inspect generator and holding intervals</summary>
      <Table caption="Generator: rates, not transition probabilities" headers={['From / to', 'On', 'Off']}
        rows={state.generator.map((row, i) => [i ? 'Off' : 'On', ...row.map(value => number(value))])} />
      <Table caption="Seeded holding intervals; final interval is clipped at the observation horizon"
        headers={['State', 'Start', 'Observed end', 'Full drawn hold', 'Departure seen?']}
        rows={state.segments.map(segment => [segment.state ? 'Off' : 'On', number(segment.start),
          number(segment.end), number(segment.holdingTime), segment.jumped ? 'Yes' : 'No'])} />
    </details>
  </Lab>;
}
...
export function BrownianPathLab() {
  ...
  const result = brownianState({ horizon, drift, scale, level, pathIndex, seed });
  const i = Math.min(increment, result.count - 1);
  const extent = Math.max(5 * Math.sqrt(horizon) + Math.abs(drift) * horizon,
    ...result.paths.flat().map(value => Math.abs(value)), 1.96 * scale * Math.sqrt(horizon) + Math.abs(drift) * horizon);
  ...
  return <Lab name="Brownian paths and coupled refinement" title="Refine the grid without replacing the path">
    ...
    <div className="process-controls">
      ...
      <Select label="Number of grid intervals" value={level} onChange={value => { setLevel(Number(value)); setIncrement(0); }}>
      ...
      <Select label="Highlighted Brownian path" value={pathIndex} onChange={value => setPathIndex(Number(value))}>
        {Array.from({ length: 8 }, (_, index) => <option key={index} value={index}>Path {index + 1}</option>)}
      </Select>
      <button onClick={() => setSeed(seed + 1)}>Draw another Brownian family</button>
      <button onClick={() => {
        setHorizon(1); setDrift(0); setScale(1); setLevel(4); setPathIndex(0); setSeed(11); setIncrement(0);
      }}>Reset Brownian paths</button>
    </div>
    <Plot title="Eight exact-at-grid Brownian paths with drift; highlighted interval and pointwise normal bands"
      xmax={horizon} ymin={-extent} ymax={extent} axis="Time">
      {({ x, y }) => <>
        <polygon points={[...result.times.map(time => [time, drift * time + band(time)]),
          ...[...result.times].reverse().map(time => [time, drift * time - band(time)])]
          .map(([t, value]) => x(t) + ',' + y(value)).join(' ')} className="process-band" />
        {result.paths.map((path, index) => <polyline key={index}
          points={path.map((value, j) => x(result.times[j]) + ',' + y(value)).join(' ')}
          className={index === pathIndex ? 'process-line' : 'process-ghost-path'} />)}
        <line x1={x(result.times[i])} x2={x(result.times[i + 1])} y1={y(result.selected[i])}
          y2={y(result.selected[i + 1])} className="process-highlight-increment" />
    ...
    <Table caption="Squared increments: realized value and exact finite-grid moments" headers={['Quantity', 'This path', 'Model expectation']}
      rows={[
        ['Σ(ΔX)²', number(result.rawVariation), number(result.rawVariationExpectation)],
        ['Σ(ΔX−μΔt)²', number(result.centeredVariation), number(result.centeredVariationExpectation)],
      ]} />
    ...
    <details><summary>Inspect covariance and grid values</summary>
      <Table caption="Exact covariance σ² min(s,t), unaffected by drift" headers={['s / t', ...result.covarianceTimes.map(time => number(time))]}
        rows={result.covariance.map((row, index) => [number(result.covarianceTimes[index]), ...row.map(value => number(value))])} />
      <Table caption="Selected grid path and its increments" headers={['Time', 'X(t)', 'Increment ending here']}
        rows={result.times.map((time, index) => [number(time), number(result.selected[index]), index ? number(result.increments[index - 1]) : 'Start'])} />
...
export function BrownianBridgeFigure() {
  ...
  return <figure className="process-inline">
    <Figure title="Conditional Brownian bridge with endpoints zero and a barrier at one; crossing remains possible between observations" height={250}>
      ...
      <line x1="160" x2="160" y1="105" y2="205" className="process-bridge-variance" />
