...
function StreamRoute({ stream, destination, text, revealed, pipe = false }) {
  return <div className={`linux-stream-route linux-stream-route--${stream}`}>
    ...
    <FlowArrow label={revealed ? "goes to" : "predict"} />
    <div className="linux-stream-destination">
      ...
      </> : <><span className="linux-stream-small">Destination</span><strong>Where will it go?</strong><span>Trace to reveal the route.</span></>}
    </div>
    {pipe && revealed && <div className="linux-stream-pipe-result"><FlowArrow label="stdout (1)" /><div className="linux-stream-destination"><span className="linux-stream-small">On screen</span><strong>Your terminal</strong><code>1</code></div></div>}
  </div>;
}
...
export default function LinuxStreamsLab() {
  ...
  const chooseRoute = next => { setRouteId(next); setPrediction(""); setRevealed(false); };
  const reset = () => { chooseRoute("stdout-file"); setFails(false); };
  const destinations = ["terminal", "result.txt", "errors.txt", "wc -l", ...(model.advanced ? ["combined.txt"] : [])];

  return <section className="lesson-lab linux-stream-lab" aria-labelledby={`${id}-title`}>
    ...
    <fieldset className="linux-stream-presets">
      <legend>1. Choose a routing instruction</legend>
    ...
    <fieldset className="linux-stream-prediction">
      <legend>2. Predict: where will the warning go?</legend>
      <div>{destinations.map(destination => <label key={destination}><input type="radio" name={`${id}-prediction`} value={destination} checked={prediction === destination} onChange={() => { setPrediction(destination); setRevealed(false); }} /><span>{destination === "terminal" ? "Your terminal" : destination}</span></label>)}</div>
    </fieldset>
    <div className="linux-stream-actions"><button type="button" className="linux-stream-trace" onClick={() => setRevealed(true)}>3. Trace the streams</button><button type="button" onClick={reset}>Reset streams</button></div>

    <figure className="linux-stream-diagram" aria-label={revealed ? model.summary : "The program produces stdout and stderr. Their destinations are hidden until you trace the command."}>
      <div className="linux-stream-source"><strong>One program</strong><span>Two separately routable streams</span></div>
      <StreamRoute stream="stdout" text={model.stdoutText} destination={model.stdout} revealed={revealed} pipe={model.id === "pipe"} />
      <StreamRoute stream="stderr" text={model.stderrText} destination={model.stderr} revealed={revealed} />
      <figcaption>Solid gold = stdout; dashed pale blue = stderr. Arrows show connections, not elapsed time.</figcaption>
    </figure>

    <div className="linux-stream-feedback" aria-live="polite" aria-atomic="true">{revealed ? <p><strong>{prediction ? prediction === model.stderr ? "Your prediction is right. " : `The warning goes to ${model.stderr === "terminal" ? "your terminal" : model.stderr}. ` : "Follow descriptor 2. "}</strong>{model.explanation}</p> : <p>Make a prediction, then trace the command to see both routes and their contents.</p>}</div>

    {revealed && <div className="linux-stream-results">
    ...
    <details className="linux-stream-deeper">
      ...
      <p>Try “Count result lines”, then make the producer fail after printing. Predict whether the count or the reported status changes.</p>
      <label className="linux-stream-failure"><input type="checkbox" checked={fails} onChange={event => { setFails(event.target.checked); setRevealed(false); }} />Exit 7 after printing the same messages</label>
    ...
    <p className="lesson-note">Transfer: you need a machine-readable metric file and a separate warning log. Choose a route, then explain why plain <code>| wc -l</code> would leave the warning visible. Check the diagram after predicting.</p>
  </section>;
}
