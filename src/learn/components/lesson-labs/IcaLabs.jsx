...
import { MIXING, SOURCE_FAMILIES, KEEP_LABELS, ENTERED_AMPLITUDE_LIMIT, rotationModel, contributionModel, rescaleComponent } from '../../data/ica-models';
import { DataTable, EqualPlot, Investigation, NumberField, RangeField, Readout, Reflection, SelectField, format, useIcaInvestigation } from './IcaShared';
import './ica-labs.css';
...
export function IcaRotationLab() {
  ...
  return <Investigation kind="rotation" title="Investigation R1 · Rotate a distribution, not just its covariance" state={state}>
    ...
    <div className="ic-controls">
      <SelectField label="Source family" value={state.draft.family} onChange={value => state.edit({ family: value })} options={FAMILY_OPTIONS} />
      <NumberField label="Angle θ in degrees" value={state.draft.angle} min={0} max={180} step="any"
        onChange={value => state.edit({ angle: value })} />
      <RangeField label="Angle slider" value={state.draft.angle} min={0} max={180} step={0.5} onChange={value => state.edit({ angle: value })} />
    </div>
    <div className="ic-buttons">
      {[5, 30, 90].map(angle => <button key={angle} type="button" onClick={() => state.edit({ angle })}>{`Set ${angle}°`}</button>)}
      <button type="button" onClick={() => state.edit({ angle: 45 })}>Set 45°</button>
    </div>
    <p className="ic-note">Current population: <strong>{family.label}</strong> at <strong>{format(state.active.angle, 2)}°</strong>. Staged: {SOURCE_FAMILIES[state.draft.family].label} at {state.draft.angle === '' ? '—' : `${format(state.draft.angle, 2)}°`}.</p>
    ...
    <div className="ic-panels">
      ...
      <section className="ic-panel">
        ...
        <p className="ic-note">Analytic population curve, not a measured fit. Solid marker: the current angle, updated as you move the control.</p>
      </section>
    </div>
...
function ScaleMode({ base }) {
  ...
  return <Investigation kind="scale" title="Investigation C1b · Rescale a component and compensate its column" state={state}>
    ...
    <div className="ic-controls">
      <SelectField label="Component to rescale" value={state.draft.component} onChange={value => state.edit({ component: value })}
        options={[['1', 'Component 1'], ['2', 'Component 2']]} />
      <NumberField label="Scale c" value={state.draft.scale} min={-4} max={4} step="any" onChange={value => state.edit({ scale: value })} />
      <SelectField label="Operation" value={state.draft.mode} onChange={value => state.edit({ mode: value })}
        options={[['compensate', 'Scale the source and divide its column by c'], ['source', 'Scale the source only']]} />
    </div>
    <div className="ic-buttons">
      {[2, -2, 0.5].map(value => <button key={value} type="button" onClick={() => state.edit({ scale: value })}>{`Set c = ${format(value)}`}</button>)}
      <button type="button" onClick={() => state.edit({ scale: 0 })}>Set c = 0</button>
    </div>
    
    {<DataTable stack caption={`Applied state: component ${applied.index + 1} at c = ${format(state.active.scale)}, ${state.active.mode === 'compensate' ? 'with a compensated column' : 'source only'}.`} headings={['Quantity', 'before', 'after']} rows={[['Source amplitudes', `(${format(applied.before.sources[0])}, ${format(applied.before.sources[1])})`, `(${format(applied.after.sources[0])}, ${format(applied.after.sources[1])})`], [`Mixing column ${applied.index + 1}`, `(${format(MIXING[0][applied.index])}, ${format(MIXING[1][applied.index])})`, `(${format(applied.after.mixing[0][applied.index], 6)}, ${format(applied.after.mixing[1][applied.index], 6)})`], ['Contribution of that component', `(${format(applied.before.contributions[applied.index].contribution[0])}, ${format(applied.before.contributions[applied.index].contribution[1])})`, `(${format(applied.after.contributions[applied.index].contribution[0], 6)}, ${format(applied.after.contributions[applied.index].contribution[1], 6)})`], ['Reconstructed sensors', `(${format(applied.before.retained[0])}, ${format(applied.before.retained[1])})`, `(${format(applied.after.retained[0], 6)}, ${format(applied.after.retained[1], 6)})`]]} />}
...
export function IcaContributionLab() {
  ...
  return <>
    <Investigation kind="contribution" title="Investigation C1 · Edit a contribution, then remove one" state={{ ...state, reset: () => { state.reset();  } }}>
      <p>The mixing matrix stays A = [[{format(MIXING[0][0])}, {format(MIXING[0][1])}], [{format(MIXING[1][0])}, {format(MIXING[1][1])}]]. Enter your own source amplitudes and choose which components to keep. The worked example — amplitudes (1, −1), both kept, observation (1, −1) — is the starting state; change either amplitude or the keep-set to inspect its contributions.</p>
      <div className="ic-controls">
        <NumberField label="Source 1 amplitude" value={state.draft.s1} min={-4} max={4} step="any" onChange={value => state.edit({ s1: value })} />
        <NumberField label="Source 2 amplitude" value={state.draft.s2} min={-4} max={4} step="any" onChange={value => state.edit({ s2: value })} />
        <SelectField label="Keep-set" value={state.draft.keep} onChange={value => state.edit({ keep: value })}
    ...
    {<ScaleMode key={`${state.active.s1}|${state.active.s2}|${state.active.keep}`} base={{
  sources: [state.active.s1, state.active.s2],
  keep: state.active.keep
}} />}
  </>;
}
