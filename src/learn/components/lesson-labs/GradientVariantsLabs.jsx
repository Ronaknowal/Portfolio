...
function Range({
  ...
  return <Field label={`${label} · ${number(value)}`}><input aria-label={label} type="range" disabled={disabled} value={value} min={min} max={max} step={step} onChange={event => onChange(Number(event.target.value))} /></Field>;
}
function Steps({
  step,
  onChange,
...
function Plot({
  ...
  const id = useId();
  return <svg className={`optimizer-plot ${className}`} viewBox="0 0 330 280" role="img" aria-labelledby={id}><title id={id}>{title}</title>{children}</svg>;
}
...
function TimePlot({
  ...
  const shownCount = preview ? series[0].values.length : current + 1;
  const values = series.flatMap(item => item.values.slice(0, shownCount));
  ...
  return <Plot title={title}>
    ...
    {series.map(item => <g key={item.label}><path d={path(item.values.slice(0, shownCount).map((value, index) => [index, value]), x, y)} className={`optimizer-line ${item.style}`} />{item.values.slice(0, shownCount).map((value, index) => <circle key={index} cx={x(index)} cy={y(value)} r={index === current ? 4 : 2} className={`optimizer-dot ${item.style}`} />)}</g>)}
  </Plot>;
}
export function BatchGradientLab() {
  ...
  return <section className="optimizer-lab" aria-label="Sampled gradient investigation">
    ...
    <p>At θ=1 the full mean gradient is positive. Inspect the direction when only observation 3 is selected. Then select all four observations. The curve is always the same full empirical objective.</p>
...
export function MomentumTrajectoryLab() {
  ...
  return <section className="optimizer-lab" aria-label="Momentum and lookahead investigation"><p className="optimizer-eyebrow">Investigate · current slope and stored direction</p><h3>The update has a memory</h3><p>Minimize F(x,y)=(x²+c·y²)/2 from (2,1). Advance one update at a time. The amber displacement is the actual move; the dashed green displacement is what plain descent at the current point would propose with the same rate.</p>
    ...
    <Range label="Inspect trajectory update" value={index} onChange={setStep} min={0} max={state.frames.length - 1} step={1} /><Steps step={index} onChange={setStep} maximum={state.frames.length - 1} /><p role="status">Loss {number(frame.loss)} at update {index}. {state.truncated ? `The finite illustration stops before a coordinate would exceed ${state.bound}; this is divergence protection, not convergence.` : 'The table and parameter-plane path stop at the cursor; the loss preview shows the complete recomputed run.'}</p><p className="optimizer-transfer">Compare β=0 with plain descent. Then find a momentum update whose current gradient and actual displacement have the same sign in one coordinate. Why does “opposite the gradient” no longer describe that move?</p></section>;
}
export function AdaptiveHistoryLab() {
  ...
  return <section className="optimizer-lab" aria-label="Adaptive gradient history investigation"><p className="optimizer-eyebrow">Investigate · numerator, history and denominator</p><h3>The same present gradient can produce a different step</h3><p>These are supplied gradient vectors, so every method sees the same history. Inspect the second-coordinate update immediately after its one large nonzero gradient. This replay isolates arithmetic; it is not a training-loss comparison.</p><div className="optimizer-controls"><Field label="Adaptive method"><select aria-label="Adaptive method" value={method} onChange={event => change(setMethod, event.target.value)}><option value="adagrad">AdaGrad</option><option value="rmsprop">RMSProp</option><option value="adam">Adam</option></select></Field><Field label="Gradient history"><select aria-label="Gradient history" value={profile} onChange={event => change(setProfile, event.target.value)}>{Object.entries(optimizerGradientProfiles).map(([key, item]) => <option value={key} key={key}>{item.title}</option>)}</select></Field><Range label="Recent-square decay beta2" disabled={method === 'adagrad'} value={beta2} onChange={value => change(setBeta2, value)} min={0} max={0.99} step={0.01} /><label className="optimizer-check"><input type="checkbox" checked={correction} disabled={method !== 'adam'} onChange={event => change(setCorrection, event.target.checked)} />Adam bias correction</label></div>
