...
function Scatter({
  ...
  return <figure className="kh-scatter">
    ...
    <p className="kh-label-explanation">Fine gray lines connect displaced labels to unchanged markers.</p>
...
export function LloydLab() {
  ...
  return <Investigation title="Watch assignment and movement change the same objective" prediction="Will moving a center change the labels immediately, or only on the next assignment?" onReset={reset}>
    ...
    <Scatter points={points} centers={state.centers} labels={state.labels} connections title={state.labels === null ? 'Choose the first assignment to connect points to their centers' : 'The dashed segments are the current residuals'} />
...
export function SeedingLab() {
  ...
  return <Investigation title="Read a D² draw as probability, not a farthest-point rule" prediction="Can the next center be a closer row even when a farther row exists?" onReset={() => {
    ...
    <Scatter points={points} centers={state.selected === null ? [points[first]] : [points[first], points[state.selected]]} title={state.stopped ? 'C0 already represents every row; no second center is drawn' : 'C0 is fixed; C1 marks the center selected by this draw'} />
