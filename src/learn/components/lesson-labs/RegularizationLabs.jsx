...
import { Field, Investigation, NumberField, Plot, SignedBars, Table, coefficientText, curve, fixed, round, signed, useInvestigation } from './RegularizationShared.jsx';
...
export function ThresholdLab() {
  ...
  return <Investigation
    ...
    question="One normalized coefficient, one data preference z, one strength λ and one mixing fraction ρ. Move the controls to see the fitted coefficient cross the threshold or remain exactly zero."
    ...
    <p className="lesson-live-note">{`S(${round(active.z, 4)}, ${round(solution.threshold, 6)}) = ${round(solution.thresholded, 9)}, divided by ${round(solution.denominator, 6)}, gives ${coefficientText(solution.coefficient, 9)}.`}</p>

    {<>
      <div className="rg-panels is-pair">
        <div className="rg-panel">
          <h4>The objective along the coefficient axis</h4>
          <Plot caption="" width={300} height={170} domain={span} range={[0, peak]} padding={{
        left: 42,
        right: 12,
        top: 14,
        bottom: 30
        ...
        <div className="rg-panel">
          ...
          <Plot caption="" width={300} height={170} domain={zSpan} range={[-4, 4]} padding={{
        left: 42,
        right: 12,
        top: 14,
        bottom: 30
      }} valueTicks={[-4, 0, 4]} describe={`The map from the data preference z to the fitted coefficient. Between minus ${round(solution.threshold, 6)} and ${round(solution.threshold, 6)} the output is exactly zero; outside it the output is the soft-thresholded value divided by ${round(solution.denominator, 6)}. The current z is ${round(active.z, 4)} and its output is ${round(solution.coefficient, 9)}.`}>
            {(scaleX, scaleY) => <>
              {solution.threshold > 0 && <rect x={scaleX(-solution.threshold)} y={scaleY(3)} width={Math.max(0, scaleX(solution.threshold) - scaleX(-solution.threshold))} height={scaleY(-3) - scaleY(3)} fill="#1d2119" stroke="#9a8347" strokeDasharray="3 3" />}
      ...
      <p className="rg-readout" aria-live="polite">
        ...
        {solution.coefficient === 0 ? ` At w = 0 the smooth part of the derivative is ${round(zeroSlopes.smooth, 6)}, which lies inside [−${round(solution.threshold, 6)}, ${round(solution.threshold, 6)}], so zero is the minimum.` : ` At the minimum the derivative from the left is ${round(slopes.fromLeft, 6)} and from the right ${round(slopes.fromRight, 6)}.`}
      ...
      <Table caption="The same z and λ under three families. These are minima of three different objectives, and the smallest of them is not a way to choose a family." headings={['family', 'ρ', 'threshold λρ', 'denominator', 'coefficient']} rows={families.map(entry => [entry.name, entry.ratio, round(entry.solution.threshold, 6), round(entry.solution.denominator, 6), coefficientText(entry.solution.coefficient, 9)])} />
    </>}
  </Investigation>;
}
...
export function CoordinateLab() {
  ...
  return <Investigation
    ...
    question="Edit the actual rows, the targets, the strength, the mixing fraction and the coordinate order. Choose an output to inspect and step through the partial residuals that produce it; every valid edit recomputes the fit."
    ...
    <p className="lesson-live-note">{`The recorded coordinate order was ${order.map(column => `feature ${column + 1}`).join(', then ')}.`}</p>

    {<>
      <div className="rg-stage">
        <h4>1. Centre the rows on their own training means</h4>
        <Table caption="Original values, the training means subtracted from them, and the centred target" headings={['row', 'x₁', 'x₂', 'target', 'centred x₁', 'centred x₂', 'centred target']} rows={active.rows.map((entry, index) => [rowNames[index], round(entry[0], 6), round(entry[1], 6), round(entry[2], 6), round(entry[0] - fit.featureMeans[0], 6), round(entry[1] - fit.featureMeans[1], 6), round(entry[2] - fit.targetMean, 6)])} />
        <p className="rg-caption">
      ...
      <div className="rg-stage">
        ...
        <Table caption="Each coordinate's partial residual association, curvature, threshold, divisor and resulting coefficient. A constant column has no data curvature: a positive penalty still minimizes at zero; only an unpenalized zero-curvature coordinate is flat and takes a declared zero." headings={['visit', 'feature', 'association c', 'curvature a', 'threshold λρ', 'divisor a + λ(1 − ρ)', 'coefficient']} rows={record.steps.map((step, index) => [index + 1, `feature ${step.column + 1}`, round(step.association, 9), round(step.curvature, 9), round(step.threshold, 6), round(step.denominator, 9), step.flat ? 'declared 0 · flat' : coefficientText(step.weight, 9)])} />
        <Table caption="The residual after every coordinate in this sweep, one column per row" headings={['after visiting', ...active.rows.map((entry, index) => rowNames[index])]} rows={record.steps.map((step, index) => [`feature ${step.column + 1}`, ...step.residual.map(value => round(value, 9))])} />
      </div>

      <div className="rg-stage">
        <h4>3. The fit, and what it predicts</h4>
        <Table caption="Coefficients, the recovered intercept and each row's fitted value" headings={['quantity', 'value']} rows={[['coefficient 1', coefficientText(fit.weights[0], 9)], ['coefficient 2', coefficientText(fit.weights[1], 9)], ['intercept', round(fit.intercept, 9)], ['sweeps used', `${fit.sweeps} of ${fit.maxSweeps}`], ['largest optimality violation', round(fit.kktResidual, 12)], ['objective', round(fit.objective, 9)], ['data term', round(fit.data, 9)], ['penalty', round(fit.penalty, 9)], ['mean squared error of the fit', round(fit.meanSquaredError, 9)], ['Pure-lasso λ threshold for all coefficients to be zero', round(fit.lambdaMax, 9)]]} />
        <Table caption="Row by row" headings={['row', 'target', 'fitted value', 'residual']} rows={active.rows.map((entry, index) => [rowNames[index], round(entry[2], 6), round(fit.fitted[index], 9), signed(entry[2] - fit.fitted[index], 9)])} rowClass={index => output === 'row' && index === selectedRow ? 'is-selected' : undefined} />
        <p className="rg-readout" aria-live="polite">
          {fit.converged ? `Converged: the largest optimality violation is ${round(fit.kktResidual, 12)}, at or below the tolerance ${fit.tolerance}, after ${fit.sweeps} sweep${fit.sweeps === 1 ? '' : 's'}.` : `Not converged: the sweep cap of ${fit.maxSweeps} was reached with a largest optimality violation of ${round(fit.kktResidual, 12)}. The values above are the current iterate and are inspectable, but they are not an exact optimum.`}
      ...
      {fit.history.length > 1 && <Plot caption="Objective by sweep, from the actual computed history" width={340} height={170} domain={[1, fit.history.length]} range={[Math.min(...fit.history.map(entry => entry.objective)) - 0.05, Math.max(...fit.history.map(entry => entry.objective)) + 0.05]} ticks={fit.history.filter(entry => entry.sweep % Math.max(1, Math.ceil(fit.history.length / 5)) === 0 || entry.sweep === 1).map(entry => entry.sweep)} describe={`The objective after each sweep: ${fit.history.map(entry => `sweep ${entry.sweep} gives ${round(entry.objective, 9)}`).join('; ')}.`}>
        {(scaleX, scaleY) => <>
...
export function AirfoilTraceLab() {
  ...
  return <Investigation
    ...
    question={`Development row ${inferenceFixture.rowId} under the saved final ridge model at λ = 0.001. Edit the five physical measurements, watch the predicted sound-pressure level change, then inspect the twenty polynomial terms and their signed contributions.`}
    ...
    <p className="lesson-live-note">{`The prediction is ${fixed(trace.prediction, 9)} dB, ${change === 0 ? 'unchanged from' : `a change of ${signed(change, 6)} dB from`} the recorded row.`}</p>

    {<>
      <div className="rg-stage">
        <h4>1. Five physical measurements</h4>
        <Table caption="The edited row against the recorded one" headings={['measurement', 'unit', 'recorded', 'now', 'observed range']} rows={rawFeatureLabels.map((label, index) => [label, rawFeatureUnits[index], round(inferenceFixture.rawFeatures[index], 7), round(active.raw[index], 7), `${rawFeatureRanges[index][0]} to ${rawFeatureRanges[index][1]}`])} rowClass={index => changed[index] ? 'is-selected' : undefined} />
      </div>
      <div className="rg-stage">
        <h4>2. Twenty polynomial terms, in the model's own order</h4>
        <p className="rg-caption">
          {touched.size === 0 ? 'Nothing is edited, so no term is highlighted.' : `Changing ${rawFeatureLabels.filter((label, index) => changed[index]).join(' and ')} moves ${touched.size} of the twenty terms together: its own value, its square and every product it takes part in.`}
        </p>
        <div className="rg-chips">
          {featureNames.map((name, index) => <span key={name} className={`rg-chip${touched.has(index) ? ' is-touched' : ''}`}>{name}</span>)}
        </div>
      </div>
      <div className="rg-stage">
        <h4>3. Signed contributions, all in decibels</h4>
        <SignedBars caption="Each term's standardized value times its saved coefficient. The raw terms carry compound units; every contribution here is in target decibels." describe={`Twenty signed contributions in decibels: ${ranked.slice(0, 5).map(item => `${item.name} contributes ${round(item.value, 6)}`).join('; ')}, and fifteen more.`} items={featureNames.map((name, index) => ({
      name,
      value: trace.contributions[index]
...
export function DropoutLab() {
  ...
  return <Investigation
    ...
    question="Two contributions, one target and one keep probability. Change the keep probability and watch the exact mask tree, mean noisy output and expected loss respond together."
    ...
    <p className="lesson-live-note">{`The clean prediction is ${round(enumeration.cleanPrediction, 9)} and the mean noisy prediction is ${round(enumeration.expectedPrediction, 9)}; the clean half-loss is ${round(enumeration.cleanLoss, 9)} and the expected noisy half-loss ${round(enumeration.expectedLoss, 9)}. The analytic excess is ${round(enumeration.analyticExtra, 9)}; subtracting two nearly equal floating-point losses may round this away.`}</p>

    {<>
      <div className="rg-stage">
        <h4>The four masks, with their actual probabilities</h4>
        <svg viewBox="0 0 340 270" role="img" aria-label={`A probability tree with four branches. ${enumeration.branches.map(branch => `mask ${branch.mask.join(' and ')} has probability ${round(branch.probability, 6)}, output ${round(branch.prediction, 6)} and half-loss ${round(branch.halfSquaredLoss, 6)}`).join('; ')}. Each branch carries a bar of its own probability, so the four are not drawn as equal shares; a branch drawn with a dashed outline has probability zero and cannot occur.`}>
        ...
        return <g key={branch.mask.join('')}>
              ...
              <rect className="rg-lane" x="148" y={top} width="186" height="50" rx="3" strokeDasharray={branch.possible ? undefined : '4 3'} />
      ...
      <Table caption="Every branch: the rescaled inputs, the output and what it contributes to the expectation" headings={['mask', 'probability', 'rescaled inputs', 'noisy output', 'half-squared loss', 'probability × loss']} rows={enumeration.branches.map(branch => [`(${branch.mask.join(', ')})`, round(branch.probability, 9), branch.scaled.map(value => round(value, 6)).join(', '), round(branch.prediction, 9), round(branch.halfSquaredLoss, 9), round(branch.probability * branch.halfSquaredLoss, 9)])} rowClass={index => enumeration.branches[index].possible ? undefined : 'is-zero'} />
      ...
      <div className="rg-panels is-pair">
        <div className="rg-panel">
          ...
          <svg viewBox="0 0 300 140" role="img" aria-label={`The distinct outputs and their combined probabilities: ${outputs.map(entry => `${round(entry.value, 6)} with probability ${round(entry.probability, 6)} from mask${entry.masks.length > 1 ? 's' : ''} ${entry.masks.map(mask => mask.join(' and ')).join(', ')}`).join('; ')}. The mean is ${round(enumeration.expectedPrediction, 6)} and the clean prediction is ${round(enumeration.cleanPrediction, 6)}.`}>
            ...
            {outputs.map((entry, index) => <g key={entry.masks.map(mask => mask.join('')).join('-')}>
                <line className="rg-stem is-penalty" x1={place(entry.value)} x2={place(entry.value)} y1="104" y2={104 - 76 * entry.probability} />
        ...
        <div className="rg-panel">
          ...
          <Table caption="Summed over branches, and from the closed formula" headings={['quantity', 'value']} rows={[['clean prediction wᵀx', round(enumeration.cleanPrediction, 9)], ['mean noisy prediction', round(enumeration.expectedPrediction, 9)], ['clean half-squared loss', round(enumeration.cleanLoss, 9)], ['expected noisy half-squared loss', round(enumeration.expectedLoss, 9)], ['difference, from the enumeration', round(enumeration.extraLoss, 9)], ['(1 − q)/(2q) × Σ (wⱼxⱼ)²', round(enumeration.analyticExtra, 9)]]} />
          <p>
            {enumeration.agrees ? 'The two routes agree within the floating-point check tolerance; the identity is exact in real arithmetic. Subtracting nearly equal losses can lose relative accuracy, so use the analytic excess for very small effects.' : 'These two floating-point routes disagree beyond the check tolerance; inspect numerical accuracy before drawing a conclusion.'}
