...
import {
  batchPredictionState, bayesianNumber as number, betaPriors, betaUpdateState,
  exposureUpdateState, normalPrecisionState, predictivePatternState, sequencePatterns,
} from '../../data/bayesian-inference-models.js';
import './bayesian-inference-labs.css';

const gold = '#e5b95f', blue = '#79bbc9', green = '#a0c9ac';
...
function Investigation({ id, title, prediction, children, reset }) {
  const heading = useId();
  return <section className="bayesian-lab lesson-lab" data-investigation={id} aria-labelledby={heading}>
...
function DensityPlot({ points, maxX = 1, shade, caption, axis }) {
  const ymax = Math.max(...points.flatMap(point => [point.prior, point.posterior])) * 1.08;
  const x = value => 48 + 280 * value / maxX, y = value => 190 - 155 * value / ymax;
...
export function BetaEvidenceFigure() {
  return <figure className="bayesian-inline" aria-label="Separate observed successes and failures update separate Beta shapes">
    ...
    <div className="bayesian-outcomes">{Array.from({ length: 10 }, (_, index) => <span className={index < 8 ? 'success' : 'failure'} key={index}>{index < 8 ? 'S' : 'F'}</span>)}</div>
...
export function BayesianUpdateLab() {
  const [prior, setPrior] = useState('moderate'), [successes, setSuccesses] = useState(8), [failures, setFailures] = useState(2);
  const [threshold, setThreshold] = useState(.7), [level, setLevel] = useState(.95);
  const state = useMemo(() => betaUpdateState(prior, successes, failures, threshold, level), [prior, successes, failures, threshold, level]);
  return <Investigation id="beta-update" title="Keep the whole posterior visible" prediction="With the same 8/10 observations, will a stronger middle prior move the mean toward .8 or toward .5?" reset={() => { setPrior('moderate'); setSuccesses(8); setFailures(2); setThreshold(.7); setLevel(.95); }}>
    ...
    <Readout values={[
      ['Posterior mean / next-success chance', number(state.mean, 6)], ['Mode in θ coordinates', number(state.mode, 6)],
      ['Equal-tailed credible interval', `[${number(state.low, 6)}, ${number(state.high, 6)}]`],
      [`P(θ > ${number(threshold)} | data)`, number(state.above, 6)],
      ['Observed fraction', state.sampleRate === null ? 'No observations' : number(state.sampleRate, 6)],
    ]} />
...
export function BatchPredictionLab() {
  const [posterior, setPosterior] = useState('10,4'), [size, setSize] = useState(10), [threshold, setThreshold] = useState(9);
  ...
  return <Investigation id="batch-prediction" title="Same expected count, different tail risk" prediction="For ten future visitors, does fixing θ at its posterior mean preserve the chance of at least nine successes?" reset={() => { setPosterior('10,4'); setSize(10); setThreshold(9); }}>
    <div className="bayesian-controls"><label className="bayesian-field">Rate uncertainty<select aria-label="Rate uncertainty" value={posterior} onChange={event => setPosterior(event.target.value)}><option value="10,4">Beta(10, 4): original posterior</option><option value="100,40">Beta(100, 40): same mean, stronger concentration</option><option value="1,1">Beta(1, 1): no data, uniform prior</option></select></label>
      <Slider label="Future batch size" value={size} setValue={value => { setSize(value); setThreshold(Math.min(value, threshold)); }} min={1} max={20} />
...
export function GammaExposureLab() {
  const [count1, setCount1] = useState(3), [count2, setCount2] = useState(6), [hours1, setHours1] = useState(.5), [hours2, setHours2] = useState(2);
  const state = useMemo(() => exposureUpdateState(count1, hours1, count2, hours2), [count1, count2, hours1, hours2]);
  return <Investigation id="gamma-exposure" title="How much observation time produced those events?" prediction="Keep both event counts fixed and double an observation window. Which direction should the inferred events-per-hour rate move?" reset={() => { setCount1(3); setCount2(6); setHours1(.5); setHours2(2); }}>
    <div className="bayesian-controls">
...
export function DirichletCompositionFigure() {
  const labels = ['Category A', 'Category B', 'Category C'], colors = [gold, blue, green];
  return <figure className="bayesian-inline" aria-label="Three category probabilities stay on a simplex and sum to one">
    ...
    <div className="bayesian-composition" role="img" aria-label="Posterior mean probabilities: seven thirteenths, four thirteenths, two thirteenths">{[7, 4, 2].map((value, index) => <span key={value} style={{ width: `${value / 13 * 100}%`, background: colors[index] }} />)}</div>
    <div className="bayesian-category-key">{labels.map((label, index) => <span key={label}><i style={{ background: colors[index] }} />{label}: {[7, 4, 2][index]}/13</span>)}</div>
...
export function NormalPrecisionLab() {
  const [priorSd, setPriorSd] = useState(1), [noiseSd, setNoiseSd] = useState(2), [mean, setMean] = useState(4), [count, setCount] = useState(4);
  const state = useMemo(() => normalPrecisionState(0, priorSd, mean, count, noiseSd), [priorSd, noiseSd, mean, count]);
  const low = Math.min(...state.intervals.map(item => item.low)) - .5, high = Math.max(...state.intervals.map(item => item.high)) + .5;
  const x = value => 34 + 290 * (value - low) / (high - low);
  return <Investigation id="normal-precision" title="A better-known mean still produces noisy readings" prediction="If you collect more genuinely independent measurements with the same average, must the next individual reading become nearly certain?" reset={() => { setPriorSd(1); setNoiseSd(2); setMean(4); setCount(4); }}>
    <div className="bayesian-controls">
...
export function PredictivePatternLab() {
  const [pattern, setPattern] = useState('clustered'), [conditioning, setConditioning] = useState('posterior');
  ...
  return <Investigation id="predictive-patterns" title="The posterior can miss an ordering problem" prediction="Reorder the same eight successes and two failures. Does the common-rate Beta posterior change? Does the visible pattern change?" reset={() => { setPattern('clustered'); setConditioning('posterior'); }}>
