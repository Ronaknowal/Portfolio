...
export function Investigation({ title, question, children, onReset }) {
  const id = useId();
  return <section className="db-investigation" aria-labelledby={id} data-live-exploration>
...
export function DbscanTrailLab() {
  ...
  const loadPreset = presetKey => { setPending({ preset: presetKey, points: presets[presetKey].points.map(p => [...p]), eps: presets[presetKey].eps, m: presets[presetKey].m, order: 'original', start: '' }); setSelected(presetKey === 'trail' ? 8 : 0);   };
  
  return <Investigation title="Can one row join two groups together?" question="Edit any coordinate, the radius or the count, choose a visiting order, and watch the core components, row types and connections update together." onReset={() => loadPreset('trail')}>
    ...
    <p className="lesson-live-note">{`${nameOf(selected, names)} has ${currentFit.counts[selected]} neighbour${currentFit.counts[selected] === 1 ? '' : 's'} including itself${currentFit.types[selected] === 'border' ? `, and its core neighbours belong to component${currentFit.eligible[selected].length > 1 ? 's' : ''} ${currentFit.eligible[selected].join(' and ')}` : ''}.`}</p>
    {<>
...
export function DbscanMetricLab() {
  ...
  return <Investigation title="Change a unit, or change the question?" question="Multiply one or both coordinates by a factor and keep or change the radius. Inspect whether every neighbourhood decision stays the same, and select a pair to see the distance arithmetic." onReset={() => { setPending(initial);   setPair([1, 4]); }}>
    <div className="db-controls">
    ...
    <div className="db-buttons">
      ...
      <span>Each preset updates the controls, neighbourhoods and distance comparison immediately.</span>
    </div>
    <p className="lesson-live-note">{same.same ? `Every row keeps exactly the same neighbour set. Your cited pair ${names[pairIndices[0]]}–${names[pairIndices[1]]} is ${number(pairBefore)} apart before and ${number(pairAfter)} after, against radii ${number(pending.baseEps)} and ${number(pending.eps)}.` : `Rows whose neighbour sets changed: ${same.differing.map(i => names[i]).join(', ')}. Your cited pair ${names[pairIndices[0]]}–${names[pairIndices[1]]} is ${number(pairBefore)} apart before (radius ${number(pending.baseEps)}) and ${number(pairAfter)} after (radius ${number(pending.eps)}).`}</p>
    {<>
      ...
      <GlyphLegend components />
      <Table caption="Pairwise distances before → after, with whether each pair is within the radius" headings={['pair', 'before', 'within ε before', 'after', 'within ε after']} rows={rows.flatMap((_, i) => rows.slice(i + 1).map((__, k) => {
    const j = i + 1 + k;
    return [`${names[i]}–${names[j]}`, number(before.matrix[i][j]), before.matrix[i][j] <= pending.baseEps ? 'yes' : 'no', number(after.matrix[i][j]), after.matrix[i][j] <= pending.eps ? 'yes' : 'no'];
  }))} highlight={index => {
    const pairs = rows.flatMap((_, i) => rows.slice(i + 1).map((__, k) => [i, i + 1 + k]));
    return pairs[index][0] === Math.min(...pairIndices) && pairs[index][1] === Math.max(...pairIndices);
  }} />
    </>}
...
export function DbscanIrisLab() {
  ...
  return <Investigation title="A better score for fewer flowers?" question="Change radius and minimum count for all four Iris measurements and watch groups, noise and coverage update together. The optional species overlay is diagnostic only; species never enter the fit." onReset={() => { setPending(initial);   setAxes([2, 3]); setShowSpecies(false); setRow(41); setSaved(null); }}>
    <div className="db-controls">
    ...
    <p className="lesson-live-note">{`${report.clusters} group${report.clusters === 1 ? '' : 's'} of sizes ${report.sizes.join(', ') || '—'}; ${report.assignedIds.length} of 150 assigned (${percent(report.coverage)}), ${report.noiseCount} noise.`}</p>
    {<>
      <Table caption="Whole-collection report for the current setting" headings={['quantity', 'value']} rows={[['groups (sizes)', `${shown.clusters} (${shown.sizes.join(', ') || '—'})`], ['core / border / noise', `${shown.coreCount} / ${shown.borderCount} / ${shown.noiseCount}`], ['coverage', `${shown.assignedIds.length} / 150 = ${percent(shown.coverage)}`], ['silhouette, assigned rows only', shown.silhouetteAssigned === null ? `undefined: needs 2 ≤ groups < assigned rows (groups ${shown.clusters}, assigned ${shown.assignedIds.length})` : `${number(shown.silhouetteAssigned, 3)} on ${shown.assignedIds.length} rows`], ['ARI against species, all 150 rows, −1 as one label', showSpecies ? number(shown.ariAllRows, 3) : 'hidden until species are revealed'], ['ARI against species, assigned rows only', showSpecies ? shown.ariAssigned === null ? 'undefined' : `${number(shown.ariAssigned, 3)} on ${shown.assignedIds.length} rows` : 'hidden until species are revealed']]} />
      <div className="db-buttons">
      ...
      <SquarePlot points={projected} title={`Projection of the four-feature fit onto ${irisFeatures[axes[0]]} and ${irisFeatures[axes[1]]} (${pending.representation === 'raw' ? 'cm' : 'standardized units'})`} xLabel={irisFeatures[axes[0]]} yLabel={irisFeatures[axes[1]]} pad={0.08} describe={`150 flowers projected onto two of the four features; glyphs give core, border and noise from the four-feature fit${showSpecies ? ', with species drawn as shapes' : ''}.`}>
        {project => projected.map((p, i) => {
      const [x, y] = project(p);
      const color = shown.fit.labels[i] >= 0 ? componentColors[shown.fit.labels[i] % componentColors.length] : undefined;
      if (showSpecies) {
        const s = irisSpecies[i];
        const fill = shown.fit.labels[i] >= 0 ? color : '#da9c86';
        if (s === 0) return <circle key={i} cx={x} cy={y} r="3.2" fill={fill} fillOpacity=".85" />;
        if (s === 1) return <rect key={i} x={x - 3} y={y - 3} width="6" height="6" fill={fill} fillOpacity=".85" />;
        return <path key={i} d={`M${x},${y - 4}l3.6,6.5h-7.2z`} fill={fill} fillOpacity=".85" />;
      }
...
export function DenseStrip({ points, fit, names, from, to, caption }) {
  const shown = points.map((point,index) => [point[0],index]).filter(([x]) => x >= from && x <= to);
...
export function DbscanIntervalLab() {
  ...
  return <Investigation title="Can any radius meet both requirements?" question="Three intended groups on one line with m = 3: two dense groups and one you can make sparser or denser. Move the spacing and radius controls to inspect whether one radius can keep the two dense groups apart while making the third group viable." onReset={() => { setPending(initial);   }}>
    <div className="db-controls">
    ...
    <p className="lesson-live-note">{`The right group is viable from ε = ${number(fixture.lower)} (closed); the two dense groups stay separate below ε = ${number(fixture.upper)} (open, because the closed boundary joins them at exactly that radius). ${fixture.exists ? `Every ε in [${number(fixture.lower)}, ${number(fixture.upper)}) works.` : 'The first requirement begins where the second has already failed: the intervals do not overlap.'} At ε = ${number(pending.eps)}: ${fit.clusters} component${fit.clusters === 1 ? '' : 's'}, ${fit.noiseIds.length} noise.`}</p>
    {<>
      <div className="db-scroll"><svg className="db-interval-svg" viewBox="0 0 320 96" role="img" aria-label={`Two requirement intervals on a radius axis from 0 to 2: right group viable from ${number(fixture.lower)} upward; dense groups separate below ${number(fixture.upper)}. ${fixture.exists ? 'They overlap.' : 'They do not overlap.'}`} style={{
      width: '100%',
      maxWidth: 560,
      display: 'block',
      margin: '1rem auto'
        ...
        <line className="db-interval" x1={axisX(fixture.lower)} x2={axisX(2)} y1="28" y2="28" style={{
        stroke: '#8eb9a5'
      }} /><circle cx={axisX(fixture.lower)} cy="28" r="4" fill="#8eb9a5" /><text x={axisX(Math.min(fixture.lower + 0.05, 1.5))} y="20">right group viable: ε ≥ {number(fixture.lower)}</text>
        <line className="db-interval" x1={axisX(0)} x2={axisX(fixture.upper)} y1="56" y2="56" style={{
        stroke: '#e7b94a'
      }} /><circle cx={axisX(fixture.upper)} cy="56" r="4" fill="#0b0f10" stroke="#e7b94a" strokeWidth="2" /><text x={axisX(0.05)} y="48">dense groups separate: ε {'<'} {number(fixture.upper)}</text>
