...
export function PlotCoordinateLab() {
  ...
  const model = coordinateModel(view),
    point = model.points[selected];
  ...
  return <Investigation id="plot-coordinates" kicker="DATA → SCALE → POSITION" title="Can a point move without its measurement changing?">
    <p>A measured 10 ms and B measured 20 ms. Only the y-axis mapping changes. Follow one value from the data into the height of its plotted marker.</p><p className="lesson-note">Change axis limits and switch between linear and logarithmic scales. Compare the same values with the changing visual spacing.</p>
    ...
    <button type="button" onClick={() => {
      setView('full');
      setSelected(0);
    }}>Reset mapping</button><p className="lesson-note">This bounded model illustrates coordinate mapping for two fixed values. Dot positions are shown; a truncated bar would also hide part of its zero-based length. Real figures can contain more panels, transforms and annotations.</p>
  </Investigation>;
}
export function PlotHistogramLab() {
  const [layout, setLayout] = useState('three'),
    ...
    [selected, setSelected] = useState(0);
  const bins = histogramModel(layout, density),
    bin = bins[Math.min(selected, bins.length - 1)],
    maxHeight = Math.max(...bins.map(b => b.height)) * 1.2;
  return <Investigation id="plot-histogram" kicker="OBSERVATIONS → BINS → AREA" title="Does a taller bin always contain more observations?">
    ...
    <p>Source observations: {histogramValues.map((value, i) => <span className="nt-chip" key={i} style={{
        borderColor: bin.indices.includes(i) ? '#e5bd66' : '#454033',
        color: bin.indices.includes(i) ? '#f0cc79' : '#aaa293'
      }}>{value}{bin.indices.includes(i) ? ' ↓' : ''}</span>)}</p>
    ...
    <button type="button" onClick={() => {
      ...
      setDensity(false);
      setSelected(0);
    }}>Reset histogram</button><p className="lesson-note">Bins include the lower edge and exclude the upper edge, except the last upper edge is included. Repeated values are separate observations. All six points lie inside these edges; other edges could exclude data.</p>
  </Investigation>;
}
export function PlotIntervalLab() {
  const [kind, setKind] = useState('sd'),
    [repeated, setRepeated] = useState(false);
  const model = intervalModel(kind, repeated),
    x = v => 34 + (v - 8) * 36;
  return <Investigation id="plot-interval" kicker="LABEL THE QUANTITY, NOT JUST THE BAR" title="Same points and mean: why did the interval shrink?">
    <p>Three runs produced 10, 12 and 14 ms. A bar labelled sample SD describes their spread. A bar labelled SEM uses SD divided by √n to estimate variability of the mean under an independent-sampling model.</p><p className="lesson-note">Switch between SD and SEM and vary the observations. Separate the spread of runs from the uncertainty of an estimated mean.</p>
    ...
    <button type="button" onClick={() => {
      setKind('sd');
      setRepeated(false);
    }}>Reset interval</button><p className="lesson-note">Neither choice here is labelled a 95% confidence interval. Sample SD uses n−1. Whether independent sampling is justified is a separate scientific question.</p>
  </Investigation>;
}
