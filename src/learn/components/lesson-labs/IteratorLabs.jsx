...
export function CursorOwnershipLab() {
  const [shared, setShared] = useState(false),
    ...
    trace = cursorTrace(shared, empty),
    cursorState = trace[step];
  return <Investigation id="iterator-ownership" kicker="ONE COLLECTION · HOW MANY POSITIONS?" title="Does b have its own next item?">
    ...
    <p className="lesson-note">Compare shared versus independent iterators and step through calls from a and b. Follow cursor ownership to see which value each call consumes.</p>
    <div className="nt-controls"><label>Cursor setup<select value={String(shared)} onChange={e => {
          setShared(e.target.value === 'true');
          setStep(0);
        }}><option value="false">a = iter(values); b = iter(values)</option><option value="true">a = iter(values); b = a</option></select></label><label>Source list<select value={String(empty)} onChange={e => {
          setEmpty(e.target.value === 'true');
          setStep(0);
        }}><option value="false">[18, 21, 24]</option><option value="true">[]</option></select></label></div>
...
export function GeneratorFrameLab() {
  const [action, setAction] = useState('exhaust'),
    [step, setStep] = useState(0),
    trace = generatorTrace(action),
    generatorState = trace[step];
  return <Investigation id="generator-frame" kicker="CALLER ⇄ SUSPENDED FUNCTION" title="Where does execution wait after yield?">
    <p>This generator assigns remaining = 2, yields it while positive, then subtracts 1 after each resumption. A finally block records cleanup. Each step is one caller operation: creation, next, or close.</p><p className="lesson-note">Step across each yield and resume point. Inspect remaining work and the precise point at which cleanup runs.</p>
    <div className="nt-controls"><label>Caller actions<select value={action} onChange={e => {
          setAction(e.target.value);
          setStep(0);
        }}><option value="exhaust">next until exhausted, then next again</option><option value="close-started">next once, then close</option><option value="close-created">close before first next</option></select></label></div>
...
export function PullPipelineLab() {
  const [limit, setLimit] = useState(2),
    ...
    [step, setStep] = useState(0),
    trace = pipelineTrace(limit, bad),
    pipelineState = trace[step];
  return <Investigation id="iterator-pipeline" kicker="DEMAND GOES UPSTREAM · DATA COMES BACK" title="Can one next call read several lines?">
    <p>A consumer wants one or two numeric readings. Its source has four lines; line 2 is blank. Select malformed line 3 to see when a delayed error appears.</p><p className="lesson-note">Change the requested result count and follow source consumption, parsing and filtering. Inspect when the bad third line becomes observable.</p>
    <div className="nt-controls"><label>Results requested<select value={limit} onChange={e => {
          setLimit(Number(e.target.value));
          setStep(0);
        }}><option value="1">1 reading</option><option value="2">2 readings</option></select></label><label>Line 3<select value={String(bad)} onChange={e => {
          setBad(e.target.value === 'true');
          setStep(0);
        }}><option value="false">24 · valid</option><option value="true">bad · invalid</option></select></label></div>
