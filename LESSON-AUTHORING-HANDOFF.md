...
Updated 11 September 2026. This is the active entry point. The user's current request defines scope; historical reports and scratch files do not define a new queue.
...
1. Read [the teaching standard](LESSON-TEACHING-STANDARD.md), starting with its [six-stage authoring workflow](LESSON-TEACHING-STANDARD.md#six-stage-authoring-workflow) and **Keep verification bounded and reusable**. It owns the stage responsibilities/completion conditions, pedagogy, depth, research, visuals, practice and stopping rules.
...
4. Read the active implementation record and ledger below. Open the current topic's source and retrieve its plan with `node scripts/build-curriculum-inventory.mjs --topic "Exact title or stable ID"`. Assess the returned destination notes.
...
## Latest authorized topic — complete
...
Current next action: let the user review this lesson and follow their next requested scope. The actual next module entry is **PCA & Dimensionality Reduction**, position 12; retrieve its plan and destination notes when it is authorized. No additional topic is active. User acceptance is separate from implementation review. Preserve IDs and module order, and do not reopen this completed topic or the previous ten-topic increment merely on resume.
...
Close the actual outstanding findings, record source-versioned author/independent evidence, and run the required final integration for the authorized increment. A build, a registered page or a lab count does not establish teaching quality. Unknown or interrupted checks stay unknown until resolved; passed unchanged checks are reused.

Update the current ledger and this implementation record's next action, remove disposable working material, and report concrete changes with relevant verification and limits. Preserve referenced final evidence and shared tools. Do not deploy unless requested.
