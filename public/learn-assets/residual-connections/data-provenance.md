...
Research packet prepared 2026-09-12; website implementation and complete native replay, 2026-09-22. All attached files are retained production inputs.
...
The original complete fitting program ran once during writing. Later exact fixture additions were recomputed with the same `mechanisms()` function and merged into the output. During implementation on 2026-09-22, all 39 fits and all mechanism fixtures were rerun from the unchanged complete program; the decoded output matches the retained writing record exactly. Browser integration and independent review are separate records, not claims made by this dataset.
...
The website preserves these distinctions. Browser models are separately compared with these native fixtures; the dataset is not evidence of rendered interaction quality. Any added experiment or data selection must get its own provenance and outputs.
