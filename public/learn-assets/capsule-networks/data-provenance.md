# Current website implementation — 22 September 2026

The complete revision-3 lesson is implemented with explicit NumPy/PyTorch source, live routing/squash/EM investigations, recorded paired fits and actual frozen-model pixel/decoder exploration. The six 600-update CPU fits, constructed mechanics and independent NumPy inference were executed again during implementation and reproduce all retained JSON values exactly. Native and browser-model verification records are linked below; formal independent review and final production browser integration are tracked separately in the repository.

The browser model is the single seed-1, three-routing-step fitted model. `frozen-model.f32` stores its 47,184 original float32 weights exactly (188,736 bytes); `frozen-model.json` declares offsets, lengths, model identity, two original images and the binary SHA-256. Both load only when the learner opens the real-image investigation. The browser uses double-precision accumulation for bounded inference; 14 native float64 changed-input comparisons differ by at most 3.34e-16 in capsule coordinates. Native float32 versus independent double rounding differences in the historical record remain applicable. The browser never fits models or fetches the full multi-run evidence to operate this lab.

- [Native checks, exact source hashes and environment](native-verification.json)
- [Complete experiment output](calculated-inputs.json)
- [Constructed mechanics output](mechanics-results.json)
- [Independent NumPy reconstruction and edits](author-check-results.json)

The fixed-score plots use a topic-only 21,775-byte uncompressed subset of the executed measurements. Node's warmed bounded-inference author check is recorded in the repository; it is not a browser or phone latency claim. Actual pixel/latent edits and responsive geometry require the separate final production check.

The following preparation record is historical provenance. Its statements about implementation not starting describe the earlier content-only checkpoint and are superseded by this current section. Dataset attribution, learning conditions, numerical fixtures and scope limits remain unchanged.
