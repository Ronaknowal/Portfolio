...
Research/write packet originally prepared 2026-09-12. The original experiment and its data remain unchanged. On 22 September the complete 27-fit/100-draw program was rerun, and the browser lesson and bounded live mechanisms were implemented. Formal independent review, production browser checks and visual closure remain pending at this author checkpoint.
...
## Original content-first retention checkpoint — 12 September
...
## Implementation author checkpoint — 22 September

All 7,629 compared numerical values from the complete replay matched the retained record exactly. The downloadable `dropout-library-checks.py` was also executed with PyTorch 2.14.0+cpu and torchvision 0.29.0+cpu; its forward, gradient, boundary, grouped-mask and locked-feature checks passed. Torchvision was isolated in a task-owned dependency folder, without changing the shared lesson runtime. The browser computes tiny exact mask mechanisms and selects saved fits/MC draws; it performs no model fitting. Current source-bound author evidence lives in `docs/teaching/evidence/dropout-implementation/` in the repository. Historical deferrals above describe the content-only checkpoint, while final browser and independent-review closure are tracked separately by the current implementation record.
